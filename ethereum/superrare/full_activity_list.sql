...
CREATE OR REPLACE FUNCTION superrare.create_full_activity_list(start_ts timestamptz, end_ts timestamptz=now()) RETURNS numeric
...
WITH all_activities AS ( --https://etherscan.io/tx/0xa2c07597bb4350d1084e78c62059d30f2179d58eade0cb0612da4ffb0251d2bb
    ...
    AND ethereum.logs.block_time >= start_ts 
    AND ethereum.logs.block_time < end_ts
    UNION ALL
    SELECT
        tx_hash,
        date_trunc('minute', ethereum.logs.block_time) AS time,
...
VALUES ('14 1 * * *', $$SELECT superrare.create_full_activity_list(SELECT max(block_time) - interval '2 days' from superrare.superrare_full_activity_list);$$)
