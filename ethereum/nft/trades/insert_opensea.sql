...
SELECT
    ...
    cast(wc.original_amount / 10.0 ^ erc20.decimals as numeric) AS original_amount,
    cast(wc.original_amount as numeric) AS original_amount_raw,
    cast(wc.original_amount / 10.0 ^ erc20.decimals * p.price / peth.price as numeric) AS eth_amount,
    -- Royalty Fees (in %, amount in original currency and in USD)
    ROUND(cast((wc.fees - 2.5*(wc.original_amount / 10.0 ^ erc20.decimals)/100.0) * (100.0) / NULLIF(wc.original_amount / 10 ^ erc20.decimals,0) as numeric),7) AS royalty_fees_percent,
    ROUND(cast(wc.fees - 2.5*(wc.original_amount / 10.0 ^ erc20.decimals)/100.0 as numeric),7) AS original_royalty_fees,
    ROUND(cast(wc.fees * p.price - 2.5 * (wc.original_amount / 10.0 ^ erc20.decimals * p.price) / 100.0 as numeric),7) AS usd_royalty_fees,
    -- Platform Fees (in %, amount in original currency and in USD)
    2.5 AS platform_fees_percent,
    ROUND(cast(2.5*(wc.original_amount / 10.0 ^ erc20.decimals)/100.0 as numeric),7) AS original_platform_fees,
    ROUND(cast(2.5*(wc.original_amount / 10.0 ^ erc20.decimals * p.price)/100.0 as numeric),7) AS usd_platform_fees, 
