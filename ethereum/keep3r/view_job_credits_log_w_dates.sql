CREATE OR REPLACE VIEW dune_user_generated.view_job_credits_log_w_dates AS (
		SELECT COALESCE("timestamp", s.dd) AS timestamp,
			...
			period_credits
		FROM (
				dune_user_generated.view_job_credits_log AS df
				RIGHT JOIN (
					SELECT job,
						dd
					FROM ROWS
					FROM (
						...
						INNER JOIN (
							...
							FROM dune_user_generated.view_job_credits_log
							GROUP BY job
