...
CREATE OR REPLACE FUNCTION zeroex.insert_0x_api_fills (start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
WITH zeroex_tx_raw AS (
      ...
      direct_uniswapv3 AS (
    	...
    	total_volume AS (
          ...
          WHERE all_tx.block_time >= start_ts
              AND all_tx.block_time < end_ts
      ),
        rows AS (
...
--backfill
SELECT zeroex.insert_0x_api_fills('2020-01-29', (SELECT now()), (SELECT max(number) FROM ethereum.blocks WHERE time < '2020-01-29'), (SELECT MAX(number) FROM ethereum.blocks where time < now() - interval '20 minutes')) WHERE NOT EXISTS (SELECT * FROM zeroex.view_0x_api_fills LIMIT 1);


INSERT INTO cron.job (schedule, command)
VALUES ('*/15 * * * *', $$SELECT zeroex.insert_0x_api_fills((SELECT max(block_time) - interval '2 days' FROM zeroex.view_0x_api_fills), (SELECT now() - interval '20 minutes'), (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '2 days' FROM zeroex.view_0x_api_fills)), (SELECT MAX(number) FROM ethereum.blocks where time < now() - interval '20 minutes'));$$)
