...
WITH lbp_pools AS (
    ...
    first_rebind AS (
        SELECT u.*
        FROM (SELECT t.*, call_block_number, call_block_time, ROW_NUMBER() OVER (PARTITION BY pool ORDER BY call_block_number) AS ranking
        FROM balancer."BPool_call_rebind" r INNER JOIN tokens_sold t ON t.pool = r.contract_address AND t.token = r.token WHERE call_success) u
        WHERE ranking = 1
    ),
    
    last_rebind AS (
        SELECT u.*
        FROM (SELECT t.*, call_block_number, call_block_time, ROW_NUMBER() OVER (PARTITION BY pool ORDER BY call_block_number DESC) AS ranking
        FROM balancer."BPool_call_rebind" r INNER JOIN tokens_sold t ON t.pool = r.contract_address AND t.token = r.token WHERE call_success) u
        WHERE ranking = 1
    ),
    ...
    limit_blocks AS (
        ...
        FROM last_rebind r
        INNER JOIN first_rebind f ON f.pool = r.pool AND f.token = r.token
