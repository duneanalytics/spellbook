CREATE OR REPLACE FUNCTION dex.insert_liquidity_dodo_v2(start_ts timestamptz, end_ts timestamptz=now()) RETURNS integer
...
dodo_pools AS (
	...
	SELECT
		...
		"baseToken" AS token0,
		"quoteToken" AS token1
	FROM dodo."DVMFactory_evt_NewDVM"
),
dex_wallet_balances AS (
	SELECT
		...
		WHERE project = 'DODO' AND version = '2' AND liq.day >= start_ts - interval '3 days'
),
