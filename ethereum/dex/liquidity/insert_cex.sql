CREATE OR REPLACE FUNCTION dex.insert_liquidity_cex(start_ts timestamptz, end_ts timestamptz=now()) RETURNS integer
...
dex_wallet_balances AS (
    ...
    INNER JOIN dex.cex_pools dex ON balances.token_address = dex.token AND balances.wallet_address = dex.pool
    ...
    WHERE project = 'Centralized Exchanges' AND version = '1' AND liq.day >= start_ts - interval '3 days'
),
balances AS (
    SELECT
        wallet_address,
...
rows AS (
    ...
    SELECT
        day,
        pools.token_name AS token_symbol,
        token_amount_raw / 10 ^ pools.decimals AS token_amount,
        pools.pool_name AS pool_name,
        project,
        version,
        category,
        token_amount_raw,
        token_amount_raw / 10 ^ pools.decimals * p.price AS usd_amount,
        token_address,
        pool_address,
        token_index,
        token_pool_percentage
    FROM (
        -- Centralized Exchanges v1
        SELECT
            d.day,
            'Centralized Exchanges' AS project,
            '1' AS version,
            'CEX' AS category,
    ...
    LEFT JOIN dex.cex_pools pools ON dexs.token_address = pools.token AND dexs.pool_address = pools.pool
...
-- Centralized Exchanges. Start on '2019-01-01'.
-- fill 2019 - Q1
...
WHERE NOT EXISTS (
    ...
    AND day < '2019-04-01'
    AND project = 'Centralized Exchanges'
    AND version = '1'
);
