CREATE OR REPLACE FUNCTION dex.insert_unifi(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
WITH rows AS (
    ...
    FROM (
        ...
        SELECT
            ...
            t.evt_index
        FROM
            unifi."UnifiPair_evt_Swap" t
        INNER JOIN unifi."UnifiFactory_evt_PairCreated" f ON f.pair = t.contract_address
    ) dexs
...
VALUES ('*/10 * * * *', $$
    SELECT dex.insert_unifi(
        ...
        (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Unifi')),
        (SELECT MAX(number) FROM ethereum.blocks where time < now() - interval '20 minutes'));
$$)
