CREATE OR REPLACE FUNCTION dex.insert_sakeswap(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
WITH rows AS (
    ...
    FROM (
        ...
        SELECT
            ...
            t.evt_index
        FROM
            sakeswap."SakeSwapPair_evt_Swap" t
        INNER JOIN sakeswap."SakeSwapFactory_evt_PairCreated" f ON f.pair = t.contract_address
    ) dexs
...
VALUES ('*/12 * * * *', $$
    SELECT dex.insert_sakeswap(
        ...
        (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Sakeswap')),
        (SELECT MAX(number) FROM ethereum.blocks where time < now() - interval '20 minutes'));
$$)
