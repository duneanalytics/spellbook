CREATE OR REPLACE FUNCTION dex.insert_mstable(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
    ...
    FROM (
        ...
        FROM (
            ...
            SELECT  
                ...
                evt_index
            FROM mstable."Masset_evt_Swapped"
            ...
            SELECT  
                ...
                NULL::integer[] AS trace_address,
                evt_index
            FROM mstable."FeederPool_evt_Swapped"
        ) f
    ) dexs
...
VALUES ('*/12 * * * *', $$
    SELECT dex.insert_mstable(
        ...
        (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='mStable')),
        (SELECT MAX(number) FROM ethereum.blocks where time < now() - interval '20 minutes'));
$$)
