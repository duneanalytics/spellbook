CREATE OR REPLACE FUNCTION dex.insert_indexed_finance(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
    ...
    FROM (
        ...
        FROM (
            ...
            SELECT  
                ...
                evt_index
            FROM indexed_finance."CC10_evt_LOG_SWAP"
            ...
            SELECT  
                ...
                NULL::integer[] AS trace_address,
                evt_index
            FROM indexed_finance."DEFI5_evt_LOG_SWAP"
            ...
            SELECT  
                ...
                NULL::integer[] AS trace_address,
                evt_index
            FROM indexed_finance."degen_evt_LOG_SWAP"
            ...
            SELECT  
                ...
                NULL::integer[] AS trace_address,
                evt_index
            FROM indexed_finance."ERROR_evt_LOG_SWAP"
            ...
            SELECT  
                ...
                NULL::integer[] AS trace_address,
                evt_index
            FROM indexed_finance."ORCL5_evt_LOG_SWAP"
        ) f
    ) dexs
...
VALUES ('*/12 * * * *', $$
    SELECT dex.insert_indexed_finance(
        ...
        (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Indexed Finance')),
        (SELECT MAX(number) FROM ethereum.blocks where time < now() - interval '20 minutes'));
$$)
