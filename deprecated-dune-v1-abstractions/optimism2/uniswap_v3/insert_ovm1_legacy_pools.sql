...
WITH jsons AS (
SELECT
    (unijson->'oldAddress') AS oldAddress,
    (unijson->'newAddress') AS newAddress,
    (unijson->'token0') AS token0,
    (unijson->'token1') AS token1,
    (unijson->'fee') AS fee
    
FROM
(
SELECT json_array_elements(unijson::json) AS unijson
    FROM
        (values --Copy and Paste From the json file in https://tokenlists.org/token-list?url=https://yearn.science/static/tokenlist.json
        ('
[
  {
...
SELECT
REPLACE(REPLACE(token0::text,'0x','\x'),'"','')::bytea AS token0,
REPLACE(REPLACE(token1::text,'0x','\x'),'"','')::bytea AS token1,
REPLACE(fee::text,'"','')::int AS fee,
REPLACE(REPLACE(newaddress::text,'0x','\x'),'"','')::bytea AS pool
