CREATE OR REPLACE FUNCTION seaport.insert_transfers (p_start_ts timestamptz, p_end_ts timestamptz=now()) RETURNS integer
...
-- same source from ethereum/insert_transactions.sql 
-- but modified below
-- 1. ETH -> MATIC
-- 2. WETH -> MATIC 0x0000000000000000000000000000000000001010 -> 0x0000000000000000000000000000000000001010
-- 3. ethereum.transactions -> polygon.transactions
-- 4. first block_number 29585000 -> 29585000
-- 5. get rid of nft_project_name
-- 6. minor update (nft_token, nft_token_identifier, ...)
-- 7. force platform value without zone_address
...
,p1_nft_trades as (
    select a.block_time
          ...
          ,case when a.original_currency_contract = '\x0000000000000000000000000000000000000000'::bytea then '\x0000000000000000000000000000000000001010'
                else a.original_currency_contract
           end as currency_contract
          ,case when a.original_currency_contract = '\x0000000000000000000000000000000000000000'::bytea then 'MATIC'
                else t1.symbol
        ...
        --   ,case when fee_amount > 0 and a.original_currency_contract = '\x0000000000000000000000000000000000000000'::bytea then 'MATIC'
        ...
        --   ,case when royalty_amount > 0 and a.original_currency_contract = '\x0000000000000000000000000000000000000000'::bytea then 'MATIC'
        ...
        --   ,reverted
          ...
          ,NULL::text as param3
      from p1_txn_level a
          left join polygon.transactions tx on tx.hash = a.tx_hash
                                              and tx.block_number > 29585000
                                            and tx.block_time >= p_start_ts
                                            and tx.block_time < p_end_ts
          left join erc20.tokens t1 on t1.contract_address = case when a.original_currency_contract = '\x0000000000000000000000000000000000000000'::bytea then '\x0000000000000000000000000000000000001010'
                                                                  else a.original_currency_contract
                                                              end                                
          left join prices.usd p1 on p1.contract_address = case when a.original_currency_contract = '\x0000000000000000000000000000000000000000'::bytea then '\x0000000000000000000000000000000000001010'
                                                                 else a.original_currency_contract
                                                            end
...
,p2_nft_trades as ( 
    select a.block_time
          ...
          ,case when concat('\x',substr(a.price_token,3,40))::bytea = '\x0000000000000000000000000000000000000000'::bytea then '\x0000000000000000000000000000000000001010'
                else concat('\x',substr(a.price_token,3,40))::bytea
           end as currency_contract
          ,case when concat('\x',substr(a.price_token,3,40))::bytea = '\x0000000000000000000000000000000000000000'::bytea then 'MATIC'
                else t1.symbol
           end as original_currency 
        ...
        --   ,case when evt_fee_amount > 0 and concat('\x',substr(a.price_token,3,40))::bytea = '\x0000000000000000000000000000000000000000'::bytea then 'MATIC'
        ...
        --   ,case when evt_royalty_amount > 0 and concat('\x',substr(a.evt_royalty_token,3,40))::bytea = '\x0000000000000000000000000000000000000000'::bytea then 'MATIC'
      ...
      from p2_transfer_level a
          left join polygon.transactions tx on tx.hash = a.tx_hash 
                                            ...
                                            and tx.block_time >= p_start_ts
                                            and tx.block_time < p_end_ts
          left join erc20.tokens t1 on t1.contract_address = case when concat('\x',substr(a.price_token,3,40))::bytea = '\x0000000000000000000000000000000000000000'::bytea then '\x0000000000000000000000000000000000001010'
                                                                  else concat('\x',substr(a.price_token,3,40))::bytea
                                                              end                                
          left join prices.usd p1 on p1.contract_address = case when concat('\x',substr(a.price_token,3,40))::bytea = '\x0000000000000000000000000000000000000000'::bytea then '\x0000000000000000000000000000000000001010'
                                                                 else concat('\x',substr(a.price_token,3,40))::bytea
                                                            end
                                  and p1.minute = date_trunc('minute', a.block_time)
                                  and p1.minute >= p_start_ts
