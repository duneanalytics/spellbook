CREATE OR REPLACE FUNCTION dex.insert_clipper_coves(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
    ...
    FROM (
        ...
        SELECT
            ...
            CASE WHEN "inAsset" = '\x0000000000000000000000000000000000000000' THEN
                '\x0000000000000000000000000000000000001010'::bytea ELSE "inAsset"
...
VALUES ('*/10 * * * *', $$
    SELECT dex.insert_clipper_coves(
        ...
        (SELECT max(number) FROM polygon.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Clipper' AND version='coves1')),
        (SELECT MAX(number) FROM polygon.blocks where time < now() - interval '20 minutes'));
$$)
