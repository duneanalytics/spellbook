CREATE OR REPLACE FUNCTION dex.insert_quickswap(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
VALUES ('*/10 * * * *', $$
    SELECT dex.insert_quickswap(
        (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Quickswap'),
        (SELECT now() - interval '20 minutes'),
        (SELECT max(number) FROM polygon.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Quickswap')),
        (SELECT MAX(number) FROM polygon.blocks where time < now() - interval '20 minutes'));
$$)
