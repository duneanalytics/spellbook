...
-- Since there is no reliable price feed, we can only assign value of 1 to trades involving stable coins.
WITH stablecoins (address, name) as (
    values (decode('e91D153E0b41518A2Ce8Dd3D7944Fa863463a97d', 'hex'), 'WXDAI'),
           (decode('DDAfbb505ad214D7b80b1f830fcCc89B60fb7A83', 'hex'), 'USDC'),
           (decode('4ECaBa5870353805a9F068101A40E0f32ed605C6', 'hex'), 'USDT'),
           (decode('eeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee', 'hex'), 'xDAI'),
           (decode('44fa8e6f47987339850636f88629646662444217', 'hex'), 'DAI'),
           (decode('0905ab807f8fd040255f0cf8fa14756c1d824931', 'hex'), 'xOWL')
),
     ...
     trades_with_prices AS (
         SELECT evt_block_time               as block_time,
                ...
                (CASE
                     WHEN trades."sellToken" IN (SELECT address FROM stablecoins) THEN 1.0
                     ELSE NULL
                    END)                     as sell_price,
                (CASE
                     WHEN trades."buyToken" IN (SELECT address FROM stablecoins) THEN 1.0
                     ELSE NULL
                    END)                     as buy_price
