with transfers as (
  select
    ...
    tx_hash, tx_digest, tx_from, tx_index, coin_type, transfer_type, "from", to, amount_raw
...
events as (
  select
    ...
    )), '^0x0*([0-9a-f]+)(::.*)$', '0x$1$2') as coin_type_hint,
    regexp_replace(lower(coalesce(
    ...
    )), '^0x0*([0-9a-f]+)(::.*)$', '0x$1$2') as coin_type_in_hint,
    regexp_replace(lower(coalesce(
    ...
    )), '^0x0*([0-9a-f]+)(::.*)$', '0x$1$2') as coin_type_out_hint
  from delta_prod.sui.events e
),
matched as (
  select
    t.transfer_unique_key,
    t.block_month, t.block_date, t.block_time, t.block_number,
    t.tx_hash, t.tx_digest, t.tx_from, t.tx_index, t.coin_type, t.transfer_type, t."from", t.to, t.amount_raw,
    e.event_index, e.event_sender, e.event_package, e.event_type, e.event_module, e.event_name, e.event_type_params,
    e.event_json, e.coin_type_hint, e.coin_type_in_hint, e.coin_type_out_hint,
    case
...
select
  lower(to_hex(md5(to_utf8(cast(
    coalesce(cast(transfer_unique_key as varchar), '_dbt_utils_surrogate_key_null_') || '-' ||
    coalesce(cast(cast(event_index as varchar) as varchar), '_dbt_utils_surrogate_key_null_') || '-' ||
    coalesce(cast(event_type as varchar), '_dbt_utils_surrogate_key_null_') || '-' ||
    coalesce(cast(coalesce(match_reason, 'unknown') as varchar), '_dbt_utils_surrogate_key_null_')
  as varchar))))) as unique_key,
  'sui' as blockchain,
  block_month, block_date, block_time, block_number,
  tx_hash, tx_digest, transfer_unique_key, tx_from, tx_index,
  coin_type, transfer_type, "from", to, amount_raw,
