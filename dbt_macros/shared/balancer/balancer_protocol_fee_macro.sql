...
--a 
    ...
    bpt_prices_1 AS (
        ...
        -- FILTER 1: Stop pulling BPT prices from the broken period
    ...
    bpt_prices AS (
        SELECT  
            ...
            decimals,
            price,
            -- FILTER 2: Hard-stop the range to prevent joins into the "bad" data period
            LEAD(DAY, 1, DATE '2025-11-03') OVER (PARTITION BY token ORDER BY DAY) AS day_of_next_change
        FROM bpt_prices_1
    ),
    ...
    daily_protocol_fee_collected AS (
        ...
        -- FILTER 3: Exclude bb-pool events after the data break
        WHERE (
    ...
    decorated_protocol_fee AS (
        SELECT 
            ...
            -- Prioritize BPT math (p3) and ensure it doesn't default to 0 to catch NULL joins
            SUM(COALESCE(p3.price, p1.price, p2.price) * protocol_fee_amount_raw / POWER(10, COALESCE(t.decimals, p1.decimals, p3.decimals, 18))) AS protocol_fee_collected_usd
        FROM daily_protocol_fee_collected d
        LEFT JOIN prices p1 ON p1.token = d.token_address AND p1.day = d.day
        LEFT JOIN prices.day p2 ON p2.contract_address = d.token_address AND p2.timestamp = d.day
