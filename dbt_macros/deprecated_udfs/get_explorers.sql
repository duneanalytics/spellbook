...
• explorers: List all explorers you need as a single comma-separated string. Output order will match input ordering.
  Current full list: Etherscan,GitHub,Arkham,Zerion,Parsec,Nansen,Dora,Zapper,OKLink,Blockscout,DeBank,Tokenview,Ethplorer,Socketscan,Bitquery,Blockchair,Ankr,Unmarshal,Beaconchain,Rated
• hash_type: 'tx' or 'address'
• blockchain: Chain to explore, useful for chain-specific explorers like Etherscan
• concise: true/false
...
WITH explorers AS (
    ...
    FROM (VALUES
    ('Etherscan', '🔎', get_chain_explorer_address(blockchain_, hash_), get_chain_explorer_tx_hash(blockchain_, hash_))
    ...
    , ('Socketscan', '🔌', NULL, 'https://www.socketscan.io/tx/' || hash_)
    ...
    WHERE explorers_ LIKE '%' || explorer || '%'
    )
...
, results AS (
    SELECT CASE WHEN concise_ AND hash_type_ = 'tx' THEN get_href(transaction, emoji)
      ...
      END AS link
    ,regexp_count(substring(explorers_ FROM 1 FOR position(explorer IN explorers_) - 1), ',') AS ordering
    FROM explorers
    )
