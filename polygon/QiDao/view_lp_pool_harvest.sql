BEGIN;
DROP VIEW IF EXISTS dune_user_generated.qidao_view_lp_pool_harvest CASCADE;

CREATE VIEW dune_user_generated.qidao_view_lp_pool_harvest AS (
with dws as (
...
from erc20."ERC20_evt_Transfer" a
     ...
     inner join dune_user_generated.qidao_view_lp_pool_basic_info c
     on b."pool_contract_address" = c."pool_contract_address"
       and b."lp_id" = c."lp_id"
    inner join dune_user_generated.qidao_view_lp_basic_info d
       on c."lp_contract_address" = d."lp_contract_address"
);

COMMIT;
select sum(amount) from dune_user_generated.qidao_view_lp_pool_harvest