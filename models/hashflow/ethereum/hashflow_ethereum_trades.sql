...
hashflow_router_call_tradesinglehop as (
    -- only date from 2022-01-10 to 2022-04-08
    select *
    from {{ source('hashflow_ethereum', 'router_call_tradesinglehop') }}
),
...
new_router as (
    ...
    from hashflow_router_call_tradesinglehop t
...
legacy_router_w_integration as (
    ...
    left join ethereum_transactions tx on tx.hash = t.tx_hash
