...
, CASE WHEN MIN(CASE WHEN s.token_standard IN ('erc721', 'erc1155') THEN s.trace_side END) = 'consideration' THEN SUM(COALESCE(s.amount, UINT256 '0')) FILTER (WHERE s.token_standard IN ('native', 'erc20') AND s.trace_side='offer')
    ELSE SUM(COALESCE(s.amount, UINT256 '0')) FILTER (WHERE s.token_standard IN ('native', 'erc20') AND s.trace_side='consideration') END AS amount_raw
...
, COALESCE(SUM(COALESCE(s.amount, UINT256 '0')) FILTER (WHERE s.recipient IN (0x8de9c5a032463c561423387a9648c5c7bcc5bc90, 0x34ba0f2379bf9b81d09f7259892e26a8b0885095, 0x0000a26b00c1f0df003000390027140000faa719)), UINT256 '0') AS platform_fee_amount_raw
...
, try((ARRAY_AGG(CASE WHEN s.token_standard IN ('native', 'erc20') THEN s.recipient ELSE NULL END ORDER BY CASE WHEN s.token_standard IN ('native', 'erc20') AND s.recipient NOT IN ( 0x8de9c5a032463c561423387a9648c5c7bcc5bc90, 0x34ba0f2379bf9b81d09f7259892e26a8b0885095, 0x0000a26b00c1f0df003000390027140000faa719) THEN s.amount ELSE NULL END DESC))[2]) AS royalty_fee_address
, CASE WHEN MIN(CASE WHEN s.token_standard IN ('erc721', 'erc1155') THEN s.trace_side END) = 'offer' THEN MAX(s.offerer) FILTER (WHERE s.token_standard IN ('erc721', 'erc1155')) ELSE MAX(s.recipient) FILTER (WHERE s.token_standard IN ('erc721', 'erc1155')) END AS buyer
, CASE WHEN MIN(CASE WHEN s.token_standard IN ('erc721', 'erc1155') THEN s.trace_side END) = 'offer' THEN MAX(s.recipient) FILTER (WHERE s.token_standard IN ('erc721', 'erc1155')) ELSE MAX(s.offerer) FILTER (WHERE s.token_standard IN ('erc721', 'erc1155')) END AS seller
