...
, ref_tokens_nft as (
    select * from {{ ref('tokens_nft') }} where blockchain = 'ethereum'
)

, ref_tokens_erc20 as (
    select * from {{ ref('tokens_erc20') }} where blockchain = 'ethereum'
)

, ref_nft_aggregators as (
    select * from {{ ref('nft_aggregators') }} where blockchain = 'ethereum'
)

, ref_nft_aggregators_marks as (
    select * from {{ ref('nft_ethereum_aggregators_markers') }}
)
...
, iv_base_pairs as (
    select 
        ...
        , sum(
            case 
                when is_platform_fee then null
                ...
                partition by tx_hash, coalesce(om_evt_index, evt_index) 
                order by evt_index, eth_erc_idx
        ) as creator_fee_idx
...
, iv_nfts as (
    select 
        ...
        , b.fee_wallet_name
    from iv_base_pairs a
    left join iv_volume b 
        on b.block_date = a.block_date  -- tx_hash and evt_index is PK, but for performance, block_time is included
        and b.tx_hash = a.tx_hash
        and b.evt_index = a.evt_index
    where a.is_traded_nft
)
...
, iv_trades as (
    select
        ...
        , a.seller
        , a.buyer
        , initcap(a.trade_type) as trade_type
        , initcap(a.order_type) as order_type
        , 'Trade' as evt_type
        ...
        , a.nft_token_amount as number_of_items
        , a.nft_token_standard as token_standard

        -- price info
        , a.price_amount_raw / power(10, e.decimals) as amount_original
        , a.price_amount_raw as amount_raw
        , a.price_amount_raw / power(10, e.decimals) * p.price as amount_usd
        ...
        , a.token_contract_address as original_currency_contract
        , e.decimals as currency_decimals -- in case calculating royalty1~4

        -- project info (platform or exchange)
        , a.platform_contract_address as project_contract_address
        , a.platform_fee_receiver as platform_fee_receive_address
        ...
        , a.platform_fee_amount_raw / power(10, e.decimals) * p.price as platform_fee_amount_usd
         ...
         -- royalty info
        , a.creator_fee_receiver_1 as royalty_fee_receive_address       
        , a.creator_fee_amount_raw as royalty_fee_amount_raw
        , a.creator_fee_amount_raw / power(10, e.decimals) as royalty_fee_amount
        , a.creator_fee_receiver_1 as royalty_fee_receive_address_1
        , a.creator_fee_receiver_2 as royalty_fee_receive_address_2
        , a.creator_fee_receiver_3 as royalty_fee_receive_address_3
        , a.creator_fee_receiver_4 as royalty_fee_receive_address_4
        , a.creator_fee_receiver_5 as royalty_fee_receive_address_5
        , a.creator_fee_amount_raw_1 as royalty_fee_amount_raw_1
        , a.creator_fee_amount_raw_2 as royalty_fee_amount_raw_2
        , a.creator_fee_amount_raw_3 as royalty_fee_amount_raw_3
        , a.creator_fee_amount_raw_4 as royalty_fee_amount_raw_4
        , a.creator_fee_amount_raw_5 as royalty_fee_amount_raw_5
        ...
        , a.sub_type
        , a.fee_wallet_name
        ,'seaport-' || cast(a.tx_hash as varchar(100)) || '-' || cast(a.evt_index as varchar(10)) || '-' || cast(a.nft_contract_address as varchar(100)) || '-' || cast(a.nft_token_id as varchar(100)) || '-' || cast(a.sub_type as varchar(20)) || '-' || cast(a.sub_idx as varchar(10)) as unique_trade_id
    from iv_nfts a
    inner join source_ethereum_transactions t 
        on t.hash = a.tx_hash
    left join ref_tokens_nft n 
        on n.contract_address = nft_contract_address
    left join ref_tokens_erc20 e 
        on e.contract_address = case 
                                    when a.token_contract_address = '{{c_native_token_address}}' then '{{c_alternative_token_address}}'
                                    else a.token_contract_address
                                end
    left join source_prices_usd p 
        on p.contract_address = case 
                                    when a.token_contract_address = '{{c_native_token_address}}' then '{{c_alternative_token_address}}'
                                    else a.token_contract_address
                                end
        and p.minute = date_trunc('minute', a.block_time)
    left join ref_nft_aggregators agg 
        on agg.contract_address = t.to
    left join ref_nft_aggregators_marks agg_m 
        on right(t.data, agg_m.hash_marker_size) = agg_m.hash_marker
    where 
        t.from != '0x110b2b128a9ed1be5ef3232d8e4e41640df5c2cd' -- this is a special address which transact English Auction, will handle later. test comment to force CI
        and (   CAST(a.zone AS VARCHAR(100)) in ('0xf397619df7bfd4d1657ea9bdd9df7ff888731a11'
                ...
                or  a.fee_wallet_name = 'opensea'
            )
)

select * from iv_trades
