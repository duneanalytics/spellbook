...
, MIN_BY((CASE 
            WHEN (bytearray_substring(et.data, 1, 4)) = 0x AND et.gas_used = 21000 AND CAST(et.value as double) > 0 THEN 'avax_transfer' 
            ELSE COALESCE(sig.function, CAST((bytearray_substring(et.data, 1, 4)) as VARCHAR))  
    END), et.block_number) as first_function
, MIN_BY(et.value/1e18, et.block_number) as first_avax_transferred
FROM 
{{ source('avalanche_c', 'transactions') }} et
LEFT JOIN (
    SELECT 
        DISTINCT id, 
