...
, MIN_BY((CASE 
            WHEN (bytearray_substring(et.data, 1, 4)) = 0x AND et.gas_used = 21000 AND CAST(et.value as double) > 0 THEN 'matic_transfer' 
            ELSE COALESCE(sig.function, CAST((bytearray_substring(et.data, 1, 4)) as VARCHAR))  
    END), et.block_number) as first_function
, MIN_BY(et.value/1e18, et.block_number) as first_matic_transferred
FROM 
{{ source('polygon', 'transactions') }} et
LEFT JOIN (
    SELECT 
        DISTINCT id, 
