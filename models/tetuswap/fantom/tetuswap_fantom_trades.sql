...
{% set project_start_date = '2021-12-27' %} -- min(evt_block_time) from tetu_io_fantom.Pair_evt_Swap
...
with dexs as (
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('tetu_io_fantom', 'Pair_evt_Swap') }} t
        inner join {{ source('tetu_io_fantom', 'TetuSwapFactory_evt_PairCreated') }} f 
