...
WITH
  bebop_raw_data AS (
    ...
    FROM
      {{ source('bebop_arbitrum', 'BebopAggregationContract_evt_AggregateOrderExecuted') }}
      LEFT JOIN {{ source('bebop_arbitrum', 'BebopAggregationContract_call_SettleAggregateOrder') }} ex ON ex.call_tx_hash = evt_tx_hash
    WHERE
      call_success = true
