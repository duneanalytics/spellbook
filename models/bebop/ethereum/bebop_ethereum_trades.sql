...
bebop_raw_data AS (
    ...
    FROM
        (SELECT * FROM {{ source('bebop_v3_ethereum', 'BebopAggregationContract_evt_AggregateOrderExecuted') }}
         UNION ALL
         SELECT * FROM {{ source('bebop_v4_ethereum', 'BebopSettlement_evt_AggregateOrderExecuted') }}) evt
    LEFT JOIN
        (SELECT
        call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order"
        FROM {{ source('bebop_v3_ethereum', 'BebopAggregationContract_call_SettleAggregateOrder') }}
        UNION ALL
        SELECT
        call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order"
        FROM {{ source('bebop_v4_ethereum', 'BebopSettlement_call_SettleAggregateOrder') }}
        UNION ALL
        SELECT
        call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order"
        FROM {{ source('bebop_v4_ethereum', 'BebopSettlement_call_SettleAggregateOrderWithTakerPermits') }}
        ) ex
...
SELECT
  ...
  CASE 
    WHEN t.trade_type = 'Multi-Buy' THEN (t.token_bought_amount / power(10, coalesce(t_bought.decimals, 0))) * p_bought.price
    WHEN t.trade_type = 'Multi-Sell' THEN (t.token_sold_amount / power(10, coalesce(t_sold.decimals, 0))) * p_sold.price
    ELSE COALESCE(
