...
bebop_raw_data AS (
    ...
    FROM
        (SELECT * FROM {{ source('bebop_v3_ethereum', 'BebopAggregationContract_evt_AggregateOrderExecuted') }}
         UNION ALL
         SELECT * FROM {{ source('bebop_v4_ethereum', 'BebopAggregationContract_evt_AggregateOrderExecuted') }}) evt
    LEFT JOIN
        (SELECT
        ...
        SELECT
        call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order"
        FROM {{ source('bebop_v4_ethereum', 'BebopAggregationContract_call_SettleAggregateOrder') }}
        UNION ALL
        SELECT
        call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order"
