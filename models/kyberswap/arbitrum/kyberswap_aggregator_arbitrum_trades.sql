...
SELECT
    ...
    ,coalesce(
        dexs.amount_usd
    -- TODO: need to check if 0xEeeeeEeeeEeEeeEeEeEeeEEEeeeeEeeeeeeeEEeE
...
--    ,coalesce(dexs.taker, tx.from) AS taker -- subqueries rely on this COALESCE to avoid redundant joins with the transactions table
    ,dexs.taker AS taker -- subqueries rely on this COALESCE to avoid redundant joins with the transactions table
    ,dexs.maker
...
--    ,tx.from AS tx_from
    ...
    ,dexs.evt_index
FROM dexs
    INNER JOIN {{ source('arbitrum', 'transactions')}} tx ON dexs.tx_hash = tx.hash
