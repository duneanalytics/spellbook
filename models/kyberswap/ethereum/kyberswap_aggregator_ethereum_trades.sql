...
(
        ...
        FROM
            {{ source('kyber_ethereum', 'AggregationRouter_evt_Swapped') }}
        {% if is_incremental() %}
        ...
        SELECT
            ...
            ,ARRAY[-1]              AS trace_address
        FROM
            {{ source('kyber_ethereum', 'AggregationRouterV2_evt_Swapped') }}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - INTERVAL '7' DAY)
        {% endif %}
        ...
        SELECT
            ...
            ,ARRAY[-1]              AS trace_address
        FROM
            {{ source('kyber_ethereum', 'AggregationRouterV3_evt_Swapped') }}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - INTERVAL '7' DAY)
        {% endif %}
        ...
        SELECT
            ...
            ,ARRAY[-1]              AS trace_address
        FROM
            {{ source('kyber_ethereum', 'MetaAggregationRouter_evt_Swapped') }}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - INTERVAL '7' DAY)
        {% endif %}
