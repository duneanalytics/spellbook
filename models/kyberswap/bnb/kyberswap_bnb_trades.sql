...
kyberswap_dex AS (
    SELECT
        ...
        ,''                                                                 AS trace_address
        ,t.evt_index
    FROM {{ source('kyber_bnb', 'DMMPool_evt_Swap') }} t
    INNER JOIN {{ source('kyber_bnb', 'DMMFactory_evt_PoolCreated') }} p
    ...
    SELECT
        ...
        ,''                                                                            AS trace_address
        ,t.evt_index
    FROM {{ source('kyber_bnb', 'Elastic_Pool_evt_swap') }} t
    INNER JOIN {{ source('kyber_bnb', 'Elastic_Factory_evt_PoolCreated') }} p
        ON t.contract_address = p.pool
    {% if is_incremental() %}
    WHERE t.evt_block_time >= date_trunc("day", now() - interval '1 week')
    {% else %}
    ...
    SELECT
        ...
        ,''                                                                AS trace_address
        ,evt_index
    FROM {{ source('kyber_bnb', 'AggregationRouter_evt_Swapped') }}
    WHERE
        {% if is_incremental() %}
    ...
    SELECT
        ...
        ,''                                                                AS trace_address
        ,evt_index
    FROM {{ source('kyber_bnb', 'MetaAggregationRouter_evt_Swapped') }}
    WHERE
        {% if is_incremental() %}
        evt_block_time >= date_trunc("day", now() - interval '1 week')
        {% else %}
