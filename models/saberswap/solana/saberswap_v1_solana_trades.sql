...
{% set project_start_date = '2021-05-26' %} --grabbed program deployed at time (account created at).
  ...
  WITH
    pools as (
        ...
        SELECT 
            ...
            , ip.account_tokenAReserve as tokenAVault
            , tkB.symbol as tokenB_symbol
            , tkB.decimals as tokenB_decimals
            , ip.account_tokenBMint as tokenB
            , ip.account_tokenBReserve as tokenBVault
            , cast(null as varchar) as fee_tier
            , ip.account_swap as pool_id
            , ip.call_tx_id as init_tx
            , ip.call_block_time as init_time
        FROM {{ source('saberswap_solana', 'stable_swap_call_initialize') }} ip 
    ...
    , all_swaps as (
        ...
        FROM {{ source('saberswap_solana', 'stable_swap_call_swap') }} sp
