...
, qa.rewards_token, r.to AS quester_address
, r.evt_tx_hash AS tx_hash, r.evt_index, r.evt_block_time AS block_time, r.evt_block_number AS block_number
...
AND 1 = (
        CASE WHEN evt_block_time >= date_trunc('day', now() - interval '1 week') THEN 1
             WHEN distributor_address NOT IN (SELECT distributor_address FROM {{this}} GROUP BY 1) --we don't have this loaded in yet.
             ELSE 0
        END
        )
{% endif %}
