{{ config(
    ...
    alias = alias('liquidation', legacy_model=True),
...
LEFT JOIN (SELECT contract_address,underlying_token_address
            FROM {{ ref('ironbank_optimism_itokens_legacy') }} ) i_collateral ON CAST(l.cTokenCollateral AS VARCHAR(100)) = i_collateral.contract_address
LEFT JOIN (SELECT contract_address,underlying_token_address
            FROM {{ ref('ironbank_optimism_itokens_legacy') }} ) i_asset ON CAST(l.contract_address AS VARCHAR(100)) = i_asset.contract_address
LEFT JOIN {{ ref('ironbank_optimism_itokens_legacy') }} i ON CAST(l.contract_address AS VARCHAR(100)) = i.contract_address
