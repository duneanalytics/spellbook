...
SELECT evt_block_time,
       ...
       isLong                                                  AS isLong,
       'Open'                                                  AS trade_type
FROM {{ source('voodoo_trade_base', 'Vault_evt_DecreasePosition') }}
...
SELECT 
    ...
    isLong AS isLong,
    'Close' AS trade_type
FROM {{ source('voodoo_trade_base','Vault_evt_IncreasePosition') }}
    {% if not is_incremental() %}
  WHERE evt_block_time >= DATE '{{project_start_date}}'
    {% else %}
  WHERE {{ incremental_predicate('evt_block_time') }}
...
SELECT 'base'                                    AS blockchain
    ...
    END
     ...
     , 'voodoo_trade'                               AS project
     , 'v1'                                          AS version
     , 'voodoo_trade'                               AS frontend
     , account                                       AS trader
