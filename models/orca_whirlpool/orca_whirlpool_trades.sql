...
with 
    ...
    , all_swaps as (
        SELECT 
            ...
            , tr_2.amount/pow(10,case when tk_1.token_mint_address = p.tokenA then wp.tokenB_decimals else tokenA_decimals end) as token_bought_amount
            ...
            , tr_1.amount/pow(10,case when tk_1.token_mint_address = p.tokenA then wp.tokenA_decimals else tokenB_decimals end) as token_sold_amount
            , wp.fee_tier
            , wp.whirlpool_id
            ...
            , case when tk_1.token_mint_address = p.tokenA then wp.tokenB
                else wp.tokenA
                end as token_bought_mint_address
            , case when tk_1.token_mint_address = p.tokenA then wp.tokenA 
                else wp.tokenB
                end as token_sold_mint_address
            , case when tk_1.token_mint_address = p.tokenA then wp.tokenBVault
                else wp.tokenAVault
