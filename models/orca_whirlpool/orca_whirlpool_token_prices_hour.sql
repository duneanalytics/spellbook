...
with raw_data as (
    SELECT *
    FROM { { ref('orca_whirlpool_trades') } }
    WHERE 1 = 1 { % if is_incremental() % }
        AND { { incremental_predicate('block_time') } } { %
        else % }
        AND block_time >= DATE('2022-03-10') { % endif % }
),
bought_price as (
...
SELECT t1.token_mint as contract_address,
    ...
    DATE_TRUNC('month', t1.hour) as block_month
FROM all_trades t1
    JOIN { { ref('tokens_solana_fungible') } } t2 ON t1.token_mint = t2.token_mint_address
GROUP BY 1,
    2,
    3,
    4