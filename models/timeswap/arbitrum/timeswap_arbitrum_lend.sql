...
SELECT
  ...
  JOIN {{ source('prices', 'usd') }} p ON p.symbol=i.token0_symbol and p.blockchain = 'arbitrum' and l.isToken0 = true
...
SELECT
  ...
  FROM {{ source('timeswap_arbitrum', 'TimeswapV2PeripheryUniswapV3LendGivenPrincipal_evt_LendGivenPrincipal') }} l
  JOIN {{ ref('timeswap_arbitrum_pools') }} i ON CAST(l.maturity as VARCHAR(100)) = i.maturity and cast(l.strike as VARCHAR(100)) = i.strike 
  JOIN {{ source('prices', 'usd') }} p ON p.symbol=i.token1_symbol and p.blockchain = 'arbitrum' and l.isToken0 = false
  AND p.minute = date_trunc('minute',l.evt_block_time)
  {% if is_incremental() %}
  AND p.minute >= date_trunc("day", now() - interval '1 week')
  {% endif %}
