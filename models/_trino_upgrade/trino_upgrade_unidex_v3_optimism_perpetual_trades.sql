{{ config(
    tags = ['prod_exclude'],
	schema = 'trino_upgrade_unidex_v3_optimism',
...
{% set project_start_date = '2023-09-18' %}
...
WITH positions AS (
	SELECT
		...
		,evt_block_number
		,'3' AS version
	FROM {{ source('unidex_optimism', 'tradingv3_evt_NewOrder') }}
	{% if is_incremental() %}
	...
	SELECT
		...
		,evt_block_number
		,'3' AS version
	FROM {{ source('unidex_optimism', 'tradingv3_evt_ClosePosition') }}
	{% if is_incremental() %}
	WHERE {{ incremental_predicate('evt_block_time') }}
	{% endif %}
),
