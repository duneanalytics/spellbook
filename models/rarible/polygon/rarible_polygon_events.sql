...
fees_data as (
    SELECT e.evt_block_number,
        ...
        row_number() over (partition by e.evt_tx_hash order by e.evt_index) as row_num
    ...
    INNER JOIN trades t ON e.evt_block_number = t.evt_block_number
        ...
        {% endif %}
    WHERE e.`to` = '0x424ace4669579986d200ebeb3c75924282324a42'
        AND t.currency_contract <> '0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270'
    ...
    SELECT e.block_number AS evt_block_number,
        ...
        CAST(e.value as double) AS platform_fee_amount_raw,
        row_number() over (partition by e.tx_hash order by e.trace_address) as row_num
