...
WITH 
    v3_fills AS (
       ...
       FROM {{ source('zeroex_v3_polygon', 'Exchange_evt_Fill') }} fills 
        ...
        LEFT OUTER JOIN {{ ref('tokens_erc20_legacy') }} mt ON mt.contract_address = SUBSTRING(fills.makerAssetData,17,20) and mt.blockchain = 'polygon'
        LEFT OUTER JOIN {{ ref('tokens_erc20_legacy') }} tt ON tt.contract_address = SUBSTRING(fills.takerAssetData,17,20) and tt.blockchain = 'polygon'
         where 1=1  
                {% if is_incremental() %}
    ...
    , v4_limit_fills AS (
        ...
        LEFT JOIN {{ source('prices', 'usd') }} mp ON
            ...
            AND CASE
                ...
                END = mp.contract_address
        LEFT OUTER JOIN {{ ref('tokens_erc20_legacy') }} mt ON mt.contract_address = fills.makerToken and mt.blockchain = 'polygon'
        LEFT OUTER JOIN {{ ref('tokens_erc20_legacy') }} tt ON tt.contract_address = fills.takerToken and tt.blockchain = 'polygon'
         where 1=1  
                {% if is_incremental() %}
                AND evt_block_time >= date_trunc('day', now() - interval '1 week')
                {% endif %}
