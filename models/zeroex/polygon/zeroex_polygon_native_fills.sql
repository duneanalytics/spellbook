...
WITH 
    v3_fills AS (
       ...
       FROM {{ source('zeroex_v3_polygon', 'Exchange_evt_Fill') }} fills 
        ...
        LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} mt ON mt.contract_address = bytearray_substring(fills.makerAssetData, 8, 10) and mt.blockchain = 'polygon'
        LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} tt ON tt.contract_address = bytearray_substring(fills.takerAssetData, 8, 10) and tt.blockchain = 'polygon'
         where 1=1  
                {% if is_incremental() %}
    ...
    , v4_limit_fills AS (
        ...
        LEFT JOIN {{ source('prices', 'usd') }} mp ON
            ...
            AND CASE
                ...
                END = mp.contract_address
        LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} mt ON mt.contract_address = fills.makerToken and mt.blockchain = 'polygon'
        LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} tt ON tt.contract_address = fills.takerToken and tt.blockchain = 'polygon'
         where 1=1  
                {% if is_incremental() %}
                AND evt_block_time >= date_trunc('day', now() - interval '7' day)
                {% endif %}
