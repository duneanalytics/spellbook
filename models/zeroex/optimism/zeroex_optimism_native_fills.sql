...
WITH 
    ...
    v4_limit_fills AS (
        ...
        LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} mt ON mt.contract_address = fills.makerToken and mt.blockchain = 'optimism'
        LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} tt ON tt.contract_address = fills.takerToken and tt.blockchain = 'optimism'
         where 1=1 
                {% if is_incremental() %}
