...
LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} ts ON ts.contract_address = taker_token and ts.blockchain = 'fantom'
LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} ms ON ms.contract_address = maker_token and ms.blockchain = 'fantom'