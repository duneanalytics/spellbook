...
v4_rfq_fills_no_bridge AS (
    ...
    {% if is_incremental() %}
    WHERE fills.evt_block_time >= date_add('week', -1, CURRENT_TIMESTAMP(6))
    {% endif %}
    {% if not is_incremental() %}
...
NewBridgeFill AS (
    ...
    WHERE topic1 = '0xe59e71a14fe90157eedc866c4f8c767d3943d6b6b2e8cd64dddcc92ab4c55af8'
        ...
        {% if is_incremental() %}
        AND logs.block_time >= date_add('week', -1, CURRENT_TIMESTAMP(6))
        {% endif %}
        {% if not is_incremental() %}
...
direct_PLP AS (
    ...
    {% if is_incremental() %}
    WHERE plp.evt_block_time >= date_add('week', -1, CURRENT_TIMESTAMP(6))
    {% endif %}
    {% if not is_incremental() %}
...
LEFT JOIN {{ source('prices', 'usd') }} tp 
    ...
    {% if is_incremental() %}
    AND tp.minute >= date_add('week', -1, CURRENT_TIMESTAMP(6))
    {% endif %}
    {% if not is_incremental() %}
...
LEFT JOIN {{ source('prices', 'usd') }} mp 
    ...
    {% if is_incremental() %}
    AND mp.minute >= date_add('week', -1, CURRENT_TIMESTAMP(6))
    {% endif %}
    {% if not is_incremental() %}
