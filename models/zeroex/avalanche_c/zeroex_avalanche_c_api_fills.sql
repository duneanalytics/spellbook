...
LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} ts ON ts.contract_address = taker_token and ts.blockchain = 'avalanche_c'
LEFT OUTER JOIN {{ source('tokens', 'erc20', True) }} ms ON ms.contract_address = maker_token and ms.blockchain = 'avalanche_c'