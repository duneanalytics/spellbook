...
WITH 
    v3_fills AS (
        ...
        LEFT OUTER JOIN {{ source('tokens_ethereum', 'erc20', True) }} mt ON mt.contract_address = greatest(bytearray_substring(makerAssetData, 17, 20),bytearray_substring(makerAssetData, 245, 20))
        LEFT OUTER JOIN {{ source('tokens_ethereum', 'erc20', True) }} tt ON tt.contract_address = greatest(bytearray_substring(takerAssetData, 17, 20),bytearray_substring(takerAssetData, 245, 20))
         where 1=1  
                {% if is_incremental() %}
    ...
    , v4_limit_fills AS (
        ...
        LEFT JOIN {{ source('prices', 'usd') }} mp ON 
            ...
            AND CASE
                ...
                END = mp.contract_address
        LEFT OUTER JOIN {{ source('tokens_ethereum', 'erc20', True) }} mt ON mt.contract_address = fills.makerToken
        LEFT OUTER JOIN {{ source('tokens_ethereum', 'erc20', True) }} tt ON tt.contract_address = fills.takerToken
         where 1=1  
                {% if is_incremental() %}
                AND evt_block_time >= date_trunc('day', now() - interval '7' day)
                {% endif %}
