    ...
    v3_fills AS (
        ...
        LEFT JOIN {{ source('prices', 'usd') }} tp ON
            ...
            AND CASE
                    ...
                    WHEN bytearray_substring(fills.takerAssetData, 17, 20) IN (0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee) THEN 0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2
                    ELSE bytearray_substring(fills.takerAssetData, 17, 20)
                END = tp.contract_address
        LEFT JOIN {{ source('prices', 'usd') }} mp ON
            DATE_TRUNC('minute', evt_block_time) = mp.minute  and mp.blockchain = 'ethereum'
            AND CASE
                    -- Set Deversifi ETHWrapper to WETH
                    WHEN bytearray_substring(fills.makerAssetData, 17, 20) IN (0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee) THEN 0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2
                    ELSE bytearray_substring(fills.makerAssetData, 17, 20)
                END = mp.contract_address
        LEFT OUTER JOIN {{ ref('tokens_ethereum_erc20') }} mt ON mt.contract_address = bytearray_substring(fills.makerAssetData, 17, 20)
        LEFT OUTER JOIN {{ ref('tokens_ethereum_erc20') }} tt ON tt.contract_address = bytearray_substring(fills.takerAssetData, 17, 20)
         where 1=1  
                {% if is_incremental() %}
