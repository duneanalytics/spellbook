...
WITH 
    v3_fills AS (
        ...
        LEFT OUTER JOIN {{ ref('tokens_ethereum_erc20') }} mt ON mt.contract_address = SUBSTRING(fills.makerAssetData,17,20) and mt.blockchain = 'ethereum'
        LEFT OUTER JOIN {{ ref('tokens_ethereum_erc20') }} tt ON tt.contract_address = SUBSTRING(fills.takerAssetData,17,20) and tt.blockchain = 'ethereum'
         where 1=1  
                {% if is_incremental() %}
      ...
      LEFT JOIN prices.usd mp ON
          ...
          AND CASE
                  -- Set Deversifi ETHWrapper to WETH
                    ...
                    ELSE fills.makerToken
              END = mp.contract_address
      LEFT OUTER JOIN {{ ref('tokens_ethereum_erc20') }} mt ON mt.contract_address = fills.makerToken and mt.blockchain = 'ethereum'
      LEFT OUTER JOIN {{ ref('tokens_ethereum_erc20') }} tt ON tt.contract_address = fills.takerToken and tt.blockchain = 'ethereum'
       where 1=1  
                {% if is_incremental() %}
                AND evt_block_time >= date_trunc('day', now() - interval '1 week')
                {% endif %}
