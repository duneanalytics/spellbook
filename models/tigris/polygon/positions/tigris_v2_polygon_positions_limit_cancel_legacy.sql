...
last_margin as (
        ...
        SELECT 
            ...
            position_id
        FROM 
        {{ ref('tigris_v2_polygon_positions_margin_legacy') }}
        GROUP BY 2 
        ) xx 
        INNER JOIN 
        {{ ref('tigris_v2_polygon_positions_margin_legacy') }} xy 
...
last_leverage as (
        ...
        SELECT 
            ...
            position_id
        FROM 
        {{ ref('tigris_v2_polygon_positions_leverage_legacy') }}
        GROUP BY 2 
        ) xx 
        INNER JOIN 
        {{ ref('tigris_v2_polygon_positions_leverage_legacy') }} xy 
            ON xx.evt_block_time = xy.evt_block_time
            AND xx.position_id = xy.position_id
)
...
SELECT 
    ...
    ll.leverage 
FROM 
{{ ref('tigris_v2_polygon_events_limit_cancel_legacy') }} lp 
INNER JOIN 
last_margin lm 
