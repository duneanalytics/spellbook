...
open_position as (
    SELECT 
        ...
        0 as fees 
    FROM 
    {{ ref('tigris_polygon_events_options_open_position') }} op 
    WHERE order_type = '0'
    {% if is_incremental() %}
...
limit_order as (
    SELECT 
        ...
        0 as fees 
    FROM 
    {{ ref('tigris_polygon_events_options_limit_order') }} op 
    {% if is_incremental() %}
...
close_position as (
    SELECT 
        ...
        f.fees as fees 
    FROM 
    {{ ref('tigris_polygon_events_options_close_position') }} op 
    INNER JOIN 
    {{ ref('tigris_polygon_events_options_fees_distributed') }} f
...
limit_cancel as (
    SELECT 
        ...
        0 as fees 
    FROM 
    {{ ref('tigris_polygon_events_options_limit_cancel') }} op 
    {% if is_incremental() %}
    WHERE op.evt_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}
)
