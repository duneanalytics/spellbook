{{ config(
    alias = 'polygon_events_close_position',
    partition_by = ['day'],
...
close_position_v1 as (
        SELECT 
            ...
            _trader as trader 
        FROM 
        {{ source('tigristrade_polygon', 'TradingV1_evt_PositionClosed') }}
        {% if is_incremental() %}
...
close_position_v2 as (
        SELECT 
            ...
            _trader as trader 
        FROM 
        {{ source('tigristrade_polygon', 'TradingV2_evt_PositionClosed') }}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc("day", now() - interval '1 week')
        {% endif %}
),
...
close_position_v3 as (
        SELECT 
            ...
            _trader as trader 
        FROM 
        {{ source('tigristrade_polygon', 'TradingV3_evt_PositionClosed') }}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc("day", now() - interval '1 week')
        {% endif %}
),
