...
-- trade based formulation, for Linear Pools (former BPT prices spell)
    ...
    bpt_trades AS (
        SELECT * 
        FROM {{ source('beethoven_x_fantom','Vault_evt_Swap') }} v
    ...
    all_trades_info AS (
        ...
        FROM bpt_trades a
        LEFT JOIN {{ source ('prices', 'usd') }} p1 ON p1.contract_address = a.tokenIn AND p1.blockchain = 'fantom'
        AND  p1.minute = date_trunc('minute', a.evt_block_time)
        LEFT JOIN {{ source ('prices', 'usd') }} p2 ON p2.contract_address = a.tokenOut AND p2.blockchain = 'fantom'
        AND  p2.minute = date_trunc('minute', a.evt_block_time)
        LEFT JOIN {{ source('tokens', 'erc20') }} t1 ON t1.contract_address = a.tokenIn AND t1.blockchain = 'fantom'
        LEFT JOIN {{ source('tokens', 'erc20') }} t2 ON t2.contract_address = a.tokenOut AND t2.blockchain = 'fantom'
    ...
    SELECT 
        ...
        END AS bpt_price
    FROM tvl l
    LEFT JOIN {{ ref('beethoven_x_fantom_bpt_supply') }} s ON l.pool_address = s.token_address
