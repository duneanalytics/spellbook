...
WITH registered AS (
    SELECT
        poolID AS pool_id,
        evt_block_time
    FROM {{ source('beethoven_x_fantom', 'Vault_evt_PoolRegistered') }}
    {% if is_incremental() %}
...
weighted_pool_factory AS (
    ...
    FROM {{ source('beethoven_x_fantom', 'WeightedPoolFactory_call_create') }} AS call_create
...
weighted_pool_2tokens_factory AS (
    SELECT
        ...
        t.token_address AS token_address,
        t2.normalized_weight AS normalized_weight
    FROM {{ source('beethoven_x_fantom', 'WeightedPool2TokensFactory_call_create') }} AS call_create
    CROSS JOIN UNNEST(call_create.tokens) WITH ORDINALITY t(token_address, pos)
    CROSS JOIN UNNEST(call_create.weights) WITH ORDINALITY t2(normalized_weight, pos)
    WHERE t.pos = t2.pos
    {% if is_incremental() %}
