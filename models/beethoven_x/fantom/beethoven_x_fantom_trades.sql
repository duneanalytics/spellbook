...
{% set project_start_date = '2021-09-19' %} -- min(evt_block_time) from beethoven_x_fantom.Vault_evt_Swap

WITH dexs as (
    -- beethoven
    SELECT 
        evt_block_time as block_time, 
        '' as taker, -- recipient not in swap event 
        '' as maker, 
        ...
        SUBSTRING(poolId, 1, 42) as project_contract_address, 
        evt_tx_hash as tx_hash, 
        '' as trace_address,
        evt_index
    FROM 
    {{ source('beethoven_x_fantom', 'Vault_evt_Swap') }}
...
SELECT
    ...
    tx.from as taker,  -- recipient not in swap event
    dexs.maker, 
