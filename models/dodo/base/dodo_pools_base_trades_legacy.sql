...
(
        ...
        SELECT
            ...
            s.evt_index
        FROM
            {{ source('dodo_base', 'DODO_evt_SellBaseToken')}} s
        ...
        SELECT
            ...
            b.evt_index
        FROM
            {{ source('dodo_base','DODO_evt_BuyBaseToken')}} b
        LEFT JOIN dodo_view_markets m
        ...
        SELECT
            ...
            evt_index
        FROM
            {{ source('dodo_base', 'DVM_evt_DODOSwap')}}
        {% if is_incremental() %}
        ...
        SELECT
            ...
            evt_index
        FROM
            {{ source('dodo_base', 'DPPAdvanced_evt_DODOSwap')}}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - interval '7' day)
        {% endif %}
        ...
        SELECT
            ...
            evt_index
        FROM
            {{ source('dodo_base', 'DPPOracle_evt_DODOSwap')}}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - interval '7' day)
        {% endif %}
        ...
        SELECT
            ...
            evt_index
        FROM
            {{ source('dodo_base', 'DSP_evt_DODOSwap')}}
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - interval '7' day)
        {% endif %}
)
