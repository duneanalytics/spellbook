...
FROM (
    {% for chain in op_chains %}
        SELECT
            ...
            SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ source('dex', 'trades') }} r WHERE t.hash = r.tx_hash AND t.block_time = r.block_time AND r.block_month = DATE_TRUNC('month',r.block_time) AND blockchain = '{{chain}}') THEN 1 ELSE 0 END) AS num_dex_trade_txs,
            SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ source('perpetual','trades') }} r WHERE t.hash = r.tx_hash AND t.block_time = r.block_time AND r.block_month = DATE_TRUNC('month',r.block_time) AND blockchain = '{{chain}}') THEN 1 ELSE 0 END) AS num_perp_trade_txs,
