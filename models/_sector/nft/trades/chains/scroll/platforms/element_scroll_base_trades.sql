...
WITH element_txs AS (
        ...
        , ee.evt_index
        FROM {{ source('element_ex_scroll','OrdersFeature_evt_ERC721SellOrderFilled') }} ee
        {% if is_incremental() %}
        ...
        , ee.evt_block_number AS block_number
        , ee.evt_index
        FROM {{ source('element_ex_scroll','OrdersFeature_evt_ERC721BuyOrderFilled') }} ee
        {% if is_incremental() %}
        WHERE {{incremental_predicate('ee.evt_block_time')}}
        {% endif %}
        ...
        , ee.maker AS seller
        , ee.taker AS buyer
        , ee.erc20FillAmount AS price_raw
        , CASE WHEN ee.erc20Token=0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 0x0d500B1d8E8eF31E21C99d1Db9A6444d3ADf1270
            ELSE ee.erc20Token END AS currency_contract
        ...
        , ee.evt_block_number AS block_number
        , ee.evt_index
        FROM {{ source('element_ex_scroll','OrdersFeature_evt_ERC1155SellOrderFilled') }} ee
        {% if is_incremental() %}
        WHERE {{incremental_predicate('ee.evt_block_time')}}
        {% endif %}
        ...
        , ee.evt_block_number AS block_number
        , ee.evt_index
        FROM {{ source('element_ex_scroll','OrdersFeature_evt_ERC1155BuyOrderFilled') }} ee
        {% if is_incremental() %}
        WHERE {{incremental_predicate('ee.evt_block_time')}}
        {% endif %}
        )
