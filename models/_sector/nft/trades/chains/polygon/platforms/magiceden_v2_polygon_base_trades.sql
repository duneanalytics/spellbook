...
{% set magiceden_start_date = "TIMESTAMP '2024-02-14'" %}
...
WITH trades AS (
    ...
    FROM {{ source('limitbreak_polygon','PaymentProcessor_evt_BuyListingERC721') }}
    {% if is_incremental() %}
    ...
    , salePrice AS price_raw
    , evt_index AS sub_tx_trade_id
    FROM {{ source('limitbreak_polygon','PaymentProcessor_evt_BuyListingERC1155') }}
    {% if is_incremental() %}
    WHERE {{incremental_predicate('evt_block_time')}}
    {% else %}
    WHERE evt_block_time >= {{magiceden_start_date}}
    ...
    , salePrice AS price_raw
    , evt_index AS sub_tx_trade_id
    FROM {{ source('limitbreak_polygon','PaymentProcessor_evt_AcceptOfferERC721') }}
    {% if is_incremental() %}
    WHERE {{incremental_predicate('evt_block_time')}}
    {% else %}
    WHERE evt_block_time >= {{magiceden_start_date}}
    ...
    , salePrice AS price_raw
    , evt_index AS sub_tx_trade_id
    FROM {{ source('limitbreak_polygon','PaymentProcessor_evt_AcceptOfferERC1155') }}
    {% if is_incremental() %}
    WHERE {{incremental_predicate('evt_block_time')}}
    {% else %}
    WHERE evt_block_time >= {{magiceden_start_date}}
...
, whitelisted_trades AS (
    ...
    , fc.message
    FROM trades t
    INNER JOIN {{ source('limitbreak_polygon','TrustedForwarder_call_forwardCall') }} fc ON fc.call_block_number=t.block_number
