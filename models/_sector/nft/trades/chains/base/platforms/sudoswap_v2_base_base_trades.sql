...
WITH
    pools as (
        SELECT 
        *
        FROM {{ ref('sudoswap_v2_base_pool_creations')}}
    )
    ...
    , sell_nft_base as (
        ...
        LEFT JOIN (
            ...
            SELECT * FROM {{ source('sudoswap_v2_base','ExponentialCurve_call_getSellInfo') }}
            UNION ALL 
            SELECT * FROM {{ source('sudoswap_v2_base','LinearCurve_call_getSellInfo') }}
            UNION ALL 
            SELECT * FROM {{ source('sudoswap_v2_base','XykCurve_call_getSellInfo') }}
            UNION ALL 
            SELECT * FROM {{ source('sudoswap_v2_base','GDACurve_call_getSellInfo') }}
        ...
        LEFT JOIN {{ source('sudoswap_v2_base','RoyaltyEngine_call_getRoyalty') }} roy 
    ...
    , buy_nft_base as (
        ...
        LEFT JOIN (
            SELECT * FROM {{ source('sudoswap_v2_base','ExponentialCurve_call_getBuyInfo') }}
            UNION ALL 
            SELECT * FROM {{ source('sudoswap_v2_base','LinearCurve_call_getBuyInfo') }}
            UNION ALL 
            SELECT * FROM {{ source('sudoswap_v2_base','XykCurve_call_getBuyInfo') }}
            UNION ALL 
            SELECT * FROM {{ source('sudoswap_v2_base','GDACurve_call_getBuyInfo') }}
        ) sp ON sp_start.call_tx_hash = sp.call_tx_hash
            AND sp_start.call_block_number = sp.call_block_number
            AND sp_start.call_trace_address_filled = slice(sp.call_trace_address,1,cardinality(sp_start.call_trace_address_filled))
            {% if is_incremental() %}
