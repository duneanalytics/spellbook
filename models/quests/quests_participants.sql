...
SELECT 'optimism' as blockchain, 'optimism quests' as platform, quester_address, block_number, block_time, contract_project as quest_name, nft_contract_address as token_address, tokenId as token_id FROM {{ ref('optimism_quests_optimism_quest_completions') }}
    GROUP BY 1,2,3,4,5,6,7,8 -- handle for multiple transfers in one tx
UNION ALL
SELECT distinct 'optimism' as blockchain, 'galxe' as platform, token_transfer_to as quester_address, block_number, block_time, null as quest_name, nft_contract_address as token_address, tokenId AS token_id FROM {{ ref('galxe_optimism_nft_mints') }}
    GROUP BY 1,2,3,4,5,6,7,8 -- handle for multiple transfers in one tx
