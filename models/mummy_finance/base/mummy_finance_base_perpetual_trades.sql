...
SELECT evt_block_time,
       ...
       isLong                                                  AS isLong,
       'Open'                                                  AS trade_type
FROM {{ source('mummy_finance_base', 'Vault_evt_DecreasePosition') }}
...
SELECT 
    ...
    isLong AS isLong,
    'Close' AS trade_type
FROM {{ source('mummy_finance_base','Vault_evt_IncreasePosition') }}
    {% if not is_incremental() %}
  WHERE evt_block_time >= DATE '{{project_start_date}}'
    {% else %}
  WHERE {{ incremental_predicate('evt_block_time') }}
...
SELECT 'base'                                    AS blockchain
    ...
    END
     ...
     , 'mummy_finance'                               AS project
     , 'v1'                                          AS version
     , 'mummy_finance'                               AS frontend
     , account                                       AS trader
