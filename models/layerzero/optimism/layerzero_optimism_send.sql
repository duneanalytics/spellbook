...
{% set native_token_contract = "0x4200000000000000000000000000000000000006" %}

WITH send_detail AS (
    SELECT CAST(111 AS integer) AS source_chain_id,
    ...
    FROM {{ source ('layerzero_optimism', 'Endpoint_call_send') }} s
    INNER JOIN {{ source('optimism','transactions') }} t on t.block_number = s.call_block_number
...
destination_gas_detail AS (
    ...
    FROM send_detail s
    INNER JOIN {{ source('optimism', 'traces') }} e on e.block_number = s.block_number
...
native_transfer_value_summary AS (
    ...
    INNER JOIN {{ source('optimism', 'traces') }} e ON e.block_number = dg.block_number
...
LEFT JOIN {{ ref('tokens_erc20') }} erc ON erc.blockchain = 'optimism' AND erc.contract_address = t.currency_contract
LEFT JOIN {{ source('prices', 'usd') }} p ON p.blockchain = 'optimism' AND p.contract_address = t.currency_contract
