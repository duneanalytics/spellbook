...
{% set transaction_start_date = "2023-03-15" %}
{% set endpoint_contract = "0x66a71dcef29a0ffbdbe3c6a460a3b5bc225cd675" %}
{% set native_token_contract = "0x27e86d206889198c9B58044b3866fF25AA479be2" %}
...
WITH send_detail AS (
    ...
    FROM {{ source ('layezero_metis', 'Endpoint_call_send') }} s
    INNER JOIN {{ source('metis','transactions') }} t on t.block_number = s.call_block_number
...
destination_gas_detail AS (
    ...
    FROM send_detail s
    INNER JOIN {{ source('metis', 'traces') }} e on e.block_number = s.block_number
...
trans_detail AS (
    ...
    FROM (
        ...
        INNER JOIN {{ source('erc20_metis', 'evt_transfer') }} et on et.evt_block_number = s.block_number
...
LEFT JOIN {{ ref('tokens_erc20') }} erc ON erc.blockchain = 'metis' AND erc.contract_address = t.currency_contract
LEFT JOIN {{ source('prices', 'usd') }} p ON p.blockchain = 'metis' AND p.contract_address = t.currency_contract
