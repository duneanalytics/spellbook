...
WITH send_detail AS (
    SELECT ROW_NUMBER() OVER(PARTITION BY s.call_block_number,s.call_tx_hash ORDER BY s.call_trace_address ASC) AS call_send_index,
        CAST(102 AS integer) AS source_chain_id,
    ...
    FROM {{ source ('layerzero_bnb', 'Endpoint_call_send') }} s
    INNER JOIN {{ source('bnb','transactions') }} t on t.block_number = s.call_block_number
...
destination_gas_detail AS (
    ...
    FROM send_detail s
    INNER JOIN {{ source('bnb', 'traces') }} e on e.block_number = s.block_number
...
trans_detail AS (
    ...
    FROM (
        ...
        INNER JOIN {{ source('erc20_bnb', 'evt_transfer') }} et on et.evt_block_number = s.block_number
...
SELECT 'bnb' AS blockchain,
    ...
    t.transfer_type,
    CASE WHEN erc.symbol = 'WBNB' AND t.transfer_type = 'native'
        THEN 'BNB'
...
LEFT JOIN {{ source('tokens', 'erc20', True) }} erc ON erc.blockchain = 'bnb' AND erc.contract_address = t.currency_contract
