...
destination_gas_detail AS (
    ...
    FROM send_detail s
    INNER JOIN {{ source('base', 'traces') }} e on e.block_number = s.block_number
...
trans_detail AS (
    ...
    FROM (
        ...
        INNER JOIN {{ source('erc20_base', 'evt_transfer') }} et on et.evt_block_number = s.block_number
...
LEFT JOIN {{ source('tokens', 'erc20', True) }} erc ON erc.blockchain = 'base' AND erc.contract_address = t.currency_contract
