...
{% set native_token_contract = "0xe91d153e0b41518a2ce8dd3d7944fa863463a97d" %}

WITH send_detail AS (
    SELECT CAST(145 AS integer) AS source_chain_id,
...
destination_gas_detail AS (
    ...
    FROM send_detail s
    INNER JOIN {{ source('gnosis', 'traces') }} e on e.block_number = s.block_number
...
native_transfer_value_summary AS (
    ...
    INNER JOIN {{ source('gnosis', 'traces') }} e ON e.block_number = dg.block_number
...
SELECT 'gnosis' AS blockchain,
    ...
    t.transfer_type,
    CASE WHEN erc.symbol = 'WXDAI' AND t.transfer_type = 'native'
        THEN 'XDAI'
...
LEFT JOIN {{ ref('tokens_erc20') }} erc ON erc.blockchain = 'gnosis' AND erc.contract_address = t.currency_contract
