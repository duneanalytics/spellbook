{{ config(alias = 'snapshot_single_choice_proposals'
...
SELECT
  ...
  TRY_CAST(dv.choice AS INTEGER) AS voting_choice,
  ELEMENT_AT(SPLIT(TRIM(TRIM(dp.choices, '[]'), ' '), ', '),TRY_CAST(dv.choice AS INTEGER)) AS voting_choice_name,
...
WHERE
  dp."space" = 'arbitrumfoundation.eth' AND dp."type" = 'single-choice'