...
WITH sushiswap_dex AS (
    ...
    FROM {{ source('sushi_bnb', 'swapETHForExactTokens') }} t
    FROM {{ source('sushi_bnb', 'swapExactETHForTokens') }} t
    FROM {{ source('sushi_bnb', 'swapExactETHForTokensSupportingFeeOnTransferTokens') }} t
