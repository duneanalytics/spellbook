...
{% set project_start_date = '2021-06-16' %} -- min(evt_block_time) from sushi_celo.Pair_evt_Swap
...
with dexs as (
    ...
    select
        ...
        t.evt_index
    from {{ source('sushi_celo', 'Pair_evt_Swap') }} t
        inner join {{ source('sushi_celo', 'SushiV2Factory_evt_PairCreated') }} f on t.contract_address = f.pair
    {% if is_incremental() %}
