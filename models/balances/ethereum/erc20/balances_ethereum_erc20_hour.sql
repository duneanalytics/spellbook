...
years as (
    select year
    from (values (sequence(timestamp '2015-05-29', cast(date_trunc('year', now()) as timestamp), interval '1' year))) s(year_array)
...
hourly_balances as (
    ...
    FROM 
    {{ ref('transfers_ethereum_erc20_rolling_hour') }}
)
...
LEFT JOIN 
{{ ref('balances_ethereum_erc20_noncompliant') }} nc
