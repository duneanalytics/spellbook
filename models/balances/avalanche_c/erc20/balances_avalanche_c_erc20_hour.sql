...
years as (
    select year
    from (values (sequence(timestamp '2020-09-23', cast(date_trunc('year', now()) as timestamp), interval '1' year))) s(year_array)
...
hourly_balances as (
    ...
    FROM 
    {{ ref('transfers_avalanche_c_erc20_rolling_hour') }}
)
...
LEFT JOIN 
{{ ref('balances_avalanche_c_erc20_noncompliant') }} nc
