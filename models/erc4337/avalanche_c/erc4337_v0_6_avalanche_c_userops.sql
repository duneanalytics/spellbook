{{ config
(
    schema = 'erc4337_v0_5_avalanche_c',
    alias ='userops',
...
with userop as (
    ...
    from {{ source('erc4337_avalanche_c','EntryPoint_v0_6_evt_UserOperationEvent') }}
    {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc("day", now() - interval '1 week')
    {% endif %}
)
, handleops as (
    select call_tx_hash as tx_hash, beneficiary from {{source('erc4337_avalanche_c', 'EntryPoint_v0_6_call_handleOps')}}
    {% if is_incremental() %}
...
, txs as (
    select 
        ...
        , cast(gas_price as bigint)*gas_used as tx_fee
