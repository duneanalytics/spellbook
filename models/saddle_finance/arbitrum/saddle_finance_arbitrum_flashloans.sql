...
WITH flashloans AS (
    ...
    FROM {{ source('saddle_finance_arbitrum','SwapFlashLoan_evt_FlashLoan') }} flash
    INNER JOIN {{ source('saddle_finance_arbitrum','PoolRegistry_evt_AddPool') }} pool ON pool.poolAddress=flash.contract_address
    {% if is_incremental() %}
...
LEFT JOIN {{ ref('tokens_arbitrum_erc20') }} tok ON tok.contract_address=flash.currency_contract
