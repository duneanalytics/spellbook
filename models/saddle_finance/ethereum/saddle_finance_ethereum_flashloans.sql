...
WITH flashloans AS (
    ...
    FROM {{ source('saddle_finance_ethereum','SwapFlashLoan_evt_FlashLoan') }} flash
    INNER JOIN {{ source('saddle_finance_ethereum','PoolRegistry_evt_AddPool') }} pool ON pool.poolAddress=flash.contract_address
    {% if is_incremental() %}
...
LEFT JOIN {{ ref('tokens_ethereum_erc20') }} tok ON tok.contract_address=flash.currency_contract
