...
WITH pool_labels AS (
    SELECT
        address AS pool_id,
        name AS pool_symbol
    FROM {{ ref('labels_balancer_v2_pools_gnosis') }}
    ),
    ...
    bpt_prices AS(
        ...
        FROM {{ ref('balancer_v2_gnosis_bpt_prices') }}
        GROUP BY 1, 2
    ),
   ...
   cumulative_usd_balance AS (
    ...
    pool_liquidity_estimates AS (
        ...
        LEFT JOIN {{ ref('balancer_v2_gnosis_pools_tokens_weights') }} w ON b.pool_id = w.pool_id
