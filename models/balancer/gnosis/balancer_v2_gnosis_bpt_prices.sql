...
WITH
    ...
    all_trades_info AS (
        ...
        LEFT JOIN {{ source('tokens', 'erc20', True) }} t1 ON t1.contract_address = a.tokenIn AND t1.blockchain = 'gnosis'
        LEFT JOIN {{ source('tokens', 'erc20', True) }} t2 ON t2.contract_address = a.tokenOut AND t2.blockchain = 'gnosis'
