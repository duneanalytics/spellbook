...
WITH
    ...
    csp_pool_ids AS (
        ...
        FROM {{ source('balancer_v2_gnosis', 'ComposableStablePoolFactory_evt_PoolCreated') }} x --#########################
        INNER JOIN (
            SELECT poolAddress, poolId 
            FROM {{ source('balancer_v2_gnosis', 'Vault_evt_PoolRegistered') }} --#########################
        ) y
        ON x.contract_address in (
            0x4bdcc2fb18aeb9e2d281b0278d946445070eada7   -- CSP v5 factory
            , 0xd87f44df0159dc78029ab9ca7d7e57e7249f5acd -- CSP v4 factory
        )
    ...
    , sub_pools AS (
        ...
        FROM {{ source('balancer_v2_gnosis', 'Vault_evt_TokensRegistered') }} x --#########################
    ...
    , join_exit_call_data AS ( 
        ...
        FROM ( --Start subquery
            SELECT 
                ...
                , y.lp_token_burned
                , y.lp_token_minted
            FROM {{ source('balancer_v2_gnosis', 'Vault_evt_PoolBalanceChanged') }} x --#########################
    ...
    , swap_data AS (
            ...
            FROM (
                ...
                SELECT 
                    ...
                    , tokenOut
                    , tokens
                FROM {{ source('balancer_v2_gnosis', 'Vault_evt_Swap') }} x --#########################
    ...
    , pool_balance_managed AS (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM {{ source('balancer_v2_gnosis', 'Vault_evt_PoolBalanceManaged') }} z --#########################
                INNER JOIN (SELECT poolId, tokens FROM sub_pools) sp
    ...
    -- Get token info
        SELECT 
            ...
            , symbol AS s
            , decimals AS d 
        FROM {{ source('tokens_gnosis', 'erc20') }} --#########################
    )
