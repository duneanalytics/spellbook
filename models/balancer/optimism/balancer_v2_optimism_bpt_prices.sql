...
WITH
    ...
    all_trades_info AS (
        ...
        LEFT JOIN {{ source ('prices', 'usd') }} p2 ON p2.contract_address = a.tokenOut AND p2.blockchain = 'optimism'
            ...
            {% endif %} 
        LEFT JOIN {{ ref ('tokens_erc20_legacy') }} t1 ON t1.contract_address = a.tokenIn AND t1.blockchain = 'optimism'
        LEFT JOIN {{ ref ('tokens_erc20_legacy') }} t2 ON t2.contract_address = a.tokenOut AND t2.blockchain = 'optimism'
