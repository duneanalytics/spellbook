...
WITH
    ...
    csp_pool_ids AS (
        ...
        FROM {{ source('balancer_v2_optimism', 'ComposableStablePoolFactory_evt_PoolCreated') }} x --#########################
        INNER JOIN (
            SELECT poolAddress, poolId 
            FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolRegistered') }} --#########################
        ) y
        ON x.contract_address in (
            0x043a2dad730d585c44fb79d2614f295d2d625412   -- CSP v5 factory
            , 0x1802953277fd955f9a254b80aa0582f193cf1d77 -- CSP v4 factory
        )
    ...
    , sub_pools AS (
        ...
        FROM {{ source('balancer_v2_optimism', 'Vault_evt_TokensRegistered') }} x --#########################
    ...
    , join_exit_call_data AS ( 
        ...
        FROM ( --Start subquery
            SELECT 
                ...
                , y.lp_token_burned
                , y.lp_token_minted
            FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolBalanceChanged') }} x --#########################
    ...
    , swap_data AS (
            ...
            FROM (
                ...
                SELECT 
                    ...
                    , tokenOut
                    , tokens
                FROM {{ source('balancer_v2_optimism', 'Vault_evt_Swap') }} x --#########################
    ...
    , pool_balance_managed AS (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM {{ source('balancer_v2_optimism', 'Vault_evt_PoolBalanceManaged') }} z --#########################
                INNER JOIN (SELECT poolId, tokens FROM sub_pools) sp
    ...
    -- Get token info
        SELECT 
            ...
            , symbol AS s
            , decimals AS d 
        FROM {{ source('tokens_optimism', 'erc20') }} --#########################
    )
