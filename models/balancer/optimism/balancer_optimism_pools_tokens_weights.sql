...
WITH events AS (
    ...
    SELECT
        ...
        bind.token,
        bind.denorm
    FROM {{ source('balancer_v1_optimism', 'BPool_call_bind') }} bind
    INNER JOIN {{ source('optimism', 'transactions') }} tx ON tx.hash = bind.call_tx_hash 
    ...
    SELECT
        ...
        rebind.token,
        rebind.denorm
    FROM {{ source('balancer_v1_optimism', 'BPool_call_rebind') }} rebind
    INNER JOIN {{ source('optimism', 'transactions') }} tx ON tx.hash = rebind.call_tx_hash 
    ...
    SELECT
        ...
        unbind.token,
        '0' AS denorm
    FROM {{ source('balancer_v1_optimism', 'BPool_call_unbind') }} unbind
    INNER JOIN {{ source('optimism', 'transactions') }} tx ON tx.hash = unbind.call_tx_hash 
