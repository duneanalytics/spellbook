...
WITH
    ...
    csp_pool_ids AS (
        ...
        FROM {{ source('balancer_v2_avalanche_c', 'ComposableStablePoolFactory_evt_PoolCreated') }} x --#########################
        INNER JOIN (
            SELECT poolAddress, poolId 
            FROM {{ source('balancer_v2_avalanche_c', 'Vault_evt_PoolRegistered') }} --#########################
        ) y
        ON x.contract_address in (
            0xe42ffa682a26ef8f25891db4882932711d42e467   -- CSP v5 factory
            , 0x3b1eb8eb7b43882b385ab30533d9a2bef9052a98 -- CSP v4 factory
        )
    ...
    , sub_pools AS (
        ...
        FROM {{ source('balancer_v2_avalanche_c', 'Vault_evt_TokensRegistered') }} x --#########################
    ...
    , join_exit_call_data AS ( 
        ...
        FROM ( --Start subquery
            SELECT 
                ...
                , y.lp_token_burned
                , y.lp_token_minted
            FROM {{ source('balancer_v2_avalanche_c', 'Vault_evt_PoolBalanceChanged') }} x --#########################
    ...
    , swap_data AS (
            ...
            FROM (
                ...
                SELECT 
                    ...
                    , tokenOut
                    , tokens
                FROM {{ source('balancer_v2_avalanche_c', 'Vault_evt_Swap') }} x --#########################
    ...
    , pool_balance_managed AS (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM {{ source('balancer_v2_avalanche_c', 'Vault_evt_PoolBalanceManaged') }} z --#########################
                INNER JOIN (SELECT poolId, tokens FROM sub_pools) sp
    ...
    -- Get token info
        SELECT 
            ...
            , symbol AS s
            , decimals AS d 
        FROM {{ source('tokens_avalanche_c', 'erc20') }} --#########################
    )
