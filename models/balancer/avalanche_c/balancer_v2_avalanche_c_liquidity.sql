...
WITH pool_labels AS (
    SELECT
        address AS pool_id,
        name AS pool_symbol
    FROM {{ ref('labels_balancer_v2_pools_avalanche') }}
    ),
        ...
        FROM
            (
                SELECT
                    ...
                    amountIn AS delta
                FROM
                    {{ source('balancer_v2_avalanche', 'Vault_evt_Swap') }}
                UNION
                ALL
                SELECT
                    date_trunc('day', evt_block_time) AS day,
...
zipped_balance_changes AS (
        ...
        FROM {{ source('balancer_v2_avalanche', 'Vault_evt_PoolBalanceChanged') }}
    ),
    ...
    managed_changes AS (
        ...
        FROM {{ source('balancer_v2_avalanche', 'Vault_evt_PoolBalanceManaged') }}
    ),
   ...
   cumulative_usd_balance AS (
    ...
    pool_liquidity_estimates AS (
        ...
        LEFT JOIN {{ ref('balancer_v2_avalanche_pools_tokens_weights') }} w ON b.pool_id = w.pool_id
