...
prices AS (
   ...
   cumulative_usd_balance AS (
        ...
        LEFT JOIN {{ source('tokens_ethereum', 'erc20', True) }} t ON t.contract_address = b.token
    ...
    pool_liquidity_estimates AS (
        ...
        AND CAST (w.normalized_weight as DOUBLE) > CAST (0 as DOUBLE)
        LEFT JOIN {{ source('tokens_ethereum', 'erc20', True) }} t ON t.contract_address = w.token_address
