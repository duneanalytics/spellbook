...
WITH
    ...
    csp_pool_ids AS (
        ...
        FROM {{ source('balancer_v2_base', 'ComposableStablePoolFactory_evt_PoolCreated') }} x --#########################
        INNER JOIN (
    ...
    , sub_pools AS (
        ...
        FROM {{ source('balancer_v2_base', 'Vault_evt_TokensRegistered') }} x --#########################
    ...
    , join_exit_call_data AS ( 
        ...
        FROM ( --Start subquery
            SELECT 
                ...
                , y.lp_token_burned
                , y.lp_token_minted
            FROM {{ source('balancer_v2_base', 'Vault_evt_PoolBalanceChanged') }} x --#########################
    ...
    , swap_data AS (
            ...
            FROM (
                ...
                SELECT 
                    ...
                    , tokenOut
                    , tokens
                FROM {{ source('balancer_v2_base', 'Vault_evt_Swap') }} x --#########################
    ...
    , pool_balance_managed AS (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM {{ source('balancer_v2_base', 'Vault_evt_PoolBalanceManaged') }} z --#########################
                INNER JOIN (SELECT poolId, tokens FROM sub_pools) sp
