...
WITH pool_labels AS (
    SELECT
        address AS pool_id,
        name AS pool_symbol
    FROM {{ ref('labels_balancer_v2_pools_base') }}
    ),
    ...
    bpt_prices AS(
        ...
        FROM {{ ref('balancer_v2_base_bpt_prices') }}
        GROUP BY 1, 2
    ),
    ...
    swaps_changes AS (
        ...
        FROM
            (
                ...
                FROM
                    {{ source('balancer_v2_base', 'Vault_evt_Swap') }}
                UNION
                ALL
                SELECT
                    date_trunc('day', evt_block_time) AS day,
   ...
   cumulative_usd_balance AS (
        ...
        LEFT JOIN {{ source('tokens', 'erc20', True) }} t ON t.contract_address = b.token
        AND blockchain = 'base'
    ...
    pool_liquidity_estimates AS (
        ...
        LEFT JOIN {{ ref('balancer_v2_base_pools_tokens_weights') }} w ON b.pool_id = w.pool_id
