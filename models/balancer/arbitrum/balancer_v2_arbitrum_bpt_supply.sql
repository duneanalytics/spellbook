...
WITH
    ...
    csp_pool_ids AS (
        ...
        FROM {{ source('balancer_v2_arbitrum', 'ComposableStablePoolFactory_evt_PoolCreated') }} x --#########################
        INNER JOIN (
            SELECT poolAddress, poolId 
            FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} --#########################
        ) y
        ON x.contract_address in (
            0xa8920455934da4d853faac1f94fe7bef72943ef1   -- CSP v5 factory
            , 0x2498a2b0d6462d2260eac50ae1c3e03f4829ba95 -- CSP v4 factory
        )
    ...
    , sub_pools AS (
        ...
        FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_TokensRegistered') }} x --#########################
    ...
    , join_exit_call_data AS ( 
        ...
        FROM ( --Start subquery
            SELECT 
                ...
                , y.lp_token_burned
                , y.lp_token_minted
            FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolBalanceChanged') }} x --#########################
    ...
    , swap_data AS (
            ...
            FROM (
                ...
                SELECT 
                    ...
                    , tokenOut
                    , tokens
                FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_Swap') }} x --#########################
    ...
    , pool_balance_managed AS (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolBalanceManaged') }} z --#########################
                INNER JOIN (SELECT poolId, tokens FROM sub_pools) sp
