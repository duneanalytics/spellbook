...
WITH sender_transfer_rates AS (
    ...
    SELECT  '{{chain}}' as blockchain
            ...
            , SUM(CASE WHEN EXISTS (SELECT 1 FROM {{ ref('dex_trades_migration_beta') }} r WHERE t.hash = r.tx_hash AND t.block_time = r.block_time AND blockchain = '{{chain}}') THEN 1 ELSE 0 END) AS num_dex_trade_txs
