...
WITH sender_transfer_rates AS (
    ...
    SELECT "from" AS sender, DATE_TRUNC('hour',block_time) AS hr
            ...
            , SUM(CASE WHEN EXISTS (SELECT 1 FROM { ref('perpetual_trades_legacy') } r WHERE t.hash = from_hex(r.tx_hash) AND t.block_time = r.block_time AND blockchain = 'optimism') THEN 1 ELSE 0 END) AS num_perp_trade_txs
