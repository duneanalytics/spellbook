{{config(tags=['dunesql'],alias = alias('tornado_cash'),
...
SELECT
    ...
    'Depositor' as name
FROM { ref('tornado_cash_deposits_legacy') } -- {{ ref('tornado_cash_deposits') }}
UNION
SELECT
    lower(blockchain) as blockchain,
    tx_hash,
    ...
    'Recipient' as name
FROM { ref('tornado_cash_withdrawals_legacy') } -- {{ ref('tornado_cash_withdrawals') }}
)
