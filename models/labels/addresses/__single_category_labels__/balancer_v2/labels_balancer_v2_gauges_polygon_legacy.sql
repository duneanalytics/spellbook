{{config(alias = alias('balancer_v2_gauges_polygon', legacy_model=True),
...
FROM
    ...
    LEFT JOIN {{ ref('labels_balancer_v2_pools_polygon_legacy') }} pools ON pools.address = streamer.pool
UNION ALL
SELECT
    'polygon' AS blockchain,
    gauge.gauge AS address,
