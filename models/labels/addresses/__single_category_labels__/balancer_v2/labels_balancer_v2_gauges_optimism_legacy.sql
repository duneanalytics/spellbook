{{config(alias = alias('balancer_v2_gauges_optimism', legacy_model=True),
...
FROM
    ...
    LEFT JOIN {{ ref('labels_balancer_v2_pools_optimism_legacy') }} pools ON pools.address = streamer.pool
UNION ALL
SELECT
    'optimism' AS blockchain,
    gauge.gauge AS address,
