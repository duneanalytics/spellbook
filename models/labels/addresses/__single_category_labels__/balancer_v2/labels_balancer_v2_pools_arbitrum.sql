{{config(
    tags=['prod_exclude', 'dunesql'],
    alias = alias('balancer_v2_pools_arbitrum'),
    materialized = 'incremental',
    file_format = 'delta',
    incremental_strategy = 'merge',
    unique_key = ['address'],
    post_hook='{{ expose_spells(\'["arbitrum"]\',
                                     "sector",
                                    "labels",
                                    \'["balancerlabs", "hosuke"]\') }}'
    )
}}

WITH pools AS (
    SELECT pool_id,
           token_address,
           weight / pow(10, 18) AS normalized_weight,
           symbol,
           pool_type
    FROM (
        SELECT c.poolId AS pool_id,
            --   explode(zip(cc.tokens, cc.weights)) AS zip,
               cc.symbol,
               token_address,
               weight,
               'WP' AS pool_type
        FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
        INNER JOIN {{ source('balancer_v2_arbitrum', 'WeightedPoolFactory_call_create') }} cc
        ON c.evt_tx_hash = cc.call_tx_hash
        -- AND c.poolId = cc.output_0
        AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
        CROSS JOIN unnest(zip(cc.tokens, cc.weights)) AS tbl(token_address, weight)
        {% if is_incremental() %}
        WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
          AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
        {% endif %}
    )

    UNION ALL

    SELECT pool_id,
           token_address,
           weight/pow(10, 18) AS normalized_weight,
           symbol,
           pool_type
    FROM (
        SELECT c.poolId AS pool_id,
               cc.symbol,
               token_address,
               weight,
               'WP' AS pool_type
        FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
        INNER JOIN {{ source('balancer_v2_arbitrum', 'WeightedPoolV2Factory_call_create') }} cc
        ON c.evt_tx_hash = cc.call_tx_hash
        -- AND c.poolId = cc.output_0
        AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
        CROSS JOIN unnest(zip(cc.tokens, cc.normalizedWeights)) AS tbl(token_address, weight)
        {% if is_incremental() %}
        WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
          AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
        {% endif %}
    )

    UNION ALL

    SELECT pool_id,
           token_address,
           weight / pow(10, 18) AS normalized_weight,
           symbol,
           pool_type
    FROM (
        SELECT c.poolId AS pool_id,
            --   explode(zip(cc.tokens, cc.weights)) AS zip,
               cc.symbol,
               token_address,
               weight,
               'IP' AS pool_type
        FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
        INNER JOIN {{ source('balancer_v2_arbitrum', 'InvestmentPoolFactory_call_create') }} cc
        ON c.evt_tx_hash = cc.call_tx_hash
        -- AND c.poolId = cc.output_0
        AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
        CROSS JOIN unnest(zip(cc.tokens, cc.weights)) AS tbl(token_address, weight)
        {% if is_incremental() %}
        WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
          AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
        {% endif %}
    )

    UNION ALL

    SELECT pool_id,
           token_address,
           weight / pow(10, 18) AS normalized_weight,
           symbol,
           pool_type
    FROM (
        SELECT c.poolId AS pool_id,
            --   explode(zip(cc.tokens, cc.weights)) AS zip,
               cc.symbol,
               token_address,
               weight,
               'WP2T' AS pool_type
        FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
        INNER JOIN {{ source('balancer_v2_arbitrum', 'WeightedPool2TokensFactory_call_create') }} cc
        ON c.evt_tx_hash = cc.call_tx_hash
        -- AND c.poolId = cc.output_0
        AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
        CROSS JOIN unnest(zip(cc.tokens, cc.weights)) AS tbl(token_address, weight)
        {% if is_incremental() %}
        WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
          AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
        {% endif %}
    )

    UNION ALL

    SELECT c.poolId AS pool_id,
           token_address,
           CAST(NULL AS DOUBLE) AS normalized_weight,
           cc.symbol,
           'SP' AS pool_type
    FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
    INNER JOIN {{ source('balancer_v2_arbitrum', 'StablePoolFactory_call_create') }} cc
    ON c.evt_tx_hash = cc.call_tx_hash
    -- AND c.poolId = cc.output_0
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN unnest(cc.tokens) AS tbl(token_address)
    {% if is_incremental() %}
    WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
      AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}

    UNION ALL

    SELECT c.poolId AS pool_id,
        token_address,
        CAST(NULL AS DOUBLE) AS normalized_weight,
        cc.symbol,
        'SP' AS pool_type
    FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
    INNER JOIN {{ source('balancer_v2_arbitrum', 'MetaStablePoolFactory_call_create') }} cc
    ON c.evt_tx_hash = cc.call_tx_hash
    -- AND c.poolId = cc.output_0
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN unnest(cc.tokens) AS tbl(token_address)
    {% if is_incremental() %}
    WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
      AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}

    UNION ALL

    SELECT c.poolId AS pool_id,
        token_address,
        0 AS normalized_weight, cc.symbol, 'LBP' AS pool_type
    FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
    INNER JOIN {{ source('balancer_v2_arbitrum', 'LiquidityBootstrappingPoolFactory_call_create') }} cc
    ON c.evt_tx_hash = cc.call_tx_hash
    -- AND c.poolId = cc.output_0
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN unnest(cc.tokens) AS tbl(token_address)
    {% if is_incremental() %}
    WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
      AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}

    UNION ALL

    SELECT c.poolId AS pool_id,
        token_address,
        0 AS normalized_weight, cc.symbol,
        'LBP' AS pool_type
    FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
    INNER JOIN {{ source('balancer_v2_arbitrum', 'NoProtocolFeeLiquidityBootstrappingPoolFactory_call_create') }} cc
    ON c.evt_tx_hash = cc.call_tx_hash
    -- AND c.poolId = cc.output_0
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN unnest(cc.tokens) AS tbl(token_address)
    {% if is_incremental() %}
    WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
      AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}

    UNION ALL

    SELECT c.poolId AS pool_id,
        token_address,
        CAST(NULL AS DOUBLE) AS normalized_weight,
        cc.symbol, 'SP' AS pool_type
    FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
    INNER JOIN {{ source('balancer_v2_arbitrum', 'ComposableStablePoolFactory_call_create') }} cc
    ON c.evt_tx_hash = cc.call_tx_hash
    -- AND c.poolId = cc.output_0
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN unnest(cc.tokens) AS tbl(token_address)
    {% if is_incremental() %}
    WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
      AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}

    UNION ALL

    SELECT c.poolId AS pool_id,
        token_address,
        CAST(NULL AS DOUBLE) AS normalized_weight,
        cc.symbol,
        'LP' AS pool_type
    FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
    INNER JOIN {{ source('balancer_v2_arbitrum', 'AaveLinearPoolFactory_call_create') }} cc
    ON c.evt_tx_hash = cc.call_tx_hash
    -- AND c.poolId = cc.output_0
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN unnest(array[cc.mainToken, cc.wrappedToken]) AS tbl(token_address)
    {% if is_incremental() %}
    WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
      AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}

    UNION ALL

    SELECT c.poolId AS pool_id,
        token_address,
        CAST(NULL AS DOUBLE) AS normalized_weight, cc.symbol,
        'LP' AS pool_type
    FROM {{ source('balancer_v2_arbitrum', 'Vault_evt_PoolRegistered') }} c
    INNER JOIN {{ source('balancer_v2_arbitrum', 'ERC4626LinearPoolFactory_call_create') }} cc
    ON c.evt_tx_hash = cc.call_tx_hash
    -- AND c.poolId = cc.output_0
    AND bytearray_substring(c.poolId, 1, 20) = cc.output_0
    CROSS JOIN unnest(array[cc.mainToken, cc.wrappedToken]) AS tbl(token_address)
    {% if is_incremental() %}
    WHERE c.evt_block_time >= date_trunc('day', now() - interval '7' day)
      AND cc.call_block_time >= date_trunc('day', now() - interval '7' day)
    {% endif %}
),

settings AS (
    SELECT pool_id,
    coalesce(t.symbol,'?') AS token_symbol,
    normalized_weight,
    p.symbol AS pool_symbol,
    p.pool_type
    FROM pools p
    LEFT JOIN { ref('tokens_erc20_legacy') } t ON p.token_address = t.contract_address
)

SELECT
  'arbitrum' AS blockchain,
--   SUBSTRING(pool_id, 0, 42) AS address,
    bytearray_substring(pool_id, 1, 20) as address,
  CASE WHEN contains(array['SP', 'LP', 'LBP'], pool_type)
      THEN lower(pool_symbol)
    ELSE lower(concat(array_join(array_sort(array_agg(token_symbol)), '/'), ' ', array_join(array_agg(cast(norm_weight AS varchar)), '/')))
  END AS name,
  'balancer_v2_pool' AS category,
  'balancerlabs' AS contributor,
  'query' AS source,
  DATE '2022-12-23'  AS created_at,
  now() AS updated_at,
  'balancer_v2_pools_arbitrum' AS model_name,
  'identifier' as label_type
FROM   (
    SELECT s1.pool_id,
          token_symbol,
          pool_symbol,
          cast(100 * normalized_weight AS integer) AS norm_weight,
          pool_type
    FROM settings s1
    ORDER BY pool_id ASC , pool_symbol DESC, token_symbol ASC
) s
GROUP BY pool_id, pool_symbol, pool_type
ORDER BY 1