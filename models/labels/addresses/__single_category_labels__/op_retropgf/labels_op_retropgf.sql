{{config(tags=['dunesql'],alias = alias('op_retropgf'),
...
FROM { ref('op_retropgf_optimism_recipients_legacy') } -- {{ ref('op_retropgf_optimism_recipients') }}
...
FROM { ref('op_retropgf_optimism_voters_legacy') } -- {{ ref('op_retropgf_optimism_voters') }}