{{
	...
	tags=['legacy'],
        ...
        alias = alias('kyt', legacy_model=True),
...
, final_bot_list AS (
    ...
    from (
        ...
        from initial_bot_list t1
            ...
            select address
            from {{ ref('labels_cex_legacy') }}
            union all 
            select address
            from {{ ref('labels_miners_legacy') }}
            union all
            select
...
, Former_traders AS (
    ...
    FROM (
        ...
        FROM {{ ref('dex_trades_legacy') }} t1
            join (select distinct tx_from from {{ ref('dex_trades_legacy') }} where block_time > now() - interval '3' month ) t3
