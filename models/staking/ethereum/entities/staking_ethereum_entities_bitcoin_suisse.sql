{{ config(
    ...
    alias = alias('entities_bitcoin_suisse'),
    tags = ['dunesql'],
...
FROM (
    ...
    UNION ALL
    SELECT depositor_address, MIN(evt_block_time) AS first_tx
    FROM {{ source('eth2_ethereum', 'DepositContract_evt_DepositEvent') }}
    WHERE withdrawal_credentials=0x010000000000000000000000d1026749530a15c20cb7b30368d8c15e200fe1d6
    {% if not is_incremental() %}
    AND evt_block_time >= DATE '2023-05-25'
    {% endif %}
    {% if is_incremental() %}
