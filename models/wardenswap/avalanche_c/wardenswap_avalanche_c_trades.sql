...
{% set project_start_date = '2022-01-03' %}
...
WITH dex AS (
    SELECT  t.evt_block_time AS block_time,
            ...
            '' AS trace_address,
            t.evt_index
    FROM {{ source('wardenswap_avalanche_c', 'WardenPair_evt_Swap') }} t
    INNER JOIN {{ source('wardenswap_avalanche_c', 'WardenFactory_evt_PairCreated') }} p
...
SELECT
    ...
    '1' AS version,
    try_cast(date_trunc('DAY', dex.block_time) AS date) AS block_date,
    dex.block_time,
    ...
    CAST(dex.token_bought_amount_raw AS DECIMAL(38,0)) AS token_bought_amount_raw,
    CAST(dex.token_sold_amount_raw AS DECIMAL(38,0)) AS token_sold_amount_raw,
    coalesce(
            dex.amount_usd
    ...
    coalesce(dex.taker, tx.from) AS taker,
    dex.maker,
