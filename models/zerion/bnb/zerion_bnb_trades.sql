...
INNER JOIN {{ source('bnb','transactions') }} bt ON bt.block_number=swap.evt_block_number
    AND bt.hash=swap.evt_tx_hash
...
with dexs as (
    ...
    SELECT
        ...
        t.evt_index
    FROM
        {{ source('sushi_bnb', 'Pair_evt_Swap') }} t
        inner join {{ source('sushi_bnb', 'Factory_evt_PairCreated') }} f 
