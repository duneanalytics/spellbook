...
{% set native_wrapped_currency_address = '0xbb4cdb9cbd36b01bd1cbaebf2de08d9173bc095c' %}
{% set native_currency_symbol = 'BNB' %}
...
WITH zerion_trades AS (
    ...
    INNER JOIN {{ source('bnb','transactions') }} pt ON pt.block_number=swap.evt_block_number
        ...
        {% endif %}
    LEFT JOIN {{ ref('tokens_bnb_bep20_legacy') }} tok_sold ON tok_sold.contract_address=swap.inputToken
    LEFT JOIN {{ ref('tokens_bnb_bep20_legacy') }} tok_bought ON tok_bought.contract_address=swap.outputToken
    {% if not is_incremental() %}
