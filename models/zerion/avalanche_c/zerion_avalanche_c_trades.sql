...
WITH zerion_trades AS (
    ...
    INNER JOIN {{ source('avalanche_c','transactions') }} pt ON pt.block_number=swap.evt_block_number
        ...
        {% endif %}
    LEFT JOIN {{ ref('tokens_avalanche_c_erc20_legacy') }} tok_sold ON tok_sold.contract_address=swap.inputToken
    LEFT JOIN {{ ref('tokens_avalanche_c_erc20_legacy') }} tok_bought ON tok_bought.contract_address=swap.outputToken
    {% if not is_incremental() %}
