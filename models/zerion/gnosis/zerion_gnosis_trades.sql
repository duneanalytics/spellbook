...
WITH zerion_trades AS (
    ...
    INNER JOIN {{ source('gnosis','transactions') }} pt ON pt.block_number=swap.evt_block_number
        ...
        {% endif %}
    LEFT JOIN {{ ref('tokens_gnosis_erc20') }} tok_sold ON tok_sold.contract_address=swap.inputToken
    LEFT JOIN {{ ref('tokens_gnosis_erc20') }} tok_bought ON tok_bought.contract_address=swap.outputToken
    {% if not is_incremental() %}
