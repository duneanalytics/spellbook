...
SELECT * FROM native_bridges
            ...
            , token_amount_usd AS token_amount_usd
            , token_amount_raw AS token_amount_raw
            , fee_amount AS fee_amount
            , fee_amount_usd AS fee_amount_usd
            , cast(fee_amount_raw as uint256) AS fee_amount_raw
            , token_address
            , fee_address
            , bmod.source_chain_id AS source_chain_id
            , bmod.destination_chain_id AS destination_chain_id
            , cid_source.chain_name AS source_chain_name
            , cid_dest.chain_name AS destination_chain_name
            , is_native_bridge
            , tx_from
...
, native_bridges AS (
    ...
    FROM (
        ...
        SELECT
            ...
            , token_symbol
            , token_amount AS token_amount
            , token_amount_usd AS token_amount_usd -- changed
            , token_amount_raw AS token_amount_raw
            , fee_amount AS fee_amount
            , fee_amount_usd AS fee_amount_usd
            , cast(fee_amount_raw as uint256) AS fee_amount_raw
