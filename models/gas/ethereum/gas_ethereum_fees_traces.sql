...
LEFT JOIN {{ source('prices', 'usd') }} pu ON pu.minute=date_trunc('minute', traces.block_time)
     ...
     {% endif %}
GROUP BY traces.tx_hash, traces.trace, txs.gas_price, traces.block_time, traces.block_number
, txs.from, txs.to, txs.gas_used, txs.gas_price, txs.data, traces.input