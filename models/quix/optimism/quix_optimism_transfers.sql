...
-- ## p1_ : quixotic_optimism.Seaport_call_fulfillBasicOrder
-- ## p2_ : quixotic_optimism.Seaport_call_fulfillAvailableAdvancedOrders
-- ##       quixotic_optimism.Seaport_call_fulfillAvailableOrders
-- ## p3_ : quixotic_optimism.Seaport_call_fulfillOrder
-- ##       quixotic_optimism.Seaport_call_fulfillAdvancedOrder
-- ## p4_ : quixotic_optimism.Seaport_call_matchOrders
...
with p1_call as (
    select 'basic_order' as main_type
      ...
      from {{ source('quixotic_optimism','Seaport_call_fulfillBasicOrder') }}
...
,p1_evt as (
    select c.main_type
          ...
          ,e.evt_index
      from
          (select *, posexplode(offer) as (offer_idx, offer2) from {{ source('quixotic_optimism','Seaport_evt_OrderFulfilled') }}
               {% if is_incremental() %} -- this filter will only be applied on an incremental run
    ...
    select c.main_type
          ...
          ,e.evt_index
      from
        (select *, posexplode(consideration) as (consideration_idx, consideration2) from {{ source('quixotic_optimism','Seaport_evt_OrderFulfilled') }}
            {% if is_incremental() %} -- this filter will only be applied on an incremental run
            where evt_block_time >= (select max(block_time) from {{ this }})
            {% endif %}
            ) e
...
,p1_add_rn as (
  select (max(case when purchase_method = 'Offer Accepted' and sub_type = 'offer' and sub_idx = 0 then token_contract_address
...
,p1_seaport_transfers as (
 ...
 -- smallest block number for source tables above
        ...
        left join {{ ref('nft_aggregators') }} agg
            ON agg.contract_address = tx.to AND agg.blockchain = 'optimism'
        left join {{ ref('tokens_nft') }} n
            on n.contract_address = nft_contract_address and n.blockchain = 'optimism'
        LEFT JOIN {{ source('erc721_optimism','evt_transfer') }} erct2 ON erct2.evt_block_time=a.block_time
 ...
 -- smallest block number for source tables above
            ...
            {% endif %}
        LEFT JOIN {{ source('erc1155_optimism','evt_transfersingle') }} erct3 ON erct3.evt_block_time=a.block_time
...
,p2_call as (
    select 'available_advanced_orders' as main_type
      ...
      from (select *, posexplode(advancedOrders) as (idx, each) from {{ source('quixotic_optimism','Seaport_call_fulfillAvailableAdvancedOrders') }}
      {% if is_incremental() %} -- this filter will only be applied on an incremental run
      where call_block_time >= (select max(block_time) from {{ this }})
      {% endif %}
      ) c
      ...
      select 'available_orders' as main_type
          ...
          ,c.call_block_number as block_number
          ,c.contract_address as exchange_contract_address
      from (select *, posexplode(orders) as (idx, each) from {{ source('quixotic_optimism','Seaport_call_fulfillAvailableOrders') }}
      {% if is_incremental() %} -- this filter will only be applied on an incremental run
      where call_block_time >= (select max(block_time) from {{ this }})
      {% endif %}
      ) c
...
,p2_evt as (
 select c.*
          ...
          ,e.evt_index
      from p2_call c
            inner join {{ source('quixotic_optimism','Seaport_evt_OrderFulfilled') }} e
...
,p2_seaport_transfers as (
    select
         ...
         ,case when concat('0x',substr(a.price_token,3,40)) =
          '0x0000000000000000000000000000000000000000' then '0xdeaddeaddeaddeaddeaddeaddeaddeaddead0000'
 ...
 -- smallest block number for source tables above
        ...
        left join {{ ref('nft_aggregators') }} agg
            ON agg.contract_address = tx.to AND agg.blockchain = 'optimism'
        left join {{ ref('tokens_nft') }} n
            on n.contract_address = concat('0x',substr(a.nft_address,3,40)) and n.blockchain = 'optimism'
        LEFT JOIN {{ source('erc721_optimism','evt_transfer') }} erct2 ON erct2.evt_block_time=a.block_time
 ...
 -- smallest block number for source tables above
        ...
        left join {{ ref('tokens_erc20') }} t1
            on t1.contract_address =
                ...
                then '0xdeaddeaddeaddeaddeaddeaddeaddeaddead0000'
                else concat('0x',substr(a.price_token,3,40)) end
            and t1.blockchain = 'optimism'
          left join {{ source('prices', 'usd') }} p1
            on p1.contract_address =
                case when concat('0x',substr(a.price_token,3,40)) = '0x0000000000000000000000000000000000000000'
...
,p3_call as (
    select 'order' as main_type
      ...
      from {{ source('quixotic_optimism','Seaport_call_fulfillOrder') }}
      {% if is_incremental() %} -- this filter will only be applied on an incremental run
      where call_block_time >= (select max(block_time) from {{ this }})
      {% endif %}
     group by 1,2,3,4
    ...
    select 'advanced_order' as main_type
      ...
      from {{ source('quixotic_optimism','Seaport_call_fulfillAdvancedOrder') }}
      {% if is_incremental() %} -- this filter will only be applied on an incremental run
      where call_block_time >= (select max(block_time) from {{ this }})
      {% endif %}
      group by 1,2,3,4
...
,p3_add_rn as (
    select (max(case when purchase_method = 'Offer Accepted' and sub_type = 'offer' and sub_idx = 0 then token_contract_address::string
...
,p4_call as (
    select 'match_orders' as main_type
          ...
          ,c.call_block_number as block_number
          ,c.contract_address as exchange_contract_address
     from (select *, posexplode(output_executions) as (idx, each) from {{ source('quixotic_optimism','Seaport_call_matchOrders') }}
     {% if is_incremental() %} -- this filter will only be applied on an incremental run
     where call_block_time >= (select max(block_time) from {{ this }})
     {% endif %}
     ) c
    ...
    select 'match_advanced_orders' as main_type
          ...
          ,c.call_block_number as block_number
          ,c.contract_address as exchange_contract_address
    from (select *, posexplode(output_executions) as (idx, each) from {{ source('quixotic_optimism','Seaport_call_matchAdvancedOrders') }}
    {% if is_incremental() %} -- this filter will only be applied on an incremental run
    where call_block_time >= (select max(block_time) from {{ this }})
    {% endif %}
    ) c
...
select * from p1_seaport_transfers
    union all
select * from p2_seaport_transfers
    union all
select * from p3_seaport_transfers
    union all
select * from p4_seaport_transfers
