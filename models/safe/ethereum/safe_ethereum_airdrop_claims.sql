...
WITH more_prices AS (
    ...
    AND contract_address=from_hex('{{safe_token_address}}')
    )
...
, CASE
    WHEN t.evt_block_time BETWEEN (SELECT min_hour FROM more_prices) AND (SELECT max_hour FROM more_prices) THEN CAST(pu.median_price*t.value/POWER(10, 18) AS double)
    ...
    ELSE CAST((SELECT max_price FROM more_prices)*t.value/POWER(10, 18) AS double)
  END AS amount_usd
, from_hex('{{safe_token_address}}') AS token_address
, 'SAFE' AS token_symbol
, t.evt_index
FROM {{ source('erc20_ethereum', 'evt_transfer') }} t
LEFT JOIN {{ ref('dex_prices') }} pu ON pu.blockchain = 'ethereum'
    AND pu.contract_address=from_hex('{{safe_token_address}}')
    AND pu.hour = date_trunc('hour', t.evt_block_time)
    {% if is_incremental() %}
    AND pu.hour >= date_trunc('day', now() - interval '7' day)
    {% endif %}
WHERE t.contract_address = from_hex('{{safe_token_address}}')
