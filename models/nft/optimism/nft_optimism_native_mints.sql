...
select
    ...
    , 'optimism' || '-' || coalesce(ec.namespace, 'Unknown') || '-Mint-' || coalesce(cast(nft_mints.tx_hash as varchar), '-1') || '-' || coalesce(nft_mints.to, '-1') || '-' ||  coalesce(nft_mints.contract_address, '-1') || '-' || coalesce(nft_mints.token_id, '-1') || '-' || coalesce(nft_mints.amount, '-1') || '-'|| coalesce(erc20s.contract_address, '0x0000000000000000000000000000000000000000') || '-' || coalesce(nft_mints.evt_index, '-1') as unique_trade_id
    , coalesce(sum(cast(tr.value as double) ), sum(cast(erc20s.value as double)), 0.0)*(cast(nft_mints.amount as double)/cast(nft_count.nfts_minted_in_tx as double) ) as amount_raw
    ...
    , coalesce(pu_eth.price*sum(cast(tr.value_decimal as double)), pu_erc20s.price*sum(cast(erc20s.value as double))/cast(power(10, pu_erc20s.decimals) as double))*(cast(nft_mints.amount as double)/cast(nft_count.nfts_minted_in_tx as double)) as amount_usd
