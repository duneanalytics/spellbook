...
, 'avalanche_c' || evt_tx_hash || '-erc721-' || contract_address || '-' || tokenId || '-' || from || '-' || to || '-' || '1' || '-' || evt_index AS unique_transfer_id
FROM {{ source('erc721_avalanche_c','evt_transfer') }}
{% if is_incremental() %}
...
, to
, evt_tx_hash AS tx_hash
, 'avalanche_c' || evt_tx_hash || '-erc721-' || contract_address || '-' || id || '-' || from || '-' || to || '-' || value || '-' || evt_index AS unique_transfer_id
FROM {{ source('erc1155_avalanche_c','evt_transfersingle') }}
{% if is_incremental() %}
WHERE evt_block_time >= date_trunc("day", now() - interval '1 week')
{% endif %}
UNION
...
FROM (
    ...
    FROM {{ source('erc1155_avalanche_c', 'evt_transferbatch') }}
    {% if is_incremental() %}
    WHERE evt_block_time >= date_trunc("day", now() - interval '1 week')
    {% endif %}
