...
(select   cast(aggregator_address as string) as aggregator_address,  cast(aggregator_name as string) as aggregator_name,  cast(amount_original as double) as amount_original,  cast(amount_raw as decimal(38,0)) as amount_raw,  cast(amount_usd as double) as amount_usd,  cast(block_number as double) as block_number,  cast(block_time as timestamp) as block_time,  cast(blockchain as string) as blockchain,  cast(buyer as string) as buyer,  cast(collection as string) as collection,  cast(currency_contract as string) as currency_contract,  cast(currency_symbol as string) as currency_symbol,  cast(evt_type as string) as evt_type,  cast(nft_contract_address as string) as nft_contract_address,  cast(number_of_items as decimal(38,0)) as number_of_items,  cast(project as string) as project,  cast(project_contract_address as string) as project_contract_address,  cast(seller as string) as seller,  cast(token_id as string) as token_id,  cast(token_standard as string) as token_standard,  cast(trade_category as string) as trade_category,  cast(trade_type as string) as trade_type,  cast(tx_from as string) as tx_from,  cast(tx_hash as string) as tx_hash,  cast(tx_to as string) as tx_to,  cast(unique_trade_id as string) as unique_trade_id,  cast(version as string) as version
...
(select nft_contract_address, price_p5_eth, row_number() over (partition by nft_contract_address order by block_date desc) rn 
from {{ref('nft_ethereum_collection_stats')}}),

eth_collection_stats_latest_floor as 
(select nft_contract_address, price_p5_eth from eth_collection_stats_floor where rn = 1),
...
buys_and_sells_w_index as 
    ...
    lag(eth_amount) over (partition by wallet, nft_contract_address, token_id order by block_time) prev_trade_eth_amount,
    lag(amount_usd) over (partition by wallet, nft_contract_address, token_id order by block_time) prev_trade_usd_amount
    from buys_and_sells_nft_trades_no_wash_w_mints),
    
lastest_eth_price_usd as 
(select blockchain, minute,  price from {{ref('prices_usd_latest')}} where blockchain = 'ethereum' and symbol = 'WETH'),
...
all_trades_profit_and_unrealized_profit as 
        ...
        coalesce(s.amount_usd, floors1.avg_floor_price*p.price, floors2.price_p5_eth*p.price, 0) + b.amount_usd usd_profit,
        ...
        case when s.wallet is null then coalesce(floors1.avg_floor_price, floors2.price_p5_eth, 0) + b.eth_amount else 0 end eth_profit_unrealized,
        ...
        left join buys_and_sells_w_index s on b.wallet = s.wallet  
                                            and b.nft_contract_address = s.nft_contract_address  
                                            and b.token_id = s.token_id  
                                            and b.nft_tx_index + 1 = s.nft_tx_index 
        left join reservoir_floors_latest_avg floors1 on cast(floors1.contract as string) = cast(b.nft_contract_address as string)
        left join eth_collection_stats_latest_floor floors2 on cast(floors2.nft_contract_address as string) = cast(b.nft_contract_address as string)
        CROSS JOIN lastest_eth_price_usd p 
        where 1=1
        and b.trade_type = 'buy' and coalesce(s.trade_type, 'sell') = 'sell'
...
aggregated_wallet_trading_stats as 
        ...
        (sum(case when nft_was_sold = 1 and eth_profit_realized = 0 then 1 else 0 end)*1.00 / sum(nft_was_sold)) breakeven_percentage,
        ...
        ((sum(sell_amount_eth)*1.00 / sum((case when nft_was_sold = 1 then buy_amount_eth else 0 end) *-1)) - 1) ROI_eth_realized,
        ((sum(floor_eth)*1.00 / sum((case when nft_was_sold = 0 then buy_amount_eth else 0 end) *-1)) - 1) ROI_eth_unrealized,
