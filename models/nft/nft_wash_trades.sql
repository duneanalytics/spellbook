...
LEFT JOIN {{ ref('nft_trades') }} filter_baf
    ...
    AND (filter_baf.block_time BETWEEN nftt.block_time - interval '3 months' AND nftt.block_time + interval '3 months')
    {% if is_incremental() %}
...
LEFT JOIN {{ ref('nft_trades') }} filter_bought_3x
    ...
    AND (filter_bought_3x.block_time BETWEEN nftt.block_time - interval '2 weeks' AND nftt.block_time + interval '2 weeks')
    {% if is_incremental() %}
