...
, filter_4 AS (
    ...
    FROM {{ ref('nft_trades_legacy') }} nftt
    INNER JOIN {{ ref('addresses_events_base_first_funded_by_legacy') }} filter_funding_buyer
        ...
        {% endif %}
    INNER JOIN {{ ref('addresses_events_base_first_funded_by_legacy') }} filter_funding_seller
...
, filter_5 AS (
    ...
    , true AS flashloan
    FROM {{ ref('nft_trades_legacy') }} nftt
    INNER JOIN {{ ref('dex_flashloans_legacy') }} df ON df.blockchain='base'
