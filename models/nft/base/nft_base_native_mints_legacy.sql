...
with namespaces as (
	...
	from {{ ref('contracts_base_contract_mapping_legacy') }}
)
, nfts_per_tx as (
    select
        tx_hash
        ,sum(amount) as nfts_minted_in_tx
        from {{ ref('nft_base_transfers_legacy') }}
        {% if is_incremental() %}
...
from {{ ref('nft_base_transfers_legacy') }} as nft_mints
left join {{ source('base','transactions') }} as etxs
    ...
    {% endif %}
left join {{ ref('tokens_base_nft_legacy') }} as tok
    on tok.contract_address=nft_mints.contract_address
left join {{ ref('tokens_base_nft_bridged_mapping_legacy') }} as bm
    on bm.contract_address=nft_mints.contract_address
left join {{ ref('transfers_base_eth_legacy') }} as tr
...
left join {{ ref('nft_base_aggregators_legacy') }} as agg
