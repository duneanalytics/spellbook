...
, 'polygon' || evt_tx_hash || '-erc721-' || contract_address || '-' || tokenId || '-' || from || '-' || to || '-' || '1' || '-' || evt_index AS unique_transfer_id
FROM {{ source('erc721_polygon','evt_Transfer') }}
{% if is_incremental() %}
...
, to
, evt_tx_hash AS tx_hash
, 'polygon' || evt_tx_hash || '-erc721-' || contract_address || '-' || id || '-' || from || '-' || to || '-' || value || '-' || evt_index AS unique_transfer_id
...
FROM (
    ...
    FROM {{ source('erc1155_polygon', 'evt_TransferBatch') }}
    {% if is_incremental() %}
    WHERE evt_block_time >= date_trunc("day", now() - interval '1 week')
    {% endif %}
