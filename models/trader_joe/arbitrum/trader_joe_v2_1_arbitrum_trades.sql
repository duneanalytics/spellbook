{{ config(
    schema = 'trader_joe_v2_1_arbitrum',
    alias = 'trades',
...
{% set project_start_date = '2023-04-06' %}
...
(
    ...
    FROM (
        ...
        from {{ source('trader_joe_v2_1_arbitrum', 'LBPair_evt_Swap') }} a
        ) t
        INNER JOIN {{ source('trader_joe_v2_1_arbitrum', 'LBFactory_evt_LBPairCreated') }} f
