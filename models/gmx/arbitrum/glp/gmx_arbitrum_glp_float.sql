...
WITH minute AS  -- This CTE generates a series of minute values
    ...
    FROM
        ...
        SELECT minute from unnest(sequence(TIMESTAMP '{{project_start_date}}', now(), INTERVAL '1' minute)) AS _u(minute) -- 2021-08-31 08:13 is the timestamp of the first vault transaction
        {% endif %}
        {% if is_incremental() %}
