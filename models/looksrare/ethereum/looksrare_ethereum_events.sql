...
-- Fetching current date once to make sure it's consistent across CTEs due to it being relied upon
...
WITH looksrare_trades AS (
    ...
    FROM (
        ...
        {% if is_incremental() %}
        AND ta.evt_block_time >= date_trunc("day", '{{current_date}}' - interval '1 week')
        {% endif %}
        ...
        {% if is_incremental() %}
        AND tb.evt_block_time >= date_trunc("day", '{{current_date}}' - interval '1 week')
        {% endif %}
        )
    )
...
, royalties AS (
    ...
    {% if is_incremental() %}
    AND evt_block_time >= date_trunc("day", '{{current_date}}' - interval '1 week')
    {% endif %}
    )
...
LEFT JOIN {{ source('prices','usd') }} pu ON pu.blockchain='ethereum'
    ...
    {% if is_incremental() %}
    AND pu.minute >= date_trunc("day", '{{current_date}}' - interval '1 week')
    {% endif %}
INNER JOIN {{ source('ethereum','transactions') }} et ON lr.block_time=et.block_time
    AND lr.tx_hash=et.hash
    {% if is_incremental() %}
    AND et.block_time >= date_trunc("day", '{{current_date}}' - interval '1 week')
    {% endif %}
...
LEFT JOIN {{ ref('nft_ethereum_transfers') }} buyer_fix ON lr.block_time=buyer_fix.block_time
    ...
    {% if is_incremental() %}
    AND buyer_fix.block_time >= date_trunc("day", '{{current_date}}' - interval '1 week')
    {% endif %}
LEFT JOIN {{ ref('nft_ethereum_transfers') }} seller_fix ON lr.block_time=seller_fix.block_time
    ...
    {% if is_incremental() %}
    AND seller_fix.block_time >= date_trunc("day", '{{current_date}}' - interval '1 week')
    {% endif %}
LEFT JOIN {{ source('erc721_ethereum','evt_transfer') }} standard ON lr.block_time=standard.evt_block_time
    ...
    {% if is_incremental() %}
    AND standard.evt_block_time >= date_trunc("day", '{{current_date}}' - interval '1 week')
    {% endif %}
