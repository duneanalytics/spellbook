...
with
  erc1155_ids_batch AS (
    ...
    FROM {{source('erc1155_fantom', 'evt_transferbatch')}}
  ),
  ...
  sent_transfers as (
    select
      ...
      id as tokenId,
      value as amount
    FROM {{source('erc1155_fantom', 'evt_transfersingle')}} single
    UNION ALL
    select
      evt_tx_hash,
      evt_tx_hash || '-' || evt_index || '-' || to as unique_tx_id,
  ...
  received_transfers as (
    select
      ...
      id as tokenId,
      - value as amount
    FROM {{source('erc1155_fantom', 'evt_transfersingle')}}
    UNION ALL
    select
      evt_tx_hash,
      evt_tx_hash || '-' || evt_index || '-' || to as unique_tx_id,
...
select 'fantom' as blockchain, wallet_address, token_address, evt_block_time, tokenId, amount, evt_tx_hash, unique_tx_id
from sent_transfers
union all
select 'fantom' as blockchain, wallet_address, token_address, evt_block_time, tokenId, amount, evt_tx_hash, unique_tx_id
