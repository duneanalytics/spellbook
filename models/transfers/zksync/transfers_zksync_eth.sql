...
with eth_transfers as (
    select
        ...
        ,t.to AS tx_to
        ,t."from" AS tx_from
    from {{ source('zksync', 'traces') }} as r
    join {{ source('zksync', 'transactions') }} as t
        on r.tx_hash = t.hash
    ...
    select
        ...
        ,t.to AS tx_to
        ,t."from" AS tx_from
    from {{ source('erc20_zksync', 'evt_transfer') }} as r
    join {{ source('zksync', 'transactions') }} as t
