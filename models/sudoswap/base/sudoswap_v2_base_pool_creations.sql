  ...
  pool_creations as (
    ...
    FROM (
          SELECT 
              ...
              call_block_time,
              call_tx_hash
        FROM {{ source('sudoswap_v2_base','LSSVMPairFactory_call_createPairERC721ETH') }}
        WHERE call_success
          {% if is_incremental() %}
        ...
        SELECT
              ...
              call_block_time,
              call_tx_hash
        FROM {{ source('sudoswap_v2_base','LSSVMPairFactory_call_createPairERC1155ETH') }}
        WHERE call_success
          {% if is_incremental() %}
          AND call_block_time >= date_trunc('day', now() - interval '7' day)
          {% endif %}
        ...
        SELECT 
            ...
            , call_block_time
            , call_tx_hash
        FROM {{ source('sudoswap_v2_base','LSSVMPairFactory_call_createPairERC1155ERC20') }}
        WHERE call_success
          {% if is_incremental() %}
          AND call_block_time >= date_trunc('day', now() - interval '7' day)
          {% endif %}
        ...
        SELECT 
            ...
            , call_block_time
            , call_tx_hash
        FROM {{ source('sudoswap_v2_base','LSSVMPairFactory_call_createPairERC721ERC20') }}
        WHERE call_success
          {% if is_incremental() %}
          AND call_block_time >= date_trunc('day', now() - interval '7' day)
          {% endif %}
