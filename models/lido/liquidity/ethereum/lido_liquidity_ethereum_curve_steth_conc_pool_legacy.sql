{{ config(
    ...
    alias = alias('curve_steth_conc_pool', legacy_model=True),
    partition_by = ['time'],
    materialized = 'table',
    file_format = 'delta',
...
where 
    ...
    to = lower('0x828b154032950C8ff7CF8085D841723Db2696056') and
...
where 
    contract_address = lower('0xae7ab96520de3a18e5e111b5eaab095312d7fe84') and 
    from = lower('0x828b154032950C8ff7CF8085D841723Db2696056') and
    DATE_TRUNC('day', evt_block_time) >= to_date('{{ project_start_date }}')
...
from {{source('erc20_ethereum','evt_Transfer')}} t
where 
    contract_address = lower('0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2') and 
    to = lower('0x828b154032950C8ff7CF8085D841723Db2696056') and
    DATE_TRUNC('day', evt_block_time) >= to_date('{{ project_start_date }}')
...
select 'ethereum curve WETH:stETH concentrated 0.04' as pool_name, '0x828b154032950C8ff7CF8085D841723Db2696056' as pool, 'ethereum' as blockchain, 'curve' as project,0.04 as fee,
