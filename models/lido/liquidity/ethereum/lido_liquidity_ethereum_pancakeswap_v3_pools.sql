...
{% set project_start_date = '2023-04-03' %} 

with 

pools as (
select pool as address, 'ethereum' as blockchain, 'pancakeswap' as project, max(cast(fee as double))/10000 as fee
from {{ source('pancakeswap_v3_ethereum','PancakeV3Factory_evt_PoolCreated')}}
...
, swap_events as (
    ...
    from {{ source('pancakeswap_v3_ethereum', 'PancakeV3Pool_evt_Swap') }} sw
    left join {{ source('pancakeswap_v3_ethereum', 'PancakeV3Factory_evt_PoolCreated') }} cr on sw.contract_address = cr.pool
    {% if not is_incremental() %}
...
, mint_events as (
    select 
        ...
        sum(cast(amount0 as DOUBLE)) as amount0,
        sum(cast(amount1 as DOUBLE)) as amount1
    from {{ source('pancakeswap_v3_ethereum', 'PancakeV3Pool_evt_Mint') }} mt
    left join {{ source('pancakeswap_v3_ethereum', 'PancakeV3Factory_evt_PoolCreated') }} cr on mt.contract_address = cr.pool
    {% if not is_incremental() %}
...
, burn_events as (
    ...
    from {{ source('pancakeswap_v3_ethereum', 'PancakeV3Pool_evt_Burn') }} bn
    left join {{ source('pancakeswap_v3_ethereum', 'PancakeV3Factory_evt_PoolCreated') }} cr on bn.contract_address = cr.pool
    {% if not is_incremental() %}
  ...
  , collect_events AS (
    SELECT
      ...
      c.evt_tx_hash
    FROM
      {{source('pancakeswap_v3_ethereum','PancakeV3Pool_evt_Collect')}} AS c
      LEFT JOIN {{source('pancakeswap_v3_ethereum','PancakeV3Factory_evt_PoolCreated')}} AS cr ON c.contract_address = cr.pool
    {% if not is_incremental() %}
