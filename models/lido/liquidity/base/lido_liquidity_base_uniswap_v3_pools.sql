...
{% set project_start_date = '2024-01-10' %} 
...
with  
  ...
  pools AS (
    SELECT
      ...
      token0, token1
    FROM
      {{source('uniswap_v3_base','UniswapV3Factory_evt_PoolCreated')}}
    WHERE
  ...
  tokens AS (
    ...
    FROM
      (
        ...
        FROM {{source('uniswap_v3_base','UniswapV3Factory_evt_PoolCreated')}}
  ...
  swap_events AS (
    ...
    FROM
      {{source('uniswap_v3_base','UniswapV3Pool_evt_Swap')}} AS sw
      LEFT JOIN {{source('uniswap_v3_base','UniswapV3Factory_evt_PoolCreated')}} AS cr ON sw.contract_address = cr.pool
  ...
  mint_events AS (
    SELECT
      ...
      SUM(CAST(amount1 AS DOUBLE)) AS amount1
    FROM
      {{source('uniswap_v3_base','UniswapV3Pool_evt_Mint')}} AS mt
      LEFT JOIN {{source('uniswap_v3_base','UniswapV3Factory_evt_PoolCreated')}} AS cr ON mt.contract_address = cr.pool
  ...
  burn_events AS (
    ...
    FROM
      {{source('uniswap_v3_base','UniswapV3Pool_evt_Burn')}} AS bn
      LEFT JOIN {{source('uniswap_v3_base','UniswapV3Factory_evt_PoolCreated')}} AS cr ON bn.contract_address = cr.pool
  ...
  collect_events AS (
    SELECT
      ...
      (-1) * SUM(CAST(amount1 AS DOUBLE)) AS amount1
    FROM
      {{source('uniswap_v3_base','UniswapV3Pool_evt_Collect')}} AS bn
      LEFT JOIN {{source('uniswap_v3_base','UniswapV3Factory_evt_PoolCreated')}} AS cr ON bn.contract_address = cr.pool
  ...
  all_metrics AS (
    SELECT
      ...
      CASE
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN l.token0
        ELSE l.token1
      END AS main_token,
      CASE
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN p0.symbol
        ELSE p1.symbol
      END AS main_token_symbol,
      CASE
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN l.token1
        ELSE l.token0
      END AS paired_token,
      CASE
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN p1.symbol
        ELSE p0.symbol
      END AS paired_token_symbol,
      CASE
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN  amount0 / CAST(POWER(10, p0.decimals) AS DOUBLE)      
        ELSE amount1 / CAST(POWER(10, p1.decimals) AS DOUBLE)
      END AS main_token_reserve,
      CASE 
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN  amount1 / CAST(POWER(10, p1.decimals) AS DOUBLE)
        ELSE amount0 / CAST(POWER(10, p0.decimals) AS DOUBLE)
      END AS paired_token_reserve,
      CASE
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN p0.price 
        ELSE p1.price END AS main_token_usd_price,
      CASE
        WHEN l.token0 = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452  THEN p1.price
