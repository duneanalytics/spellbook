{{ config(
    ...
    alias = alias('camelot_pools', legacy_model=True),
    partition_by = ['time'],
    materialized = 'table',
    file_format = 'delta',
...
, trading_volume_hourly as (
    select hour as time, pool, token0, amount0, p.price, coalesce(p.price*abs(amount0)/power(10, t.decimals),0) as volume
