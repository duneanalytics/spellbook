...
with pools as (
select pool as address, 'optimism' as blockchain, 'kyberswap' as project, 
max(cast(swapFeeUnits as double))/1000 as fee
from {{ source('kyber_optimism', 'ElasticFactoryV2_evt_PoolCreated') }}
...
, swap_events as (
    ...
    from {{ source('kyber_optimism', 'ElasticPoolV2_evt_Swap') }} sw
    left join {{ source('kyber_optimism', 'ElasticFactoryV2_evt_PoolCreated') }} cr on sw.contract_address = cr.pool
    {% if not is_incremental() %}
...
, mint_events as (
    ...
    from {{ source('kyber_optimism', 'ElasticPoolV2_evt_Mint') }} mt
    left join {{ source('kyber_optimism', 'ElasticFactoryV2_evt_PoolCreated') }} cr on mt.contract_address = cr.pool
    {% if not is_incremental() %}
...
, burn_events as (
    ...
    from {{ source('kyber_optimism', 'ElasticPoolV2_evt_Burn') }} bn
    left join {{ source('kyber_optimism', 'ElasticFactoryV2_evt_PoolCreated') }} cr on bn.contract_address = cr.pool
    {% if not is_incremental() %}
    ...
    select
        ...
        (-1) * sum(cast(qty0 as double)) as amount0,
        (-1) * sum(cast(qty1 as double)) as amount1
    from {{ source('kyber_optimism', 'ElasticPoolV2_evt_BurnRTokens') }} bn
    left join {{ source('kyber_optimism', 'ElasticFactoryV2_evt_PoolCreated') }} cr on bn.contract_address = cr.pool
    {% if not is_incremental() %}
    WHERE DATE_TRUNC('day', bn.evt_block_time) >= DATE '{{ project_start_date }}'
    {% else %}
