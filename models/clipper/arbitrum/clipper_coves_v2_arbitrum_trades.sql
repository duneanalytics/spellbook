{{
    config(
    schema = 'clipper_coves_v2_arbitrum',
    alias = 'trades',
...
WITH event_data as (
    SELECT
        ...
        evt_index
    FROM  {{ source('clipper_arbitrum', 'ClipperCove_v2_evt_CoveSwapped') }}
    WHERE 1=1
...
INNER JOIN {{ source('arbitrum', 'transactions') }} tx
    ...
    {% endif %}
LEFT JOIN {{ source('tokens', 'erc20', True) }} t_bought
    ON t_bought.contract_address = e.token_bought_address
    AND t_bought.blockchain = 'arbitrum'
LEFT JOIN {{ source('tokens', 'erc20', True) }} t_sold
