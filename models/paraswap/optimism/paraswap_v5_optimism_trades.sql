...
{% set trade_event_tables = [
    ...
    ,source('paraswap_optimism', 'AugustusSwapper_evt_SwappedDirect')
] %}
...
INNER JOIN {{ source('optimism', 'transactions') }} tx ON d.tx_hash = tx.hash
    ...
    {% endif %}
LEFT JOIN {{ source('tokens', 'erc20', True) }} e1 ON e1.contract_address = d.token_bought_address
    AND e1.blockchain = 'optimism'
LEFT JOIN {{ source('tokens', 'erc20', True) }} e2 on e2.contract_address = d.token_sold_address
