...
daily_balances as (
    SELECT 
        *, 
        SUM(value) OVER (PARTITION BY dao, dao_wallet_address, asset_contract_address, blockchain ORDER BY day) as balance, 
        lead(day, 1, now()) OVER (PARTITION BY dao, dao_wallet_address, asset_contract_address, blockchain ORDER BY day) AS next_day
    FROM 
    balances_all 
)
