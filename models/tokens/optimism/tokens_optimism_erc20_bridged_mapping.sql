...
FROM (
        ...
        {% if is_incremental() %}
        AND evt_block_time >= date_trunc("day", now() - interval '7' week)
        {% endif %}
        GROUP BY 1,2
        ...
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc("day", now() - interval '7' week)
        {% endif %}
        GROUP BY 1,2
        ...
        {% if is_incremental() %}
        WHERE call_block_time >= date_trunc("day", now() - interval '7' week)
        {% endif %}
        GROUP BY 1,2,3,4
