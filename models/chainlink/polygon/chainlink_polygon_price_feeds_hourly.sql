...
WITH hourly_sequence AS (
        ...
        SELECT explode(
                sequence(
                        {% if not is_incremental() %}
                            date_trunc('hour', CAST('{{project_start_date}}' AS TIMESTAMP)),
                        {% endif %}
                        {% if is_incremental() %}
                            date_trunc('hour', now() - interval '{{incremental_interval}}' day),
                        {% endif %}
                            date_trunc('hour', now()),
                            interval '1' hour
                        )
    ...
    LEFT JOIN {{ ref('chainlink_polygon_price_feeds_oracle_token_mapping') }} token_mapping 
...
FROM (
    ...
    (
        ...
        FROM (
            ...
            WHERE
                {% if not is_incremental() %}
                hourly_sequence.hr >= CAST('{{project_start_date}}' AS TIMESTAMP)
                {% endif %}
                {% if is_incremental() %}
