...
WITH
  ...
  ccip_fulfilled_transactions AS (
    ...
    FROM
      ...
      RIGHT JOIN {{ ref('chainlink_optimism_ccip_send_requested_logs_v1') }} ccip_v1_logs ON ccip_v1_logs.tx_hash = tx.hash
      {% if is_incremental() %}
        AND 
    ...
    FROM
      {{ source('optimism', 'transactions') }} tx
      RIGHT JOIN {{ ref('chainlink_optimism_ccip_send_requested_logs_v1_2') }} ccip_v2_logs ON ccip_v2_logs.tx_hash = tx.hash
      {% if is_incremental() %}
        AND
          {{ incremental_predicate('tx.block_time') }}
      {% endif %}
