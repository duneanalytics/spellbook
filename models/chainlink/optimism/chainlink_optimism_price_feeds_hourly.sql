    ...
    FROM (
        ...
        FROM (
            SELECT gs.hr, gs.feed_name,
                    ...
                    gs.underlying_token_address, AVG(underlying_token_price) AS underlying_token_price_avg
                FROM gs
                ...
                WHERE
                {% if not is_incremental() %}
                            f.block_time >= '{{project_start_date}}'
                        {% endif %}
                        {% if is_incremental() %}
                            f.block_time >= date_trunc('hour', now() - interval '1 week')
                        {% endif %}
                GROUP BY gs.hr, gs.feed_name, gs.proxy_address, gs.aggregator_address, gs.underlying_token_address
            ) a
        ) b
    ) c
WHERE orcale_price IS NOT NULL --don't overwrite where we don't have a value