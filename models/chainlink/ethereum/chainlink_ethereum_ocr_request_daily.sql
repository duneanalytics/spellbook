{{ config(
    alias = 'ocr_request_daily',
    materialized = 'view',
    post_hook='{{ expose_spells(\'["ethereum"]\',
                                "sector",
                                "chainlink",
                                \'["linkpool_ryan"]\') }}'
    )
}}

{% set truncate_by = 'day' %}

WITH
  ocr_request_daily_meta AS (
    SELECT
      COALESCE(
        date_trunc('{{truncate_by}}', fulfilled.block_time),
        date_trunc('{{truncate_by}}', reverted.block_time)
      ) AS "date_start",      
      COALESCE(
        fulfilled.node_address,
        reverted.node_address
      ) AS "node_address",
      COALESCE(COUNT(fulfilled.token_value), 0) as fulfilled_requests,
      COALESCE(COUNT(reverted.token_value), 0) as reverted_requests,
      COALESCE(COUNT(fulfilled.token_value), 0) + COALESCE(COUNT(reverted.token_value), 0) as total requests
    FROM
      {{ ref('chainlink_ethereum_ocr_fulfilled_transactions') }} fulfilled
      FULL OUTER JOIN {{ ref('chainlink_ethereum_ocr_reverted_transactions') }} reverted ON
        reverted.block_time = fulfilled.block_time AND
        reverted.node_address = fulfilled.node_address
    GROUP BY
      1, 2
    ORDER BY
      1, 2
  ),
  ocr_request_daily AS (
    SELECT
      'ethereum' as blockchain,
      date_start,
      ocr_request_daily_meta.node_address as node_address,
      operator_name,
      fulfilled_requests,
      reverted_requests,
      total_requests
    FROM ocr_request_daily_meta
    LEFT JOIN {{ ref('chainlink_ethereum_ocr_operator_node_meta') }} ON LOWER(ocr_operator_node_meta.node_address) = LOWER(ocr_request_daily_meta.node_address)
  )
SELECT 
  blockchain,
  date_start,
  node_address,
  operator_name,
  fulfilled_requests,
  reverted_requests,
  total_requests
FROM
  ocr_request_daily
ORDER BY
  "date_start"
