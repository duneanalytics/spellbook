...
WITH
  admin_address_meta as (
    SELECT DISTINCT
      admin_address
    FROM
      {{ref('chainlink_avalanche_c_fm_reward_evt_transfer_daily')}} fm_reward_evt_transfer_daily
  ),
  link_usd_daily AS (
    SELECT
  ...
  fm_reward_daily AS (
    ...
    LEFT JOIN {{ ref('chainlink_avalanche_c_ocr_operator_admin_meta') }} ocr_operator_admin_meta ON ocr_operator_admin_meta.admin_address = fm_reward_evt_transfer_daily.admin_address
    ORDER BY date_start
  )
SELECT
