...
WITH gs AS (
    ...
    FROM (
        SELECT explode(
                ...
                proxy_address,
                aggregator_address
        FROM {{ ref('chainlink_bnb_price_feeds_oracle_addresses_legacy') }}
    ) oa LEFT JOIN {{ ref('chainlink_bnb_price_feeds_oracle_token_mapping_legacy') }} c ON c.proxy_address = oa.proxy_address
)
