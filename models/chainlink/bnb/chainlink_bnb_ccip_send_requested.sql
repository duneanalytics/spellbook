...
FROM
  {{ ref('chainlink_bnb_ccip_send_traces') }} ccip_traces
  LEFT JOIN {{ source('erc20_bnb', 'evt_Transfer') }} reward_evt_transfer ON reward_evt_transfer.evt_tx_hash = ccip_traces.tx_hash
  LEFT JOIN {{ ref('chainlink_bnb_ccip_token_meta') }} token_addresses ON token_addresses.token_contract = reward_evt_transfer.contract_address
  LEFT JOIN {{ ref('chainlink_bnb_ccip_tokens_locked_logs') }} tokens_locked ON tokens_locked.tx_hash = ccip_traces.tx_hash
WHERE
    (
        (
            ccip_traces.value > 0
            AND reward_evt_transfer.contract_address IN (
                SELECT
                    token_contract
                FROM
                    {{ ref('chainlink_bnb_ccip_token_meta') }}
                WHERE
                    token_symbol = 'WBNB'
            )
        )
