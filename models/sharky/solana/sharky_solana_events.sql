...
WITH sharky_txs AS (
    ...
    raw_events AS (
        SELECT 'solana'                                                                                     AS blockchain,
               ...
               (abs(filtered_txs.post_balances[1] - filtered_txs.pre_balances[1]) / 1e9) * p.price          AS amount_usd,
               (abs(filtered_txs.post_balances[1] - filtered_txs.pre_balances[1]) / 1e9)                    AS amount_original,
               CAST(abs(filtered_txs.post_balances[1] - filtered_txs.pre_balances[1]) AS DECIMAL(38, 0))    AS amount_raw,
               filter(
    ...
    decoded_events AS (
        SELECT *,
            ...
            UPPER(SUBSTR(CAST(fromBase58(sharky_instructions[1].data) AS VARCHAR), 3)) AS first_ix_data
        FROM raw_events
        WHERE cardinality(sharky_instructions) > 0
    ),
    events AS (
        SELECT blockchain,
            project,
            ...
            CASE
               ...
               WHEN starts_with(first_ix_data, '2C0C4C90D2D0EF55') THEN 'Offer'
               WHEN starts_with(first_ix_data, '4040A0D33324B19E') THEN 'Rescind'
               WHEN (
                    starts_with(first_ix_data, '9935333BDE663483') -- TakeLoan
                            OR starts_with(first_ix_data, 'FF73DC3A1A9D70B9') -- TakeLoanV3
                        ) THEN 'Take'
               WHEN (
                            starts_with(first_ix_data, 'E05D904D3D118936') -- RepayLoan
                            OR starts_with(first_ix_data, 'BB51FA5992571440') -- RepayLoanEscrow
                            OR starts_with(first_ix_data, '617B55364C103D9D') -- RepayLoanV3
                        ) THEN 'Repay'
               WHEN (
                            starts_with(first_ix_data, 'CB5477E28901B417') -- ForecloseLoan
                            OR starts_with(first_ix_data, 'DAF5ED6D2ECE0D0E') -- ForecloseLoanEscrow
                            OR starts_with(first_ix_data, '88B8323AB75C3FD8') -- ForecloseLoanV3
                        ) THEN 'Foreclose'
               WHEN (
                            starts_with(first_ix_data, '02D0DEBE6D94F775') -- ExtendLoan
                            OR starts_with(first_ix_data, '35BC6DD273220509') -- ExtendLoanEscrow
                            OR starts_with(first_ix_data, '471B11834E493E5C') -- ExtendLoanV3
                        ) THEN 'Extend'
            END AS evt_type
    FROM decoded_events
)
