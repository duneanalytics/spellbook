...
with
 onramp_trades as (
    ...
    from (
        ...
        and token_bought_address not in (select contract_address from {{ source('tokens_ethereum', 'erc20_stablecoins') }})
        and token_sold_address in (select contract_address from {{ source('tokens_ethereum', 'erc20_stablecoins') }})
        UNION ALL
        select tx_hash, evt_index, project, version
        from {{ ref('dex_trades') }}
        where blockchain = 'ethereum'
