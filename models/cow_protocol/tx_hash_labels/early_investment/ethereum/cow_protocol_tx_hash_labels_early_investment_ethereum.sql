...
with
  early_investment_tokens as (
    select
        early_investment_token_address
    from
        (
        ...
        ) t(early_investment_token_address)
 ),
 ...
 early_investment_trades as (
    ...
    from (
        ...
        and token_sold_address in (select early_investment_token_address from early_investment_tokens)
        UNION ALL
        select tx_hash
        from {{ ref('dex_trades') }}
        where blockchain = 'ethereum'
