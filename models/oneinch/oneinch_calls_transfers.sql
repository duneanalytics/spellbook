    ...
    join (
        ...
        where 
            ...
            {% else %}
                block_time >= {{ project_start_date }}
            {% endif %}
            and (blockchain, "to", {{ selector }}) in (select blockchain, call_to, call_selector from methods)
            and call_type = 'call'
    ...
    left join (
        ...
        from (
            select 
                ...
                , array_agg(if(value > uint256 '0', null, trace_address)) over(partition by blockchain, tx_hash) as trf
            from {{ source('evms', 'traces') }}
            where
                {% if is_incremental() %}
