...
(
    SELECT
        call_block_number,
        from_hex(json_extract_scalar(order_, '$.maker')) AS maker,
        from_hex(json_extract_scalar(order_, '$.takerAsset')) AS token_bought_address,
        from_hex(json_extract_scalar(order_, '$.makerAsset')) AS token_sold_address,
        contract_address,
...
(
    SELECT
        ...
        '3' as version,
        CAST(NULL as VARBINARY) as taker, -- get from base table downstream
        maker,
...
INNER JOIN {{ source('ethereum', 'transactions') }} as tx
    ...
    {% endif %}
LEFT JOIN {{ source('tokens', 'erc20', True) }} as token_bought
    ON token_bought.contract_address = src.token_bought_address
    AND token_bought.blockchain = '{{blockchain}}'
LEFT JOIN {{ source('tokens', 'erc20', True) }} as token_sold
