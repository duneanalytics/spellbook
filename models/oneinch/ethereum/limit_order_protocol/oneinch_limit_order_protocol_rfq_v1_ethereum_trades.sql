...
(
    ...
    INNER JOIN
        ...
        {% else %}
        AND ts.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(6) WITH TIME ZONE)
        {% endif %}
    INNER JOIN
        ...
        {% else %}
        AND tf1.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(6) WITH TIME ZONE)
        {% endif %}
    INNER JOIN
        ...
        {% else %}
        AND tf2.block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(6) WITH TIME ZONE)
        {% endif %}
    WHERE
        call.call_success
        {% if is_incremental() %}
        ...
        {% else %}
        AND call.call_block_time >= CAST('{{project_start_date}}' AS TIMESTAMP(6) WITH TIME ZONE)
        {% endif %}
)
SELECT
