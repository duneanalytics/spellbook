...
{% set project_start_date = '2022-06-03 00:00:00' %}
...
(
    SELECT
        ...
        token_bought_amount_raw / 10 ^ erc20a.decimals AS token_a_amount,
        token_sold_amount_raw / 10 ^ erc20b.decimals AS token_b_amount,
        project,
        version,
        category,
        ...
        coalesce(
            amount_usd,
            token_bought_amount_raw / 10 ^ (CASE token_bought_address WHEN '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' THEN 18 ELSE pa.decimals END) * (CASE token_bought_address WHEN '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' THEN pe.price ELSE pa.price END),
            token_sold_amount_raw / 10 ^ (CASE token_sold_address WHEN '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' THEN 18 ELSE pb.decimals END) * (CASE token_sold_address WHEN '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' THEN pe.price ELSE pb.price END)
        ) as amount_usd,
    ...
    FROM (
        ...
        (
            ...
            UNION ALL
            SELECT sender as taker, srcToken as from_token, dstToken as to_token, spentAmount as from_amount, returnAmount as to_amount, evt_tx_hash as tx_hash, evt_block_time as block_time, '' as call_trace_address, evt_index, contract_address, '2' as version 
            FROM {{ source('oneinch_v2_ethereum', 'OneInchExchange_evt_Swapped') }}
            WHERE
                evt_block_time >= '{{project_start_date}}'
            UNION ALL
            SELECT sender as taker, srcToken as from_token, dstToken as to_token, spentAmount as from_amount, returnAmount as to_amount, evt_tx_hash as tx_hash, evt_block_time as block_time, '' as call_trace_address, evt_index, contract_address, '3' as version
    ...
    --             SELECT fromToken as from_token, toToken as to_token, fromTokenAmount as from_amount, minReturnAmount as to_amount, call_tx_hash as tx_hash, call_block_time as block_time, call_trace_address as trace_address, CAST(NULL as integer) AS evt_index, contract_address, '1' as version FROM {{ source('oneinch_ethereum', 'exchange_v7_call_swap') }} WHERE call_success and call_block_time >= '{{project_start_date}}' UNION ALL
    --             SELECT fromToken as from_token, toToken as to_token, fromTokenAmount as from_amount, minReturnAmount as to_amount, call_tx_hash as tx_hash, call_block_time as block_time, call_trace_address as trace_address, CAST(NULL as integer) AS evt_index, contract_address, '1' as version FROM {{ source('oneinch_ethereum', 'OneInchExchange_call_swap') }} WHERE call_success and call_block_time >= '{{project_start_date}}' UNION ALL
    --             SELECT srcToken as from_token, dstToken as to_token, spentAmount as from_amount, returnAmount as to_amount, evt_tx_hash as tx_hash, evt_block_time as block_time, '' as call_trace_address, evt_index, contract_address, '2' as version 
    --                 FROM {{ source('oneinch_v2_ethereum', 'OneInchExchange_evt_Swapped') }}
    --                 WHERE
    --                     evt_block_time >= '{{project_start_date}}'
    --             UNION ALL
    --             SELECT srcToken as from_token, dstToken as to_token, spentAmount as from_amount, returnAmount as to_amount, evt_tx_hash as tx_hash, evt_block_time as block_time, '' as call_trace_address, evt_index, contract_address, '3' as version 
    --                 FROM {{ source('oneinch_v3_ethereum', 'AggregationRouterV3_evt_Swapped') }}
    --                 WHERE
    --                     evt_block_time >= '{{project_start_date}}'
    --             UNION ALL
    ...
    --             SELECT fromToken as from_token, toToken as to_token, fromTokenAmount as from_amount, minReturnAmount as to_amount, call_tx_hash as tx_hash, call_block_time as block_time, call_trace_address as trace_address, CAST(NULL as integer) AS evt_index, contract_address, '1' as version FROM {{ source('oneinch_ethereum' ,'exchange_v7_call_swap') }} WHERE call_success and call_block_time >= '{{project_start_date}}' UNION ALL
    --             SELECT fromToken as from_token, toToken as to_token, fromTokenAmount as from_amount, minReturnAmount as to_amount, call_tx_hash as tx_hash, call_block_time as block_time, call_trace_address as trace_address, CAST(NULL as integer) AS evt_index, contract_address, '1' as version FROM {{ source('oneinch_ethereum' ,'OneInchExchange_call_swap') }} WHERE call_success and call_block_time >= '{{project_start_date}}' UNION ALL
    --             SELECT srcToken as from_token, dstToken as to_token, spentAmount as from_amount, returnAmount as to_amount, evt_tx_hash as tx_hash, evt_block_time as block_time, '' as call_trace_address, evt_index, contract_address, '2' as version FROM {{ source('oneinch_v2_ethereum', 'OneInchExchange_evt_Swapped') }} where evt_block_time >= '{{project_start_date}}' UNION ALL
