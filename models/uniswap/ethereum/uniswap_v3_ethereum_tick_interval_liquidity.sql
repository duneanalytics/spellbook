...
WITH pools AS (
    SELECT pool,
           ...
           token0,
           token1,
           te0.symbol   AS symbol0,
           te1.symbol   AS symbol1,
           te0.decimals AS decimals0,
           te1.decimals AS decimals1
    FROM {{ source('uniswap_v3_ethereum', 'Factory_evt_PoolCreated') }} pl
    LEFT JOIN (SELECT * FROM {{ ref('tokens_erc20') }} WHERE blockchain='ethereum') te0
        ON pl.token0=te0.contract_address
    LEFT JOIN (SELECT * FROM {{ ref('tokens_erc20') }} WHERE blockchain='ethereum') te1
            ON pl.token1=te1.contract_address
    WHERE pool IN (
...
SELECT pool,
       ...
       tickUpper,
       tickSpacing,
       (tickUpper - tickLower) / tickSpacing AS tickintervals
...
, minttick AS (
    SELECT pool,
           ...
           symbol1,
           block,
           tickLower + ((INCREMENT - 1) * tickSpacing) AS lowerbound,
           tickLower + (INCREMENT * tickSpacing)       AS upperbound,
