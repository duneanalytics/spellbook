...
FROM {{ source('celer_ethereum', 'Bridge_evt_Send') }} cb
...
LEFT JOIN {{ source('prices', 'usd') }} peth ON peth.minute = date_trunc('minute', cb.evt_block_time)
                       ...
                       {% endif %}
WHERE p.blockchain ='ethereum' AND peth.blockchain ='ethereum'
{% if is_incremental() %}
...
SELECT DISTINCT 'ethereum' as blockchain,
       ...
       source_chain_id,
       destination_chain_id,
       10 as source_chain_name,
       1 as destination_chain_name,
       lower(token) as token_address,
       p.symbol as token_symbol,
...
FROM {{ source('celer_ethereum', 'Bridge_evt_WithdrawDone') }} cb
LEFT JOIN {{ source('prices', 'usd') }} p ON p.minute = date_trunc('minute', cb.evt_block_time)
                       AND lower(p.contract_address) = lower(cb.token)
                       {% if is_incremental() %}
                       AND p.minute >= date_trunc("day", now() - interval '1 week')
