...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'optimism'
inner join {{ source('optimism', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('optimism', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
and tr.success = true
