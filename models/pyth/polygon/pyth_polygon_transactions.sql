...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'polygon'
inner join {{ source('polygon', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('polygon', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
