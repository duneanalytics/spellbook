...
from {{ source('scroll', 'traces') }} tr
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'scroll'
inner join {{ source('scroll', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('scroll', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
