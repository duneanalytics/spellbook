...
{% set project_start_date = '2023-07-10' %}
...
from {{ source('mantle', 'traces') }} tr
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'mantle'
inner join {{ source('mantle', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('mantle', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
