...
{% set project_start_date = '2023-06-14' %}
...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'gnosis'
inner join {{ source('gnosis', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('gnosis', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
and tr.success = true
