...
with price_feed_ids as (
    select identifier, category, token1, token2, "hash", pending, expo from (
        ...
        FROM
            {{ ref('pyth_gnosis_active_price_feed_ids') }}
...
, pyth_contracts as (
    ...
    where chain_type = 'evm' and chain = 'gnosis'
...
left join {{ source('gnosis', 'contracts') }} bc on log.tx_to = bc.address
where 
{% if is_incremental() %}
