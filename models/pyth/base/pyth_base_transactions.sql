...
{% set project_start_date = '2023-07-26' %}
...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'base'
inner join {{ source('base', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('base', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
and tr.success = true
