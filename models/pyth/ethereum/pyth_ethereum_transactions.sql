...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'ethereum'
inner join {{ source('ethereum', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('ethereum', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
