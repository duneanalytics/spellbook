...
with price_feed_ids as (
    select identifier, category, token1, token2, "hash", pending, expo from (
        ...
        FROM
            {{ ref('pyth_zksync_active_price_feed_ids') }}
...
, pyth_contracts as (
    ...
    where chain_type = 'evm' and chain = 'zksync'
...
left join {{ source('zksync', 'contracts') }} bc on log.tx_to = bc.address
where 
{% if is_incremental() %}
