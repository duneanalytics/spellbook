...
{% set project_start_date = '2023-04-24' %}
...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'zksync'
inner join {{ source('zksync', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('zksync', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
