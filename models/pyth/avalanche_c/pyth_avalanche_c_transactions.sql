...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'avalanche_c'
inner join {{ source('avalanche_c', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('avalanche_c', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
