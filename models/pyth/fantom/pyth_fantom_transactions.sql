...
inner join pyth_chain_contract pcc on tr.to = pcc.contract_address and pcc.chain = 'fantom'
inner join {{ source('fantom', 'transactions') }} txs on tr.tx_hash = txs.hash
left join {{ source('fantom', 'contracts') }} bc on tr."from" = bc.address
where 
tr.tx_success = true
