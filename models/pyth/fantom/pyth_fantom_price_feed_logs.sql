...
with price_feed_ids as (
    select identifier, category, token1, token2, "hash", pending, expo from (
        ...
        FROM
            {{ ref('pyth_fantom_active_price_feed_ids') }}
...
, pyth_contracts as (
    ...
    where chain_type = 'evm' and chain = 'fantom'
...
left join {{ source('fantom', 'contracts') }} bc on log.tx_to = bc.address
where 
{% if is_incremental() %}
