...
LEFT JOIN {{ source('prices', 'usd') }} prices ON prices.blockchain = '{{blockchain}}'
    AND (
            ...
            OR t.contract_address IS NULL
            AND prices.contract_address=(SELECT from_hex(wrapped_native_token_address) FROM {{ source('evms', 'info') }} WHERE blockchain='{{blockchain}}')
        )
