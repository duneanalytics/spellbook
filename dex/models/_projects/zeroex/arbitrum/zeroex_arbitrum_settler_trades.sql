...
select  logs.tx_hash, logs.block_time, logs.block_number,
    ...
    case when varbinary_substring(when varbinary_substring(logs.topic2, 13, 20) = logs.tx_from then 1 )
...
results as (
    ...
    join {{ source('arbitrum', 'transactions') }} tr on tr.hash = trades.tx_hash and tr.block_time = trades.block_time and tr.block_number = trades.block_number 
    left join tokens tt on tt.blockchain = 'arbitrum' and tt.contract_address = taker_token
    left join tokens tm on tm.blockchain = 'arbitrum' and tm.contract_address = maker_token
    left join prices pt on pt.blockchain = 'arbitrum' and pt.contract_address = taker_token and pt.minute = date_trunc('minute', trades.block_time)
    left join prices pm on pm.blockchain = 'arbitrum' and pm.contract_address = maker_token and pm.minute = date_trunc('minute', trades.block_time)
