...
where contract_address = 0x00000000000004533fe15556b1e086bb1a72ceae and blockchain = 'avalanche_c'
...
from (
  ...
  join result_0x_settler_addresses a on a.settler_address = tr.to and a.blockchain = 'avalanche_c' 
...
tokens as (  
        select distinct te.* from {{ source('tokens', 'erc20') }} as te 
           ...
           where te.blockchain = 'avalanche_c'
...
prices as (  
        select distinct pu.* from {{ source('prices', 'usd') }} as  pu
           ...
           where pu.blockchain = 'avalanche_c' 
           {% if is_incremental() %}
...
results as (
    ...
    join {{ source('avalanche_c', 'transactions') }} tr on tr.hash = trades.tx_hash and tr.block_time = trades.block_time and tr.block_number = trades.block_number 
    left join tokens tt on tt.blockchain = 'avalanche_c' and tt.contract_address = taker_token
    left join tokens tm on tm.blockchain = 'avalanche_c' and tm.contract_address = maker_token
    left join prices pt on pt.blockchain = 'avalanche_c' and pt.contract_address = taker_token and pt.minute = date_trunc('minute', trades.block_time)
    left join prices pm on pm.blockchain = 'avalanche_c' and pm.contract_address = maker_token and pm.minute = date_trunc('minute', trades.block_time)
...
results_usd as (
 ...
 select  
        ...
        CASE WHEN maker_token IN (0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7,
                                ...
                                0x9702230a8ea53601f5cd2dc00fdbc13d4df4a8c7) AND  maker_amount IS NOT NULL
             THEN maker_amount
             WHEN taker_token IN (0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7,
                                0xb97ef9ef8734c71904d8002f8b6bc66dd9c48a6e,
                                0x152b9d0fdc40c096757f570a51e494bd4b943e50,
                                0x49d5c2bdffac6ce2bfdb6640f4f80f226bc10bab,
                                0x9702230a8ea53601f5cd2dc00fdbc13d4df4a8c7) AND taker_amount IS NOT NULL
             THEN taker_amount
