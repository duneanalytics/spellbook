...
where contract_address = 0x00000000000004533fe15556b1e086bb1a72ceae and blockchain = 'optimism'
...
from (
  ...
  join result_0x_settler_addresses a on a.settler_address = tr.to and a.blockchain = 'optimism' 
...
results as (
    ...
    join {{ source('optimism', 'transactions') }} tr on tr.hash = trades.tx_hash and tr.block_time = trades.block_time and tr.block_number = trades.block_number 
    left join tokens tt on tt.blockchain = 'optimism' and tt.contract_address = taker_token
    left join tokens tm on tm.blockchain = 'optimism' and tm.contract_address = maker_token
    left join prices pt on pt.blockchain = 'optimism' and pt.contract_address = taker_token and pt.minute = date_trunc('minute', trades.block_time)
    left join prices pm on pm.blockchain = 'optimism' and pm.contract_address = maker_token and pm.minute = date_trunc('minute', trades.block_time)
