...
where contract_address = 0x00000000000004533fe15556b1e086bb1a72ceae and blockchain = 'bnb'
...
from (
  ...
  join result_0x_settler_addresses a on a.settler_address = tr.to and a.blockchain = 'bnb' 
...
results as (
    ...
    join {{ source('bnb', 'transactions') }} tr on tr.hash = trades.tx_hash and tr.block_time = trades.block_time and tr.block_number = trades.block_number 
    left join tokens tt on tt.blockchain = 'bnb' and tt.contract_address = taker_token
    left join tokens tm on tm.blockchain = 'bnb' and tm.contract_address = maker_token
    left join prices pt on pt.blockchain = 'bnb' and pt.contract_address = taker_token and pt.minute = date_trunc('minute', trades.block_time)
    left join prices pm on pm.blockchain = 'bnb' and pm.contract_address = maker_token and pm.minute = date_trunc('minute', trades.block_time)
 ...
 select  
        ...
        CASE WHEN maker_token IN (0x55d398326f99059ff775485246999027b3197955,
             ...
             WHEN taker_token IN (0x55d398326f99059ff775485246999027b3197955,
                                ...
                                0x7130d2a12b9bcbfae4f2634d864a1ee1ce3ead9c,
                                0x2170ed0880ac9a755fd29b2688956bd959f933f8,
                                0xe9e7cea3dedca5984780bafc599bd69add087d56)  AND taker_amount IS NOT NULL
             THEN taker_amount
