...
where contract_address = 0x00000000000004533fe15556b1e086bb1a72ceae and blockchain = 'base'
...
results as (
    ...
    join {{ source('base', 'transactions') }} tr on tr.hash = trades.tx_hash and tr.block_time = trades.block_time and tr.block_number = trades.block_number 
    left join tokens tt on tt.blockchain = 'base' and tt.contract_address = taker_token
    left join tokens tm on tm.blockchain = 'base' and tm.contract_address = maker_token
    left join prices pt on pt.blockchain = 'base' and pt.contract_address = taker_token and pt.minute = date_trunc('minute', trades.block_time)
    left join prices pm on pm.blockchain = 'base' and pm.contract_address = maker_token and pm.minute = date_trunc('minute', trades.block_time)
