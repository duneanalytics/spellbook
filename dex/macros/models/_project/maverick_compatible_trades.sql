{% macro maverick_v1_compatible_trades(
        blockchain = '',
        project = '',
        version = '',
        source_evt_swap = '',
...
{% macro maverick_v2_compatible_trades(
        blockchain = '',
        project = '',
        version = '',
        source_evt_swap = '',
...
WITH dexs AS (
    SELECT
        ...
        t.amountOut AS token_bought_amount_raw,
        t.amountIn AS token_sold_amount_raw,
        CASE WHEN json_extract_scalar(json_parse(t.params),'$.tokenAIn') = 'true' 
            THEN f.tokenB ELSE f.tokenA 
        END AS token_bought_address,
        CASE WHEN json_extract_scalar(json_parse(t.params),'$.tokenAIn') = 'true' 
            THEN f.tokenA ELSE f.tokenB 
