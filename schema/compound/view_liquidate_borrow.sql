CREATE OR REPLACE VIEW compound.view_liquidate_borrow AS
 SELECT t.symbol,                                                                                                                                              
    ...
    tx.block_time                                                                                                                                              
   FROM ( SELECT "cErc20_evt_LiquidateBorrow".liquidator,                                                                                                      
        ...
        UNION                                                                                                                                                  
         SELECT "CErc20Delegator_evt_LiquidateBorrow".liquidator,                                                                                              
           ...
           FROM compound_v2."CErc20Delegator_evt_LiquidateBorrow") events                                                                                      
     LEFT JOIN ( SELECT "cErc20_evt_RepayBorrow".payer,                                                                                                        
        ...
        UNION
         SELECT "CErc20Delegator_evt_RepayBorrow".payer,
           ...
           FROM compound_v2."CErc20Delegator_evt_RepayBorrow") repay ON events.evt_tx_hash = repay.evt_tx_hash
     ...
     LEFT JOIN ( SELECT usd.minute,
           ...
           FROM prices.usd
        UNION
         SELECT generate_series('2019-11-18 00:00:00+00'::timestamp with time zone, now(), '00:01:00'::interval) AS generate_series,
            '\x6b175474e89094c44da98b954eedeac495271d0f'::bytea AS contract_address,
            'DAI'::text AS symbol,
            1 AS price) p ON p.minute = date_trunc('minute'::text, tx.block_time) AND p.contract_address = c_repay.underlying_token_address;
