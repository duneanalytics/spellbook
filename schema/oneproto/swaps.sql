...
WITH swap AS (
    ...
    FROM (
        SELECT "fromToken" as from_token,
               ...
               contract_address,
               evt_index
        FROM oneproto."oneprotoAudit_evt_Swapped"
    ) tmp
...
VALUES ('14 1 * * *', $$SELECT oneproto.insert_swap((SELECT max(block_time) - interval '2 days' FROM oneproto.swaps), (SELECT now()), (SELECT max(number) FROM ethereum.blocks WHERE time < (SELECT max(block_time) - interval '2 days' FROM oneproto.swaps)), (SELECT MAX(number) FROM ethereum.blocks));$$)
