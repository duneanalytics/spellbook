...
WITH rows AS (
    ...
    SELECT
        ...
        trade."fromAmount"/1e18 * (SELECT currency_rate FROM synthetix.rates WHERE trade."fromCurrencyKey" = currency_key AND block_time <= trade.evt_block_time ORDER BY block_time DESC LIMIT 1)/1e18 as token_a_amount,
        trade."toAmount"/1e18 * (SELECT currency_rate FROM synthetix.rates WHERE trade."toCurrencyKey" = currency_key AND block_time <= trade.evt_block_time ORDER BY block_time DESC LIMIT 1)/1e18 as token_b_amount,
