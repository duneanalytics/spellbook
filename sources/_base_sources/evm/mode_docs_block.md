...
The `mode.blocks` table contains information about blocks on the mode blockchain. It includes:
...
The `mode.traces` table contains records of execution steps for transactions on the mode blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
The `mode.contracts` table contains information about verified smart contracts on the mode blockchain. It includes:
...
{% docs mode_contracts_submitted_doc %}

The `mode.contracts_submitted` table contains information about manually submitted contract verifications on the mode blockchain. It includes:
...
The `mode.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `mode.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
This table is used for tracking ERC20 token movements on the mode network.
