...
The `apechain.blocks` table contains information about blocks on the apechain blockchain. It includes:
...
The `apechain.transactions` table contains detailed information about transactions on the apechain blockchain. It includes:
...
The `apechain.logs` table contains event logs emitted by smart contracts on the apechain blockchain. It includes:
...
The `apechain.traces` table contains records of execution steps for transactions on the apechain blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
The `apechain.creation_traces` table contains data about contract creation events on the apechain blockchain. It includes:
...
The `apechain.contracts` table contains information about verified smart contracts on the apechain blockchain. It includes:
...
The `apechain.contracts_submitted` table contains information about manually submitted contract verifications on the apechain blockchain. It includes:
...
The `apechain.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `apechain.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs erc20_apechain_evt_Transfer_doc %}

The `erc20_apechain.evt_transfer` table contains Transfer events for ERC20 tokens on the apechain blockchain. It includes:
...
This table is used for tracking ERC20 token movements on the apechain network.
...
{% docs erc20_apechain_evt_Approval_doc %}
...
{% docs erc1155_apechain_evt_TransferSingle_doc %}

The `erc1155_apechain.evt_TransferSingle` table contains TransferSingle events for ERC1155 tokens on the apechain blockchain. It includes:
...
This table is used for tracking individual ERC1155 token transfers on the apechain network.
...
{% docs erc1155_apechain_evt_TransferBatch_doc %}

The `erc1155_apechain.evt_TransferBatch` table contains TransferBatch events for ERC1155 tokens on the apechain blockchain. It includes:
...
This table is used for tracking batch transfers of multiple ERC1155 tokens on the apechain network.

Please be aware that this table is the raw ERC1155 event data, and does not include any additional metadata, context or is in any way filtered or curated. Use nft.transfers for a more complete and curated view of NFT transfers.

{% enddocs %}

{% docs erc1155_apechain_evt_ApprovalForAll_doc %}

The `erc1155_apechain.evt_ApprovalForAll` table contains ApprovalForAll events for ERC1155 tokens on the apechain blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC1155 token collections on the apechain network.

{% enddocs %}

{% docs erc721_apechain_evt_Transfer_doc %}

The `erc721_apechain.evt_Transfer` table contains Transfer events for ERC721 tokens on the apechain blockchain. It includes:
...
This table is used for tracking ERC721 token (NFT) transfers on the apechain network.
...
{% docs erc721_apechain_evt_Approval_doc %}

The `erc721_apechain.evt_Approval` table contains Approval events for ERC721 tokens on the apechain blockchain. It includes:
...
This table is used for analyzing approvals for individual ERC721 tokens (NFTs) on the apechain network.

{% enddocs %}

{% docs erc721_apechain_evt_ApprovalForAll_doc %}

The `erc721_apechain.evt_ApprovalForAll` table contains ApprovalForAll events for ERC721 tokens on the apechain blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC721 token collections on the apechain network.
{% enddocs %}
