...
The `flare.blocks` table contains information about blocks on the flare blockchain. It includes:
...
The `flare.traces` table contains records of execution steps for transactions on the flare blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
The `flare.creation_traces` table contains data about contract creation events on the flare blockchain. It includes:
...
{% docs flare_contracts_submitted_doc %}

The `flare.contracts_submitted` table contains information about manually submitted contract verifications on the flare blockchain. It includes:
...
The `flare.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `flare.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `erc20_flare.evt_transfer` table contains Transfer events for ERC20 tokens on the flare blockchain. It includes:
...
This table is used for tracking ERC20 token movements on the flare network.
...
The `erc20_flare.evt_Approval` table contains Approval events for ERC20 tokens on the flare blockchain. It includes:
...
This table is used for analyzing ERC20 token approvals and spending permissions on the flare network.

{% enddocs %}

{% docs erc1155_flare_evt_TransferSingle_doc %}

The `erc1155_flare.evt_TransferSingle` table contains TransferSingle events for ERC1155 tokens on the flare blockchain. It includes:
...
This table is used for tracking individual ERC1155 token transfers on the flare network.
...
The `erc1155_flare.evt_TransferBatch` table contains TransferBatch events for ERC1155 tokens on the flare blockchain. It includes:
...
This table is used for tracking batch transfers of multiple ERC1155 tokens on the flare network.
...
The `erc1155_flare.evt_ApprovalForAll` table contains ApprovalForAll events for ERC1155 tokens on the flare blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC1155 token collections on the flare network.

{% enddocs %}

{% docs erc721_flare_evt_Transfer_doc %}

The `erc721_flare.evt_Transfer` table contains Transfer events for ERC721 tokens on the flare blockchain. It includes:
...
This table is used for tracking ERC721 token (NFT) transfers on the flare network.
...
The `erc721_flare.evt_Approval` table contains Approval events for ERC721 tokens on the flare blockchain. It includes:
...
This table is used for analyzing approvals for individual ERC721 tokens (NFTs) on the flare network.

{% enddocs %}

{% docs erc721_flare_evt_ApprovalForAll_doc %}

The `erc721_flare.evt_ApprovalForAll` table contains ApprovalForAll events for ERC721 tokens on the flare blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC721 token collections on the flare network.
{% enddocs %}
