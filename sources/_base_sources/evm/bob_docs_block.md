...
The `bob.blocks` table contains information about blocks on the bob blockchain. It includes:
...
The `bob.traces` table contains records of execution steps for transactions on the bob blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
The `bob.contracts` table contains information about verified smart contracts on the bob blockchain. It includes:
...
{% docs bob_contracts_submitted_doc %}

The `bob.contracts_submitted` table contains information about manually submitted contract verifications on the bob blockchain. It includes:
...
The `bob.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `bob.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
This table is used for tracking ERC20 token movements on the bob network.
