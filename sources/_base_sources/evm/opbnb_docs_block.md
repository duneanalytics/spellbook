...
The `opbnb.traces` table contains records of execution steps for transactions on the opbnb blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
The `opbnb.creation_traces` table contains data about contract creation events on the opbnb blockchain. It includes:
...
{% docs opbnb_contracts_doc %}

The `opbnb.contracts` table contains information about verified smart contracts on the opbnb blockchain. It includes:
...
{% docs opbnb_contracts_submitted_doc %}

The `opbnb.contracts_submitted` table contains information about manually submitted contract verifications on the opbnb blockchain. It includes:
...
{% docs opbnb_traces_decoded_doc %}

The `opbnb.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs opbnb_logs_decoded_doc %}

The `opbnb.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs erc20_opbnb_evt_Transfer_doc %}

The `erc20_opbnb.evt_transfer` table contains Transfer events for ERC20 tokens on the opbnb blockchain. It includes:
...
This table is used for tracking ERC20 token movements on the opbnb network.
...
{% docs erc20_opbnb_evt_Approval_doc %}

The `erc20_opbnb.evt_Approval` table contains Approval events for ERC20 tokens on the opbnb blockchain. It includes:
...
This table is used for analyzing ERC20 token approvals and spending permissions on the opbnb network.

{% enddocs %}

{% docs erc1155_opbnb_evt_TransferSingle_doc %}

The `erc1155_opbnb.evt_TransferSingle` table contains TransferSingle events for ERC1155 tokens on the opbnb blockchain. It includes:
...
This table is used for tracking individual ERC1155 token transfers on the opbnb network.
...
{% docs erc1155_opbnb_evt_TransferBatch_doc %}

The `erc1155_opbnb.evt_TransferBatch` table contains TransferBatch events for ERC1155 tokens on the opbnb blockchain. It includes:
...
This table is used for tracking batch transfers of multiple ERC1155 tokens on the opbnb network.

Please be aware that this table is the raw ERC1155 event data, and does not include any additional metadata, context or is in any way filtered or curated. Use nft.transfers for a more complete and curated view of NFT transfers.

{% enddocs %}

{% docs erc1155_opbnb_evt_ApprovalForAll_doc %}

The `erc1155_opbnb.evt_ApprovalForAll` table contains ApprovalForAll events for ERC1155 tokens on the opbnb blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC1155 token collections on the opbnb network.

{% enddocs %}

{% docs erc721_opbnb_evt_Transfer_doc %}

The `erc721_opbnb.evt_Transfer` table contains Transfer events for ERC721 tokens on the opbnb blockchain. It includes:
...
This table is used for tracking ERC721 token (NFT) transfers on the opbnb network.
...
{% docs erc721_opbnb_evt_Approval_doc %}

The `erc721_opbnb.evt_Approval` table contains Approval events for ERC721 tokens on the opbnb blockchain. It includes:
...
This table is used for analyzing approvals for individual ERC721 tokens (NFTs) on the opbnb network.

{% enddocs %}

{% docs erc721_opbnb_evt_ApprovalForAll_doc %}

The `erc721_opbnb.evt_ApprovalForAll` table contains ApprovalForAll events for ERC721 tokens on the opbnb blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC721 token collections on the opbnb network.
{% enddocs %}
