{% docs mezo_blocks_doc %}

The `mezo.blocks` table contains information about blocks on the mezo blockchain. It includes:
...
{% docs mezo_transactions_doc %}

The `mezo.transactions` table contains detailed information about transactions on the mezo blockchain. It includes:
...
The `mezo.logs` table contains event logs emitted by smart contracts on the mezo blockchain. It includes:
...
{% docs mezo_traces_doc %}

The `mezo.traces` table contains records of execution steps for transactions on the mezo blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
{% docs mezo_creation_traces_doc %}

The `mezo.creation_traces` table contains data about contract creation events on the mezo blockchain. It includes:
...
{% docs mezo_contracts_doc %}

The `mezo.contracts` table contains information about verified smart contracts on the mezo blockchain. It includes:
...
{% docs mezo_contracts_submitted_doc %}

The `mezo.contracts_submitted` table contains information about manually submitted contract verifications on the mezo blockchain. It includes:
...
{% docs mezo_traces_decoded_doc %}

The `mezo.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs mezo_logs_decoded_doc %}

The `mezo.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs erc20_mezo_evt_Transfer_doc %}

The `erc20_mezo.evt_transfer` table contains Transfer events for ERC20 tokens on the mezo blockchain. It includes:
...
This table is used for tracking ERC20 token movements on the mezo network.
...
{% docs erc20_mezo_evt_Approval_doc %}

The `erc20_mezo.evt_Approval` table contains Approval events for ERC20 tokens on the mezo blockchain. It includes:
...
This table is used for analyzing ERC20 token approvals and spending permissions on the mezo network.

{% enddocs %}

{% docs erc1155_mezo_evt_TransferSingle_doc %}

The `erc1155_mezo.evt_TransferSingle` table contains TransferSingle events for ERC1155 tokens on the mezo blockchain. It includes:
...
This table is used for tracking individual ERC1155 token transfers on the mezo network.
...
{% docs erc1155_mezo_evt_TransferBatch_doc %}

The `erc1155_mezo.evt_TransferBatch` table contains TransferBatch events for ERC1155 tokens on the mezo blockchain. It includes:
...
This table is used for tracking batch transfers of multiple ERC1155 tokens on the mezo network.

Please be aware that this table is the raw ERC1155 event data, and does not include any additional metadata, context or is in any way filtered or curated. Use nft.transfers for a more complete and curated view of NFT transfers.

{% enddocs %}

{% docs erc1155_mezo_evt_ApprovalForAll_doc %}

The `erc1155_mezo.evt_ApprovalForAll` table contains ApprovalForAll events for ERC1155 tokens on the mezo blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC1155 token collections on the mezo network.

{% enddocs %}

{% docs erc721_mezo_evt_Transfer_doc %}

The `erc721_mezo.evt_Transfer` table contains Transfer events for ERC721 tokens on the mezo blockchain. It includes:
...
This table is used for tracking ERC721 token (NFT) transfers on the mezo network.
...
{% docs erc721_mezo_evt_Approval_doc %}

The `erc721_mezo.evt_Approval` table contains Approval events for ERC721 tokens on the mezo blockchain. It includes:
...
This table is used for analyzing approvals for individual ERC721 tokens (NFTs) on the mezo network.

{% enddocs %}

{% docs erc721_mezo_evt_ApprovalForAll_doc %}

The `erc721_mezo.evt_ApprovalForAll` table contains ApprovalForAll events for ERC721 tokens on the mezo blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC721 token collections on the mezo network.
{% enddocs %}
