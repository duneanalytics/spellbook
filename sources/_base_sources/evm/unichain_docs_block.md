...
The `unichain.contracts` table contains information about verified smart contracts on the unichain blockchain. It includes:
...
The `unichain.contracts_submitted` table contains information about manually submitted contract verifications on the unichain blockchain. It includes:
...
The `unichain.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `unichain.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `erc20_unichain.evt_transfer` table contains Transfer events for ERC20 tokens on the unichain blockchain. It includes:
...
This table is used for tracking ERC20 token movements on the unichain network.
...
{% docs erc20_unichain_evt_Approval_doc %}

The `erc20_unichain.evt_Approval` table contains Approval events for ERC20 tokens on the unichain blockchain. It includes:
...
This table is used for analyzing ERC20 token approvals and spending permissions on the unichain network.

{% enddocs %}

{% docs erc1155_unichain_evt_TransferSingle_doc %}

The `erc1155_unichain.evt_TransferSingle` table contains TransferSingle events for ERC1155 tokens on the unichain blockchain. It includes:
...
This table is used for tracking individual ERC1155 token transfers on the unichain network.
...
The `erc1155_unichain.evt_TransferBatch` table contains TransferBatch events for ERC1155 tokens on the unichain blockchain. It includes:
...
This table is used for tracking batch transfers of multiple ERC1155 tokens on the unichain network.

Please be aware that this table is the raw ERC1155 event data, and does not include any additional metadata, context or is in any way filtered or curated. Use nft.transfers for a more complete and curated view of NFT transfers.

{% enddocs %}

{% docs erc1155_unichain_evt_ApprovalForAll_doc %}

The `erc1155_unichain.evt_ApprovalForAll` table contains ApprovalForAll events for ERC1155 tokens on the unichain blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC1155 token collections on the unichain network.

{% enddocs %}

{% docs erc721_unichain_evt_Transfer_doc %}

The `erc721_unichain.evt_Transfer` table contains Transfer events for ERC721 tokens on the unichain blockchain. It includes:
...
This table is used for tracking ERC721 token (NFT) transfers on the unichain network.
...
{% docs erc721_unichain_evt_Approval_doc %}

The `erc721_unichain.evt_Approval` table contains Approval events for ERC721 tokens on the unichain blockchain. It includes:
...
This table is used for analyzing approvals for individual ERC721 tokens (NFTs) on the unichain network.

{% enddocs %}

{% docs erc721_unichain_evt_ApprovalForAll_doc %}

The `erc721_unichain.evt_ApprovalForAll` table contains ApprovalForAll events for ERC721 tokens on the unichain blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC721 token collections on the unichain network.
{% enddocs %}
