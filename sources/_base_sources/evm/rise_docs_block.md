{% docs rise_blocks_doc %}

The `rise.blocks` table contains information about blocks on the rise blockchain. It includes:
...
{% docs rise_transactions_doc %}

The `rise.transactions` table contains detailed information about transactions on the rise blockchain. It includes:
...
The `rise.logs` table contains event logs emitted by smart contracts on the rise blockchain. It includes:
...
{% docs rise_traces_doc %}

The `rise.traces` table contains records of execution steps for transactions on the rise blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
{% docs rise_creation_traces_doc %}

The `rise.creation_traces` table contains data about contract creation events on the rise blockchain. It includes:
...
{% docs rise_contracts_doc %}

The `rise.contracts` table contains information about verified smart contracts on the rise blockchain. It includes:
...
{% docs rise_contracts_submitted_doc %}

The `rise.contracts_submitted` table contains information about manually submitted contract verifications on the rise blockchain. It includes:
...
{% docs rise_traces_decoded_doc %}

The `rise.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs rise_logs_decoded_doc %}

The `rise.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs erc20_rise_evt_Transfer_doc %}

The `erc20_rise.evt_transfer` table contains Transfer events for ERC20 tokens on the rise blockchain. It includes:
...
This table is used for tracking ERC20 token movements on the rise network.
...
{% docs erc20_rise_evt_Approval_doc %}

The `erc20_rise.evt_Approval` table contains Approval events for ERC20 tokens on the rise blockchain. It includes:
...
This table is used for analyzing ERC20 token approvals and spending permissions on the rise network.

{% enddocs %}

{% docs erc1155_rise_evt_TransferSingle_doc %}

The `erc1155_rise.evt_TransferSingle` table contains TransferSingle events for ERC1155 tokens on the rise blockchain. It includes:
...
This table is used for tracking individual ERC1155 token transfers on the rise network.
...
{% docs erc1155_rise_evt_TransferBatch_doc %}

The `erc1155_rise.evt_TransferBatch` table contains TransferBatch events for ERC1155 tokens on the rise blockchain. It includes:
...
This table is used for tracking batch transfers of multiple ERC1155 tokens on the rise network.

Please be aware that this table is the raw ERC1155 event data, and does not include any additional metadata, context or is in any way filtered or curated. Use nft.transfers for a more complete and curated view of NFT transfers.

{% enddocs %}

{% docs erc1155_rise_evt_ApprovalForAll_doc %}

The `erc1155_rise.evt_ApprovalForAll` table contains ApprovalForAll events for ERC1155 tokens on the rise blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC1155 token collections on the rise network.

{% enddocs %}

{% docs erc721_rise_evt_Transfer_doc %}

The `erc721_rise.evt_Transfer` table contains Transfer events for ERC721 tokens on the rise blockchain. It includes:
...
This table is used for tracking ERC721 token (NFT) transfers on the rise network.
...
{% docs erc721_rise_evt_Approval_doc %}

The `erc721_rise.evt_Approval` table contains Approval events for ERC721 tokens on the rise blockchain. It includes:
...
This table is used for analyzing approvals for individual ERC721 tokens (NFTs) on the rise network.

{% enddocs %}

{% docs erc721_rise_evt_ApprovalForAll_doc %}

The `erc721_rise.evt_ApprovalForAll` table contains ApprovalForAll events for ERC721 tokens on the rise blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC721 token collections on the rise network.
{% enddocs %}
