...
The `optimism.blocks` table contains information about blocks on the optimism blockchain. It includes:
...
{% docs optimism_transactions_doc %}
...
The `optimism.traces` table contains records of execution steps for transactions on the optimism blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
The `optimism.contracts` table contains information about verified smart contracts on the optimism blockchain. It includes:
...
{% docs optimism_contracts_submitted_doc %}

The `optimism.contracts_submitted` table contains information about manually submitted contract verifications on the optimism blockchain. It includes:
...
The `optimism.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
The `optimism.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
This table is used for tracking ERC20 token movements on the optimism network.
