{% docs monad_blocks_doc %}

The `monad.blocks` table contains information about blocks on the monad blockchain. It includes:
...
{% docs monad_transactions_doc %}

The `monad.transactions` table contains detailed information about transactions on the monad blockchain. It includes:
...
The `monad.logs` table contains event logs emitted by smart contracts on the monad blockchain. It includes:
...
{% docs monad_traces_doc %}

The `monad.traces` table contains records of execution steps for transactions on the monad blockchain. Each trace represents an atomic operation that modifies the blockchain state. Key components include:
...
{% docs monad_creation_traces_doc %}

The `monad.creation_traces` table contains data about contract creation events on the monad blockchain. It includes:
...
{% docs monad_contracts_doc %}

The `monad.contracts` table contains information about verified smart contracts on the monad blockchain. It includes:
...
{% docs monad_contracts_submitted_doc %}

The `monad.contracts_submitted` table contains information about manually submitted contract verifications on the monad blockchain. It includes:
...
{% docs monad_traces_decoded_doc %}

The `monad.traces_decoded` table contains decoded traces with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs monad_logs_decoded_doc %}

The `monad.logs_decoded` table contains decoded event logs with additional information based on submitted smart contracts and their ABIs. It includes:
...
{% docs erc20_monad_evt_Transfer_doc %}

The `erc20_monad.evt_transfer` table contains Transfer events for ERC20 tokens on the monad blockchain. It includes:
...
This table is used for tracking ERC20 token movements on the monad network.
...
{% docs erc20_monad_evt_Approval_doc %}

The `erc20_monad.evt_Approval` table contains Approval events for ERC20 tokens on the monad blockchain. It includes:
...
This table is used for analyzing ERC20 token approvals and spending permissions on the monad network.

{% enddocs %}

{% docs erc1155_monad_evt_TransferSingle_doc %}

The `erc1155_monad.evt_TransferSingle` table contains TransferSingle events for ERC1155 tokens on the monad blockchain. It includes:
...
This table is used for tracking individual ERC1155 token transfers on the monad network.
...
{% docs erc1155_monad_evt_TransferBatch_doc %}

The `erc1155_monad.evt_TransferBatch` table contains TransferBatch events for ERC1155 tokens on the monad blockchain. It includes:
...
This table is used for tracking batch transfers of multiple ERC1155 tokens on the monad network.

Please be aware that this table is the raw ERC1155 event data, and does not include any additional metadata, context or is in any way filtered or curated. Use nft.transfers for a more complete and curated view of NFT transfers.

{% enddocs %}

{% docs erc1155_monad_evt_ApprovalForAll_doc %}

The `erc1155_monad.evt_ApprovalForAll` table contains ApprovalForAll events for ERC1155 tokens on the monad blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC1155 token collections on the monad network.

{% enddocs %}

{% docs erc721_monad_evt_Transfer_doc %}

The `erc721_monad.evt_Transfer` table contains Transfer events for ERC721 tokens on the monad blockchain. It includes:
...
This table is used for tracking ERC721 token (NFT) transfers on the monad network.
...
{% docs erc721_monad_evt_Approval_doc %}

The `erc721_monad.evt_Approval` table contains Approval events for ERC721 tokens on the monad blockchain. It includes:
...
This table is used for analyzing approvals for individual ERC721 tokens (NFTs) on the monad network.

{% enddocs %}

{% docs erc721_monad_evt_ApprovalForAll_doc %}

The `erc721_monad.evt_ApprovalForAll` table contains ApprovalForAll events for ERC721 tokens on the monad blockchain. It includes:
...
This table is used for analyzing blanket approvals for ERC721 token collections on the monad network.
{% enddocs %}
