{{
  config(
        alias='trades', materialized='table'
  )
}}
...
with iv_availadv as (
    select 'avail' as order_type
          ...
          ,exec:item:amount as amount_original
...
,iv_transfer_level_pre as ( 
    select 'normal' as main_type
          ...
          ,each_offer:amount::bigint as amount_original
    ...
    select 'normal' as main_type
          ...
          ,each_consideration:amount::bigint as amount_original
    ...
    select 'advanced' as main_type
          ...
          ,a.each_consideration:amount::bigint as amount_original
...
,iv_txn_level as (
    select tx_hash
          ...
          ,sum(case when trade_category = 'auction' and sub_idx in (1,2) then amount_original
                    when trade_category = 'offer accepted' and sub_type = 'offer' and sub_idx = 1 then amount_original
                    when trade_category = 'click buy now' and sub_type = 'consideration' then amount_original
              end) as amount_original
          ,max(case when trade_category = 'auction' and sub_idx in (1,2) then token_contract_address
                    when trade_category = 'offer accepted' and sub_type = 'offer' and sub_idx = 1 then token_contract_address
                    when trade_category = 'click buy now' and sub_type = 'consideration' then token_contract_address
              end) as currency_contract_original
          ,case when max(case when trade_category = 'auction' and sub_idx in (1,2) then token_contract_address
                            when trade_category = 'offer accepted' and sub_type = 'offer' and sub_idx = 1 then token_contract_address
                            when trade_category = 'click buy now' and sub_type = 'consideration' then token_contract_address
                      end) = '0x0000000000000000000000000000000000000000'
                then '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
                else max(case when trade_category = 'auction' and sub_idx in (1,2) then token_contract_address
                            when trade_category = 'offer accepted' and sub_type = 'offer' and sub_idx = 1 then token_contract_address
                            when trade_category = 'click buy now' and sub_type = 'consideration' then token_contract_address
                      end)
            end as currency_contract
          ,max(case when trade_category = 'auction' and sub_idx = 2 then receiver
                    when trade_category = 'offer accepted' and sub_type = 'consideration' and item_type = '1' then receiver
                    when trade_category = 'click buy now' and sub_type = 'consideration' and sub_idx = 2 then receiver
              end) as fee_receive_address
          ,sum(case when trade_category = 'auction' and sub_idx = 2 then amount_original
                    when trade_category = 'offer accepted' and sub_type = 'consideration' and item_type = '1' then amount_original
                    when trade_category = 'click buy now' and sub_type = 'consideration' and sub_idx = 2 then amount_original
              end) as fee_amount
          ,max(case when trade_category = 'auction' and sub_idx = 2 then token_contract_address
                    when trade_category = 'offer accepted' and sub_type = 'consideration' and item_type = '1' then token_contract_address
                    when trade_category = 'click buy now' and sub_type = 'consideration' and sub_idx = 2 then token_contract_address
              end) as fee_currency_contract
          ,case when max(case when trade_category = 'auction' and sub_idx = 2 then token_contract_address
                            when trade_category = 'offer accepted' and sub_type = 'consideration' and item_type = '1' then token_contract_address
                            when trade_category = 'click buy now' and sub_type = 'consideration' and sub_idx = 2 then token_contract_address
                      end) = '0x0000000000000000000000000000000000000000'
                then '0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2'
                else max(case when trade_category = 'auction' and sub_idx = 2 then token_contract_address
                            when trade_category = 'offer accepted' and sub_type = 'consideration' and item_type = '1' then token_contract_address
                            when trade_category = 'click buy now' and sub_type = 'consideration' and sub_idx = 2 then token_contract_address
                      end)
          ...
          ,coalesce(sum(case when item_type = '2' then amount_original end),0) as erc721_item_count
          ,coalesce(sum(case when item_type = '3' then amount_original end),0) as erc1155_item_count
          ,coalesce(sum(case when item_type in ('2','3') then amount_original end),0) as nft_item_count
      from (
            select a.*
...
,iv_nft_trades as ( 
    select a.block_time
          ...
          ,'Trade' as evt_type
          ,a.amount_original / power(10, e1.decimals) * p1.price as amount_usd
          ,seller
          ,buyer
          ,a.amount_original / power(10, e1.decimals) as amount_original
          ,a.amount_original as amount_raw
          ,case when a.currency_contract_original = '0x0000000000000000000000000000000000000000' then 'ETH'
                else p1.symbol 
          end as currency_symbol
          ,a.currency_contract_original
          ,a.currency_contract
          ...
          end as fee_currency_symbol
          ,a.fee_amount as fee_amount_raw
          ,a.fee_amount / power(10, e2.decimals) as fee_amount
          ,a.fee_amount / power(10, e2.decimals) * p2.price as fee_amount_usd
          ...
          ,case when spc1.call_tx_hash is not null then 'Collection/Trait Offers' -- include English Auction and Dutch Auction
                ...
                when spc2.call_tx_hash is not null and spc2.parameters:basicOrderType::integer between 16 and 23 and spc2.parameters:considerationIdentifier = a.token_id then 'Individual Offer'
                when spc2.call_tx_hash is not null then 'Buy Now'
                when spc3.call_tx_hash is not null and a.currency_contract_original = '0x0000000000000000000000000000000000000000' then 'Buy Now'
...
select 'ethereum' as blockchain
      ...
      ,collection
      ,amount_usd
      ,amount_usd / peth.price as eth_amount
      ,token_standard
      ,trade_type
      ,number_of_items
      ,trade_category
      ...
      ,fee_currency_symbol
      ,zone_address
      ,tx_hash || '-' || number_of_items::string || '-' || token_id as unique_trade_id
  from iv_nft_trades
  left join {{ ref('nft_ethereum_aggregators') }} agg on agg.contract_address = tx_to
  left join {{ source('prices', 'usd') }} peth on peth.minute = date_trunc('minute', block_time)
