...
with iv_transfer_level as (
    select 'normal' as main_type
          ...
          ,zone
          ,concat('0x',substr(offer2->>'token',3,40)) as token_contract_address
     ...
     where 1=1
       ...
       and zone in ('0xf397619df7bfd4d1657ea9bdd9df7ff888731a11'
    ...
    select 'normal' as main_type 
          ...
          ,recipient as sender 
          ,concat('0x',substr(consideration2->>'recipient',3,40)) as receiver
          ,zone 
          ,concat('0x',substr(consideration2->>'token',3,40)) as token_contract_address 
    ...
    select 'private' as main_type
          ...
          ,e.offerer as sender
          ,concat('0x',substr(a.consideration2->>'recipient',3,40)) as receiver
          ,a.zone
          ,concat('0x',substr(a.consideration2->>'token',3,40)) as token_contract_address
     ...
     from (select *
            ...
            where a.recipient = '0x0000000000000000000000000000000000000000'
              and a.zone in ('0xf397619df7bfd4d1657ea9bdd9df7ff888731a11'
                           ,'0x9b814233894cd227f561b78cc65891aa55c62ad2'
                           ,'0x004c00500000ad104d7dbd00e3ae0a5c00560c00'
                           )
           ) a
          inner join (select *
                        ...
                        where b.recipient = '0x0000000000000000000000000000000000000000'
                          ...
                          and e.zone in ('0xf397619df7bfd4d1657ea9bdd9df7ff888731a11'
                                       ,'0x9b814233894cd227f561b78cc65891aa55c62ad2'
                                       ,'0x004c00500000ad104d7dbd00e3ae0a5c00560c00'
                                       )
)
,iv_txn_level as (
    select tx_hash
          ...
          ,zone
          ,max(case when item_type in ('2','3') then sender::text end) as seller
          ,max(case when item_type in ('2','3') then receiver::text end) as buyer
...
,iv_nft_trades as (
    select a.block_time
          ...
          ,'Trade' as evt_type
          ,a.original_amount / power(10,t1.decimals) * p1.price as usd_amount
          ,seller
          ,buyer
          ,a.original_amount / power(10,t1.decimals) as original_amount
          ...
          ,a.fee_amount / power(10,t2.decimals) as fee_amount
          ,a.fee_amount / power(10,t2.decimals) * p2.price as fee_usd_amount
