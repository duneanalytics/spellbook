...
WITH
    ...
    csp_pool_ids AS (
        ...
        FROM {{ source('balancer_v2_polygon', 'ComposableStablePoolFactory_evt_PoolCreated') }} x --#########################
        INNER JOIN (
            SELECT poolAddress, poolId 
            FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolRegistered') }} --#########################
        ) y
        ON x.contract_address in (
            0xe2fa4e1d17725e72dcdafe943ecf45df4b9e285b   -- CSP v5 factory
            , 0x6ab5549bbd766a43afb687776ad8466f8b42f777 -- CSP v4 factory
        )
    ...
    , sub_pools AS (
        ...
        FROM {{ source('balancer_v2_polygon', 'Vault_evt_TokensRegistered') }} x --#########################
    ...
    , join_exit_call_data AS ( 
        ...
        FROM ( --Start subquery
            SELECT 
                ...
                , y.lp_token_burned
                , y.lp_token_minted
            FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolBalanceChanged') }} x --#########################
    ...
    , swap_data AS (
            ...
            FROM (
                ...
                SELECT 
                    ...
                    , tokenOut
                    , tokens
                FROM {{ source('balancer_v2_polygon', 'Vault_evt_Swap') }} x --#########################
    ...
    , pool_balance_managed AS (
        ...
        FROM (
            ...
            FROM (
                ...
                FROM {{ source('balancer_v2_polygon', 'Vault_evt_PoolBalanceManaged') }} z --#########################
                INNER JOIN (SELECT poolId, tokens FROM sub_pools) sp
