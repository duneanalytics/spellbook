...
WITH
  ...
  base_pools_underlying_tokens_bought as ( -- because when you trade on ellipsis (the underlying exchange event) you can only buy the underlying tokens weirdly, you can't sell them or rather if you do, the contarct actually only sells the lp token so we need underlying boought & sold tables
    ...
    FROM
      ...
      INNER JOIN base_pools b ON a.pool = b.pool -- joining here to get the pool details 
  ),
  ...
  plain_pools as ( -- getting plain pools data 
    ...
    FROM
      (
        ...
        WITH
          ORDINALITY AS coin_elements(token_address, token_id)
        WHERE
          call_success = true
      ) x
    WHERE
      x.pool IS NOT NULL -- some pools are weirdly having a null output_0 so will enter them manually
  ),
  ...
  plain_pools_pool_tokens as (
    ...
    FROM
      plain_pools
    WHERE
      token_address != 0x0000000000000000000000000000000000000000 -- the 3rd & 4th coins are usually 0x000 so filtering here
    UNION ALL
    SELECT
      *,
      'Plain Pool' as pool_type
    ...
    FROM
      (
        ...
        FROM
          ...
          INNER JOIN base_pools_lp_tokens b ON a._base_pool = b.pool
  ...
  meta_pools_underlying_tokens_bought as ( -- getting the underlying token bought, using the base pool pools token table from earlier
    ...
    FROM
      ...
      INNER JOIN base_pools_pool_tokens bt ON mt.base_pool = bt.pool
  ),
  ...
  all_pool_tokens as (
    ...
    SELECT
      ...
      pool_type
    FROM
      base_pools_underlying_tokens_bought
    UNION
    SELECT
      pool,
      token_id,
      ...
      pool_type
    FROM
      base_pools_underlying_tokens_sold
    UNION
    SELECT
      pool,
      token_id,
    ...
    SELECT
      ...
      pool_type
    FROM
      meta_pools_underlying_tokens_bought
    UNION
    SELECT
      pool,
      token_id,
      ...
      pool_type
    FROM
      meta_pools_underlying_tokens_sold
    UNION
    SELECT
      pool,
      token_id,
...
SELECT
  ...
  '2' as version,
  pool,
  CAST(token_id as INT256) as token_id,
  token_address,
  token_type,
  pool_type
FROM
