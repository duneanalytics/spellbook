...
bebop_raw_data AS (
    ...
    FROM
        (SELECT
         ...
         FROM {{ source('bebop_v3_ethereum', 'BebopAggregationContract_evt_AggregateOrderExecuted') }}
         UNION ALL
         SELECT
            evt_index, evt_tx_hash, evt_block_time, ROW_NUMBER() OVER (PARTITION BY evt_tx_hash ORDER BY evt_index) AS row_num
         FROM {{ source('bebop_v4_ethereum', 'BebopSettlement_evt_AggregateOrderExecuted') }}) evt
    LEFT JOIN
        (SELECT
