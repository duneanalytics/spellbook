CREATE OR REPLACE FUNCTION dex.insert_dodoex_bsc(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
WITH rows AS (
    ...
    LEFT JOIN bep20.tokens erc20a ON erc20a.contract_address = dexs.token_a_address
    LEFT JOIN bep20.tokens erc20b ON erc20b.contract_address = dexs.token_b_address
...
VALUES ('*/10 * * * *', $$
    SELECT dex.insert_dodoex_bsc(
        ...
        (SELECT max(number) FROM bsc.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='DODOEX')),
        (SELECT MAX(number) FROM bsc.blocks where time < now() - interval '20 minutes'));
$$)
