CREATE OR REPLACE FUNCTION dex.insert_pancakeswap_v2(start_ts timestamptz, end_ts timestamptz=now(), start_block numeric=0, end_block numeric=9e18) RETURNS integer
...
WITH rows AS (
    ...
    FROM (
        ...
        SELECT
            ...
            t.evt_index
        FROM
            pancakeswap_v2."PancakePair_evt_Swap" t
        INNER JOIN pancakeswap_v2."PancakeFactory_evt_PairCreated" f ON f.pair = t.contract_address
        -- WHERE t.contract_address NOT IN () -- placeholder to remove wash trading pairs
...
VALUES ('17,47 * * * *', $$
    SELECT dex.insert_pancakeswap_v2(
        (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Pancakeswap' AND version = '2'),
        (SELECT now() - interval '20 minutes'),
        (SELECT max(number) FROM bsc.blocks WHERE time < (SELECT max(block_time) - interval '1 days' FROM dex.trades WHERE project='Pancakeswap' AND version = '2')),
