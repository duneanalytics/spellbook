...
WITH markets AS (
    ...
    FROM {{ source('morpho_blue_unichain', 'MorphoBlue_evt_CreateMarket') }}
    )
...
, evt_index
FROM {{ source('morpho_blue_unichain', 'MorphoBlue_evt_Supply') }}
...
, evt_tx_hash AS tx_hash
, evt_index
FROM {{ source('morpho_blue_unichain', 'MorphoBlue_evt_Withdraw') }}
INNER JOIN markets USING (id)
{% if is_incremental() %}
WHERE {{ incremental_predicate('evt_block_time') }}
{% endif %}
...
, evt_tx_hash AS tx_hash
, evt_index
FROM {{ source('morpho_blue_unichain', 'MorphoBlue_evt_Liquidate') }}
INNER JOIN markets USING (id)
{% if is_incremental() %}
WHERE {{ incremental_predicate('evt_block_time') }}
{% endif %}