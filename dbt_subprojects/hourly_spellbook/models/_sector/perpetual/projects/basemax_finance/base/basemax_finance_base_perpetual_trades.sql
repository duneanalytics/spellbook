...
perp_events as (
    ...
    FROM 
    {{ source('basemax_finance_base', 'PositionVault_evt_DecreasePosition') }}
    ...
    SELECT
        ...
        evt_tx_hash as tx_hash 
    FROM 
    {{ source('basemax_finance_base', 'PositionVault_evt_IncreasePosition') }}
    {% if not is_incremental() %}
    WHERE evt_block_time >= DATE '{{project_start_date}}'
    {% else %}
    WHERE {{ incremental_predicate('evt_block_time') }}
    ...
    SELECT
        ...
        evt_tx_hash as tx_hash 
    FROM 
    {{ source('basemax_finance_base', 'LiquidateVault_evt_LiquidatePosition') }}
    {% if not is_incremental() %}
    WHERE evt_block_time >= DATE '{{project_start_date}}'
    {% else %}
    WHERE {{ incremental_predicate('evt_block_time') }}
...
SELECT 
    'base' as blockchain, 
    'basemax_finance' as project, 
    '1' as version, 
    'basemax_finance' as frontend,
