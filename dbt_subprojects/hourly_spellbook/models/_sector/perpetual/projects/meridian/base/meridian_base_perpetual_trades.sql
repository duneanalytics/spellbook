...
{%
set project_start_date = '2023-08-13' %}
...
SELECT evt_block_time,
       ...
       'Open' AS trade_data
FROM {{ source('meridian_trade_base', 'Vault_evt_IncreasePosition') }}
...
SELECT 
    ...
    CASE WHEN isLong = false THEN 'short' ELSE 'long' END as trade_type,
    'Close' AS trade_data
FROM {{ source('meridian_trade_base','Vault_evt_DecreasePosition') }}
    {% if not is_incremental() %}
  WHERE evt_block_time >= DATE '{{project_start_date}}'
    {% else %}
  WHERE {{ incremental_predicate('evt_block_time') }}
...
SELECT 'base'                                    AS blockchain
     ...
     , (CASE
            WHEN trade_type = 'short' AND trade_data = 'Open' THEN 'Open Short'
            WHEN trade_type = 'long' AND trade_data = 'Open' THEN 'Open Long'
            WHEN trade_type = 'short' AND trade_data = 'Close' THEN 'Close Short'
            WHEN trade_type = 'long' AND trade_data = 'Close' THEN 'Close Long'
    END
    )                                                AS trade
     , 'meridian'                                         AS project
     , 'v1'                                          AS version
     , 'meridian'                                         AS frontend
     , account                                       AS trader
