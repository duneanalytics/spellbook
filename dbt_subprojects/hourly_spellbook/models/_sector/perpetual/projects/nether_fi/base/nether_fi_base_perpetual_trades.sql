...
SELECT evt_block_time,
       ...
       isLong AS isLong,
       'Open' AS trade_type
FROM {{ source('nether_fi_base', 'Vault_evt_DecreasePosition') }}
...
SELECT 
    ...
    isLong AS isLong,
    'Close' AS trade_type
FROM {{ source('nether_fi_base','Vault_evt_IncreasePosition') }}
    {% if not is_incremental() %}
  WHERE evt_block_time >= DATE '{{project_start_date}}'
    {% else %}
  WHERE {{ incremental_predicate('evt_block_time') }}
...
SELECT 'base' AS blockchain
     ...
     , CAST(fee / 1e30 AS DOUBLE) AS fee_usd
    ...
    END
     ...
     , 'nether_fi' AS project
     , 'v1' AS version
     , 'nether_fi' AS frontend
     , account AS trader
