...
WITH base_model as (
    SELECT
        ...
        -- Proper EIP-1559 calculation: base_fee + priority_fee for EIP-1559, gas_price for legacy
        ,case 
            when txns.priority_fee_per_gas is not null and txns.priority_fee_per_gas >= 0
            then (cast(blocks.base_fee_per_gas as uint256) * cast(txns.gas_used as uint256)) + 
                 (cast(txns.priority_fee_per_gas as uint256) * cast(txns.gas_used as uint256))
            else cast(gas_price as uint256) * cast(txns.gas_used as uint256)
        end as tx_fee_raw
        ,map_concat(
            map()
            ,case when txns.priority_fee_per_gas is null or txns.priority_fee_per_gas < 0
                then map(array['base_fee'], array[(cast(gas_price as uint256) * cast(txns.gas_used as uint256))])
                         ...
                         array[(cast(blocks.base_fee_per_gas as uint256) * cast(txns.gas_used as uint256))
                                ,(cast(txns.priority_fee_per_gas as uint256) * cast(txns.gas_used as uint256))]
                         )
                end
