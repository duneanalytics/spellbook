...
WITH base_model as (
    SELECT
        ...
        ,txns.to AS tx_to
        ,cast(effective_gas_price as uint256) as gas_price
        ,txns.gas_used as gas_used
        ,cast(effective_gas_price as uint256) * cast(txns.gas_used as uint256) as tx_fee_raw
        ,map_concat(
            map()
            ,map(array['l1_fee','base_fee']
                , array[cast(coalesce(gas_used_for_l1,0) * effective_gas_price as uint256)
                        ,cast((txns.gas_used - coalesce(gas_used_for_l1,0)) * effective_gas_price as uint256)])
