...
-- morph has l1_fee but does not expose l1_gas_used / l1_gas_price / l1_fee_scalar,
-- so we cannot reuse op_stack_gas_fees() directly. Logic mirrors tezos_evm's
-- custom model with l1_fee added to tx_fee_raw and the breakdown.
...
base_model as (
  select
    ...
    cast(coalesce(txns.l1_fee, 0) as uint256)
      + cast(gas_price as uint256) * cast(txns.gas_used as uint256) as tx_fee_raw,
    map_concat(
      map(),
      map(array['l1_fee'], array[cast(coalesce(txns.l1_fee, 0) as uint256)]),
      case
