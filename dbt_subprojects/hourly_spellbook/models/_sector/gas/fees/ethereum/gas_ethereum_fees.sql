...
, base_model as (
    SELECT
        ...
        ,cast(coalesce(blob.blob_base_fee,0) as uint256) * cast(coalesce(blob.blob_gas_used,0) as uint256) +cast(gas_price as uint256) * cast(txns.gas_used as uint256) as tx_fee_raw
        ,map_concat(
            map()
    ...
    LEFT JOIN {{ source(blockchain, 'blobs_submissions') }} blob
        ...
        {% elif target.name == 'ci' %}
        AND (
            blob.block_time >= current_date - interval '1' day
            OR blob.tx_hash in (
                select tx_hash
                from {{ ref('evm_gas_fees') }}
