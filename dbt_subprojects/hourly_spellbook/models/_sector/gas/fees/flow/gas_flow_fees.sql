...
, base_model as (
    SELECT
        ...
        -- Flow-specific calculation: base_fee + priority_fee
        ,cast(base_fee_per_gas as uint256) * cast(txns.gas_used as uint256) + cast(priority_fee_per_gas as uint256) * cast(txns.gas_used as uint256) as tx_fee_raw
        -- Flow-specific breakdown: separate base_fee and priority_fee
        ,map_concat(
            map()
            ,map(array['base_fee'], array[cast(base_fee_per_gas as uint256) * cast(txns.gas_used as uint256)])
            ,map(array['priority_fee'], array[cast(priority_fee_per_gas as uint256) * cast(txns.gas_used as uint256)])
