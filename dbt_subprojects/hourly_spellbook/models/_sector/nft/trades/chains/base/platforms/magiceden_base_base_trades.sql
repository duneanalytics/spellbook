...
WITH trades AS (
    ...
    FROM {{ source('limitbreak_base','PaymentProcessor_evt_BuyListingERC721') }}
    {% if is_incremental() %}
    ...
    , salePrice AS price_raw
    , evt_index AS sub_tx_trade_id
    FROM {{ source('limitbreak_base','PaymentProcessor_evt_BuyListingERC1155') }}
    {% if is_incremental() %}
    WHERE {{incremental_predicate('evt_block_time')}}
    {% else %}
    WHERE evt_block_time >= {{magiceden_start_date}}
    ...
    , salePrice AS price_raw
    , evt_index AS sub_tx_trade_id
    FROM {{ source('limitbreak_base','PaymentProcessor_evt_AcceptOfferERC721') }}
    {% if is_incremental() %}
    WHERE {{incremental_predicate('evt_block_time')}}
    {% else %}
    WHERE evt_block_time >= {{magiceden_start_date}}
    ...
    , salePrice AS price_raw
    , evt_index AS sub_tx_trade_id
    FROM {{ source('limitbreak_base','PaymentProcessor_evt_AcceptOfferERC1155') }}
    {% if is_incremental() %}
    WHERE {{incremental_predicate('evt_block_time')}}
    {% else %}
    WHERE evt_block_time >= {{magiceden_start_date}}
...
, fees AS (
    ...
    FROM {{ source('tokens_base','base_transfers') }} tr
