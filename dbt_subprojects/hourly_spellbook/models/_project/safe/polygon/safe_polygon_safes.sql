...
from (
    select
        ...
        et.tx_hash,
        row_number() over (partition by et."from", try_cast(date_trunc('day', et.block_time) as date) order by et.block_time, et.block_number, et.tx_index, et.trace_address) as rn
    ...
    where et.success = true
        ...
        and et.block_time > TIMESTAMP '2021-03-07' -- for initial query optimisation
        {% endif %}
        {% if is_incremental() %}
