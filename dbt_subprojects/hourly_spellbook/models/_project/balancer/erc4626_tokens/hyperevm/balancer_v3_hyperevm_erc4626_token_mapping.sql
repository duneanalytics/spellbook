...
FROM {{ source('aave_hyperevm', 'StaticATokenLM_evt_Initialized') }} a
JOIN {{ source('aave_hyperevm', 'StaticATokenFactory_evt_StaticTokenCreated') }} b
ON b.staticaToken = a.contract_address
JOIN {{ source('aave_v3_hyperevm', 'VariableDebtToken_evt_Initialized') }} c
ON a.aToken = c.contract_address
JOIN {{ source('tokens', 'erc20') }} t
ON t.blockchain = 'hyperevm'
...
FROM {{ source('aave_hyperevm', 'StataTokenV2_evt_Initialized') }} a
JOIN {{ source('aave_hyperevm', 'StataTokenFactory_evt_StataTokenCreated') }} b
...
SELECT DISTINCT
    ...
    t.symbol AS underlying_token_symbol,
    18 AS decimals
FROM {{ source('metamorpho_factory_hyperevm', 'MetaMorphoFactory_evt_CreateMetaMorpho') }} a
JOIN {{ source('tokens', 'erc20') }} t
ON t.blockchain = 'hyperevm'
