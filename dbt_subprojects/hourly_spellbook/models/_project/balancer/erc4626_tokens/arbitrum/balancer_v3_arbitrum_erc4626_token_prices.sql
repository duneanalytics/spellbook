...
WITH wrap_unwrap AS(
        ...
        FROM {{ source('balancer_v3_arbitrum', 'Vault_evt_Wrap') }}
        {% if is_incremental() %}
        ...
        FROM {{ source('balancer_v3_arbitrum', 'Vault_evt_Unwrap') }}    
        {% if is_incremental() %}
        WHERE {{ incremental_predicate('evt_block_time') }}
        {% endif %}      
    ),
    ...
    FROM wrap_unwrap w
    JOIN {{ref('balancer_v3_arbitrum_erc4626_token_mapping')}} m ON m.erc4626_token = w.wrappedToken
