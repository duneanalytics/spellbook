...
WITH wrap_unwrap AS(
        ...
        FROM {{ source('balancer_v3_monad', 'Vault_evt_Wrap') }}
        {% if is_incremental() %}
        ...
        FROM {{ source('balancer_v3_monad', 'Vault_evt_Unwrap') }}    
        {% if is_incremental() %}
        WHERE evt_block_time >= date_trunc('day', now() - interval '4' day)
        {% endif %}      
    ),
    ...
    FROM wrap_unwrap w
    JOIN {{ref('balancer_v3_monad_erc4626_token_mapping')}} m ON m.erc4626_token = w.wrappedToken
    JOIN {{ source('prices', 'usd') }} p ON m.underlying_token = p.contract_address
    AND p.blockchain = 'monad'
