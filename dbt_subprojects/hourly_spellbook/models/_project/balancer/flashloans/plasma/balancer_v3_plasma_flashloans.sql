...
WITH flashloans AS (
    ...
    , f.contract_address
    FROM {{ source('balancer_v3_plasma','Vault_evt_FlashLoan') }} f
    LEFT JOIN {{ source('tokens_plasma', 'erc20') }} erc20 ON f.token = erc20.contract_address
        {% if is_incremental() %}
...
LEFT JOIN {{ source('prices','usd') }} pu ON pu.blockchain = 'plasma'
