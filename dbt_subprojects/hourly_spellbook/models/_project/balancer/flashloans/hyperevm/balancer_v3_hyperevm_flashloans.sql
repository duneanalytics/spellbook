...
WITH flashloans AS (
    ...
    , f.contract_address
    FROM {{ source('balancer_v3_hyperevm','Vault_evt_FlashLoan') }} f
    LEFT JOIN {{ source('tokens_hyperevm', 'erc20') }} erc20 ON f.token = erc20.contract_address
        {% if is_incremental() %}
        WHERE f.evt_block_time >= date_trunc('day', now() - interval '7' Day)
        {% endif %}
    )

SELECT 'hyperevm' AS blockchain
, 'balancer' AS project
, '3' AS version
...
LEFT JOIN {{ source('prices','usd') }} pu ON pu.blockchain = 'hyperevm'
