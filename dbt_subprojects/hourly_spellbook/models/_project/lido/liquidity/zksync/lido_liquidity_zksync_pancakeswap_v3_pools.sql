...
pools as (
select pool as address, 'zksync' as blockchain, 'pancakeswap' as project, max(cast(fee as double))/10000 as fee
from {{ source('pancakeswap_v3_zksync','PancakeV3Factory_evt_PoolCreated')}}
where token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 or token1 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867
group by 1,2,3
)
...
, swap_events as (
    ...
    from {{ source('pancakeswap_v3_zksync', 'PancakeV3Pool_evt_Swap') }} sw
    left join {{ source('pancakeswap_v3_zksync', 'PancakeV3Factory_evt_PoolCreated') }} cr on sw.contract_address = cr.pool
...
, mint_events as (
    select
        ...
        sum(cast(amount0 as DOUBLE)) as amount0,
        sum(cast(amount1 as DOUBLE)) as amount1
    from {{ source('pancakeswap_v3_zksync', 'PancakeV3Pool_evt_Mint') }} mt
    left join {{ source('pancakeswap_v3_zksync', 'PancakeV3Factory_evt_PoolCreated') }} cr on mt.contract_address = cr.pool
...
, burn_events as (
    ...
    from {{ source('pancakeswap_v3_zksync', 'PancakeV3Pool_evt_Burn') }} bn
    left join {{ source('pancakeswap_v3_zksync', 'PancakeV3Factory_evt_PoolCreated') }} cr on bn.contract_address = cr.pool
  ...
  , collect_events AS (
    SELECT
      ...
      c.evt_tx_hash
    FROM
      {{source('pancakeswap_v3_zksync','PancakeV3Pool_evt_Collect')}} AS c
      LEFT JOIN {{source('pancakeswap_v3_zksync','PancakeV3Factory_evt_PoolCreated')}} AS cr ON c.contract_address = cr.pool
...
select l.pool, pools.blockchain, pools.project, pools.fee, cast(l.time as date) as time,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then token0 else token1 end main_token,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then p0.symbol else p1.symbol end main_token_symbol,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then token1 else token0 end paired_token,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then p1.symbol else p0.symbol end paired_token_symbol,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then amount0/power(10, p0.decimals)  else amount1/power(10, p1.decimals)  end main_token_reserve,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then amount1/power(10, p1.decimals)  else amount0/power(10, p0.decimals)  end paired_token_reserve,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then p0.price else p1.price end as main_token_usd_price,
    case when token0 = 0x703b52F2b28fEbcB60E1372858AF5b18849FE867 then p1.price else p0.price end as paired_token_usd_price,
