...
{% set project_start_date = '2023-11-20' %}
...
select pool AS address,
      ...
      'v2' AS pool_type,
      *
from {{source('zebra_scroll','ZebraV2Factory_evt_PoolCreated')}}
...
, mint_events AS (
 ...
 from {{source('zebra_scroll','ZebraV2Pool_evt_Mint')}} m
 left join {{source('zebra_scroll','ZebraV2Factory_evt_PoolCreated')}} cr on m.contract_address = cr.pool 
 ...
 from {{source('zebra_scroll','ZebraV2Pool_evt_Burn')}} b
 left join {{source('zebra_scroll','ZebraV2Factory_evt_PoolCreated')}} cr on b.contract_address = cr.pool 
