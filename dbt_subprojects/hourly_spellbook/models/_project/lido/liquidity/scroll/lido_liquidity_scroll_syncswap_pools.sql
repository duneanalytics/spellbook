...
select pool AS address,
      ...
      'syncswap' AS project,
      *
from {{source('syncswap_scroll','SyncSwapClassicPoolFactory_evt_PoolCreated')}}
...
, mint_events AS (
 ...
 from {{source('syncswap_scroll','SyncSwapClassicPool_evt_Mint')}} m
 left join {{source('syncswap_scroll','SyncSwapClassicPoolFactory_evt_PoolCreated')}} cr on m.contract_address = cr.pool 
 ...
 from {{source('syncswap_scroll','SyncSwapClassicPool_evt_Burn')}} b
 left join {{source('syncswap_scroll','SyncSwapClassicPoolFactory_evt_PoolCreated')}} cr on b.contract_address = cr.pool 
...
, swap_events as (
 ...
 from {{source('syncswap_scroll','SyncSwapClassicPool_evt_Swap')}} s
 left join {{source('syncswap_scroll','SyncSwapClassicPoolFactory_evt_PoolCreated')}} cr on s.contract_address = cr.pool
...
, swap_events_hourly as (
 select DATE_TRUNC('hour', s.evt_block_time) AS time,
      s.contract_address AS pool,
      sum(case when cr.token0 = 0xf610A9dfB7C89644979b4A0f27063E9e7d7Cda32 then CAST(amount0In AS DOUBLE) + CAST(amount0Out AS DOUBLE)
