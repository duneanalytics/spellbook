...
{% set project_start_date = '2024-06-23' %}

with

pools as (
select poolAddress, tokenA, tokenB, cast(feeAIn as double)/1e16 as fee
from {{source('maverick_v2_base','V2Factory_evt_PoolCreated')}}
where tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452
   or tokenB = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452
)
...
from {{source('maverick_v2_base','V2Factory_evt_PoolCreated')}}
where tokenB = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452
union
select tokenB
from {{source('maverick_v2_base','V2Factory_evt_PoolCreated')}}
where tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452
...
, swap_events as (
    ...
    from {{source('maverick_v2_base','V2Pool_evt_PoolSwap')}} sw
    left join {{source('maverick_v2_base','V2Factory_evt_PoolCreated')}} cr on sw.contract_address = cr.poolAddress
...
from {{source('maverick_v2_base','V2Pool_evt_PoolAddLiquidity')}} a
left join {{source('maverick_v2_base','V2Factory_evt_PoolCreated')}} cr on a.contract_address = cr.poolAddress
...
from {{source('maverick_v2_base','V2Pool_evt_PoolRemoveLiquidity')}} a
left join {{source('maverick_v2_base','V2Factory_evt_PoolCreated')}} cr on a.contract_address = cr.poolAddress
join pools on a.contract_address = pools.poolAddress
 {% if not is_incremental() %}
 WHERE DATE_TRUNC('day', a.evt_block_time) >= DATE '{{ project_start_date }}'
...
, wsteth_traded_hourly as (
 select
        ...
        sw.contract_address as pool,
        cr.tokenA, cr.tokenB,
        sum(case when (cr.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 and json_extract_scalar(params, '$.tokenAIn') = 'true') then amountIn
                 when (cr.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 and json_extract_scalar(params, '$.tokenAIn') = 'false') then amountOut
                 when (cr.tokenA != 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 and json_extract_scalar(params, '$.tokenAIn') = 'true') then amountOut
                 when (cr.tokenA != 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 and json_extract_scalar(params, '$.tokenAIn') = 'false') then amountIn
                 end) as amount
    from {{source('maverick_v2_base','V2Pool_evt_PoolSwap')}} sw
    left join {{source('maverick_v2_base','V2Factory_evt_PoolCreated')}} cr on sw.contract_address = cr.poolAddress
    join pools on sw.contract_address = pools.poolAddress
...
select
        ...
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then o.tokenA else o.tokenB end as main_token,
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then pA.symbol else pB.symbol end as main_token_symbol,
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then o.tokenB else o.tokenA end as paired_token,
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then pB.symbol else pA.symbol end as paired_token_symbol,
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then amountA/power(10,pA.decimals) else amountB/power(10,pB.decimals) end as main_token_reserve,
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then amountB/power(10,pB.decimals) else amountA/power(10,pA.decimals) end as paired_token_reserve,
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then pA.price else pB.price end as main_token_usd_price,
        case when o.tokenA = 0xc1CBa3fCea344f92D9239c08C0568f6F2F0ee452 then pB.price else pA.price end as paired_token_usd_price,
