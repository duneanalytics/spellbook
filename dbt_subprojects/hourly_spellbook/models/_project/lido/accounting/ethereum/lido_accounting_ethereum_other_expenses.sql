...
, other_expenses_txns as (
	...
	where
		...
		and not exists (select 1 from ldo_referral_payments_addr as l where l.address = t.to)
		and not exists (select 1 from dai_referral_payments_addr as d where d.address = t.to)
		and not exists (select 1 from steth_referral_payments_addr as s where s.address = t.to)
		...
		and not exists (select 1 from stonks as s where s.address = t.to)
	...
	where
		...
		and not exists (select 1 from multisigs_list as m where m.address = tr.to)
		and not exists (select 1 from diversifications_addresses as d where d.address = tr.to)
		and tr.type = 'call'
