...
{% set project_start_date = '2022-04-14' %}
...
, operating_expenses_txns as (
	-- ERC20 outflow (from operating multisigs)
	select
		t.evt_block_time
	...
	where
		...
		and exists (
			...
			where
				m.address = t."from"
				and m.name in ('ATCMsig', 'PMLMsig', 'RCCMsig', 'AllianceMsig', 'EcosystemBORGMsig', 'LabsBORGMsig')
	...
	-- ERC20 inflow (to operating multisigs, negative - refunds)
	select
		t.evt_block_time
		, -cast(t.value as double) as value
		, t.evt_tx_hash
	...
	-- ETH inflow (to operating multisigs, negative - refunds)
	select
		tr.block_time as evt_block_time
		, -cast(tr.value as double) as value
		, tr.tx_hash as evt_tx_hash
