...
, lox_incentives_txns as (
	...
	from
		{{ source('erc20_polygon', 'evt_Transfer') }} as t
	where
	...
	-- Ethereum PolygonTeamRewards (to PolygonTeamRewardsMsig, from Aragon)
	select
		t.evt_block_time
		, cast(t.value as double) as value
		, t.evt_tx_hash
	...
	where
		...
		and exists (
			...
			where
				m.address = t.to
				and m.name in ('PolygonTeamRewardsMsig')
	...
	-- Ethereum PolygonTeamRewards (from PolygonTeamRewardsMsig, to Aragon)
	select
		t.evt_block_time
		, -cast(t.value as double) as value
		, t.evt_tx_hash
	...
	-- Solana, Terra Liq Incentives (from multisigs to Jumpgate/Wormhole)
	select
		t.evt_block_time
		, cast(t.value as double) as value
		, t.evt_tx_hash
	...
	where
		...
		and exists (
			...
			where
				m.address = t."from"
				and m.name in ('LiquidityRewardsMsig', 'Aragon', 'ReferralRewardsMsig')
				and m.chain = 'Ethereum'
		)
		and exists (
			select
				1
			from
				intermediate_addresses as i
			where
				i.address = t.to
				and i.name in ('Jumpgate(Solana)', 'Wormhole bridge')
		)
	union all
	select
		t.evt_block_time
	...
	where
		...
		and exists (
			...
			where
				i.address = t.to
				and i.name in ('AnySwap bridge (Polkadot, Kusama)')
		)
	union all
	select
		t.evt_block_time
...
select
	...
	, case
		...
		when m.contract_address = 0xc3c7d422809852031b44ab29eec9f1eff2a58756 then 0x5a98fcbea516cf06857215779fd812ca3bef1b32 --Poly LDO
		when m.contract_address = 0x03b54A6e9a984069379fae1a4fC4dBAE93B3bCCD then 0x7f39C581F595B53c5cb19bD0b3f8dA6c935E2Ca0 --Poly wstETH
		else m.contract_address
	end as token
from
	lox_incentives_txns as m
where
