...
solvers_ranked as (
    ...
    from {{ source('avalanche_c', 'traces') }} t
        inner join {{ ref('cow_protocol_avalanche_c_solvers') }} solvers
...
batch_counts as (
    ...
    from {{ source('gnosis_protocol_v2_avalanche_c', 'GPv2Settlement_evt_Settlement') }} s
        left outer join {{ source('gnosis_protocol_v2_avalanche_c', 'GPv2Settlement_evt_Interaction') }} i
...
batch_values as (
    select
        ...
        price           as eth_price
    from  {{ source('cow_protocol_avalanche_c', 'trades') }}
        left outer join {{ source('prices', 'usd') }} as p
            on p.contract_address = 0xB31f66AA3C1e785363F0875A1B74E27b85FD66c7
            {% if is_incremental() %}
