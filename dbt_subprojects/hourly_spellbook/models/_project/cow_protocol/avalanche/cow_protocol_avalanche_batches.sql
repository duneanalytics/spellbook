{{  config(
        ...
        post_hook='{{ expose_spells(blockchains = \'["avalanche"]\',
                                    spell_type = "project",
...
solvers_ranked as (
    ...
    from {{ source('avalanche', 'traces') }} t
        inner join {{ ref('cow_protocol_avalanche_solvers') }} solvers
...
batch_counts as (
    ...
    from {{ source('gnosis_protocol_v2_avalanche', 'GPv2Settlement_evt_Settlement') }} s
        left outer join {{ source('gnosis_protocol_v2_avalanche', 'GPv2Settlement_evt_Interaction') }} i
...
batch_values as (
    select
        ...
        price           as eth_price
    from  {{ source('cow_protocol_avalanche', 'trades') }}
        left outer join {{ source('prices', 'usd') }} as p
            on p.contract_address = 0x49d5c2bdffac6ce2bfdb6640f4f80f226bc10bab -- WETH contract address
            {% if is_incremental() %}
