...
solvers_ranked as (
    ...
    from {{ source('lens', 'traces') }} t
        inner join {{ ref('cow_protocol_lens_solvers') }} solvers
...
batch_counts as (
    ...
    from {{ source('gnosis_protocol_v2_lens', 'GPv2Settlement_evt_Settlement') }} s
        left outer join {{ source('gnosis_protocol_v2_lens', 'GPv2Settlement_evt_Interaction') }} i
...
batch_values as (
    select
        ...
        price           as eth_price
    from  {{ source('cow_protocol_lens', 'trades') }}
        left outer join {{ source('prices', 'usd') }} as p
            on p.contract_address = 0x6bDc36E20D267Ff0dd6097799f82e78907105e2F
            {% if is_incremental() %}
...
combined_batch_info as (
    ...
    from batch_counts b
        ...
        inner join {{ source('lens', 'transactions') }} tx
            on evt_tx_hash = hash
