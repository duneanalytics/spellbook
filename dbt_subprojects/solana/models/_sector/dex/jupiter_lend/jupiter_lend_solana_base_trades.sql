...
/*
  Label each transfer with exactly one side, derived from the offset bucket that
  admitted it. A transfer whose from_token_account and to_token_account are both
  pool vaults therefore counts once, not once per side.
*/
, transfers_labeled AS (
    SELECT
        ...
        , tf.token_mint_address
        , CASE
            WHEN tf.inner_instruction_index IN (s.inner_instruction_index + 5, s.inner_instruction_index + 6) THEN 'buy'
            WHEN tf.inner_instruction_index IN (s.inner_instruction_index + 2, s.inner_instruction_index + 3) THEN 'sell'
...
, matched AS (
    SELECT
        ...
        , max(CASE WHEN transfer_side = 'buy'  THEN amount END)             AS token_bought_amount_raw
        , max(CASE WHEN transfer_side = 'sell' THEN amount END)             AS token_sold_amount_raw
        , max(CASE WHEN transfer_side = 'buy'  THEN from_token_account END) AS token_bought_vault
        , max(CASE WHEN transfer_side = 'sell' THEN to_token_account END)   AS token_sold_vault
        , max(CASE WHEN transfer_side = 'buy'  THEN token_mint_address END) AS token_bought_mint_address
        , max(CASE WHEN transfer_side = 'sell' THEN token_mint_address END) AS token_sold_mint_address
    ...
    GROUP BY
        ...
        , surrogate_key
    HAVING 1=1
        AND count_if(transfer_side = 'buy')  = 1
        AND count_if(transfer_side = 'sell') = 1
)
