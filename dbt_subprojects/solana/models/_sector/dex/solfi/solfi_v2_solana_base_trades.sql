...
{% set project_start_date = '2025-08-07' %}
...
WITH swaps AS (
    SELECT
        ...
        , pool_id
        , surrogate_key
    FROM {{ ref('solfi_v2_solana_stg_raw_swaps') }}
    WHERE 1=1
        {% if is_incremental() -%}
...
-- Step 3: Attach transfer_side (now this join is only over the pruned set)
...
, transfers AS (
    ...
    GROUP BY
        ...
        , s.tx_index
        , s.surrogate_key
    HAVING COUNT_IF(tf.transfer_side = 1) = 1
...
SELECT
    ...
    , token_sold_vault
    , project_program_id
    , 'SV2EYYJyRz2YhfXwXnhNAevDEui5Q6yrfyo13WtupPF' AS project_main_id
    , trader_id
    , tx_id
    , outer_instruction_index
    , inner_instruction_index
