...
, swaps_with_transfers AS (
    SELECT
        ...
        t.mint_address as token_bought_mint_address,
        -- Get mint addresses directly from transfers table
        (SELECT DISTINCT mint_address FROM {{ ref('tokens_solana_transfers') }} 
         WHERE token_account = s.account_poolTokenAccountA LIMIT 1) as mint_a,
        (SELECT DISTINCT mint_address FROM {{ ref('tokens_solana_transfers') }} 
         WHERE token_account = s.account_poolTokenAccountB LIMIT 1) as mint_b
    FROM solfi_swaps s
