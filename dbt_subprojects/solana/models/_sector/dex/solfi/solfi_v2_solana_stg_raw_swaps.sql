...
-- solfi v2 swap data from instruction_calls table with filter for program and discriminator
WITH swaps AS (
  SELECT
    block_slot
  ...
  WHERE
    1=1
    AND executing_account = 'SV2EYYJyRz2YhfXwXnhNAevDEui5Q6yrfyo13WtupPF'
    AND BYTEARRAY_SUBSTRING(data, 1, 1) = 0x07
    AND tx_success = true
    {% if is_incremental() -%}
