...
{% set project_start_date = '2024-01-20' %}
{% set fee_receiver = "'9RYJ3qr5eU5xAooqVcbmdeusjcViL5Nkiq7Gske3tiKq', 'F4hJ3Ee3c5UuaorKAMfELBjYCjiiLH75haZTKqTywRP3'" %} -- Main fee wallet changed 11/16/2024
...
WITH
  feePayments AS (
    ...
    WHERE
      ...
      AND (
        (
          address IN ({{fee_receiver}})
          AND balance_change > 0 -- SOL fee payments
        )
        OR (
          token_balance_owner IN ({{fee_receiver}})
  ...
  botTrades AS (
    ...
    WHERE
      trades.trader_id NOT IN ({{fee_receiver}}) -- Exclude trades signed by FeeWallet
      AND transactions.signer NOT IN ({{fee_receiver}}) -- Exclude trades signed by FeeWallet
      {% if is_incremental() %}
