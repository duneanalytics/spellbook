...
{% set batch_end = model.batch.event_time_end if model.batch else '2099-12-31' %}
...
with swaps as (
	...
	from
		{{ source('meteora_pools_solana', 'amm_call_swap') }} as sp
	left join
		{{ source('meteora_vault_solana', 'vault_call_deposit') }} as dp
