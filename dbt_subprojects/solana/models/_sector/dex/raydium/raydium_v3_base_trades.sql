...
, all_swaps as (
    ...
    INNER JOIN {{ ref('tokens_solana_transfers') }} tr_1
        ON tr_1.tx_id = sp.call_tx_id --once transfers is rebuilt, update to use tx_index
    ...
    INNER JOIN {{ ref('tokens_solana_transfers') }} tr_2
        ON tr_2.tx_id = sp.call_tx_id --once transfers is rebuilt, update to use tx_index
