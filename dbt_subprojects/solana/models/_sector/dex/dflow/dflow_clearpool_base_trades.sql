-- forked from https://github.com/duneanalytics/spellbook/blob/main/dbt_subprojects/solana/models/_sector/dex/orca_whirlpool/orca_whirlpool_base_trades.sql
-- clearpool is whirlpool fork
{{
  config(

        schema = 'dflow_clearpool',
        alias = 'base_trades',
...
{% set project_start_date = '2024-10-04' %} -- min block time from dflow_clearpool_solana.whirlpool_call_swap
...
with
    ...
    with
        fee_tiers_defaults as (
            ...
            FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_initializeFeeTier') }}
            ...
            , defaultfeeRate as fee_rate
            , call_block_time as fee_time
            FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_setDefaultFeeRate') }}
        )

        --https://docs.orca.so/reference/trading-fees, should track protocol fees too. and rewards.
        -- clearpool is orca whirlpool fork 
        , fee_updates as (
            SELECT
                whirlpool_id
                , update_time
            ...
            FROM (
                ...
                FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_initializePool') }} fi
            ...
            FROM (
                ...
                SELECT
                    ...
                    , ftd.fee_rate
                    , row_number() over (partition by fi.account_whirlpool order by ftd.fee_time desc) as recent_update
                FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_initializePoolV2') }} fi
                LEFT JOIN fee_tiers_defaults ftd ON ftd.fee_tier = account_feeTier AND ftd.fee_time <= fi.call_block_time
            )
            WHERE recent_update = 1
            ...
            SELECT
                ...
                , feeRate as fee_rate
            FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_setFeeRate') }}
        )
    ...
    FROM (
        ...
        , tickSpacing
        , call_tx_id
        FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_initializePool') }}
        ...
        , tickSpacing
        , call_tx_id
        FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_initializePoolV2') }}
        ) ip
    ...
    , two_hop as (
        ...
        FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_twoHopSwap') }} sp
    ...
    , all_swaps as (
        SELECT
            ...
            , case when sp.call_outer_executing_account = 'C1ear1po7kcLBZiiArGMXPhGnjRZ8KxkqQ8EEskzHWmc' then 'direct'
        ...
        FROM (
                SELECT
                    ...
                    , call_block_slot
                    , call_outer_executing_account
                FROM {{ source('dflow_clearpool_solana', 'whirlpool_call_swap') }}
                WHERE 1=1
                {% if is_incremental() %}
...
SELECT
    ...
    , 'C1ear1po7kcLBZiiArGMXPhGnjRZ8KxkqQ8EEskzHWmc' as project_main_id
    , tb.trader_id
    , tb.tx_id
