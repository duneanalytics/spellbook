...
WITH swaps AS (
    SELECT
        ...
        , pool_id
        , surrogate_key
    FROM {{ ref('alphaq_solana_stg_raw_swaps') }}
    WHERE 1=1
        {% if is_incremental() -%}
...
-- Step 1: Create a smaller set of just the transfer keys we need to look up
, swap_transfer_keys AS (
    SELECT DISTINCT
        ...
        , transfer_side
    FROM (
        -- Token bought transfer (inner_instruction_index + 2)
        SELECT
              tx_id
            , block_date
            , block_slot
            ...
            , 1 AS transfer_side  -- 1 = bought
        FROM swaps

        UNION ALL

        -- Token sold transfer (inner_instruction_index + 1)
        SELECT
              tx_id
            , block_date
            , block_slot
            ...
            , 2 AS transfer_side  -- 2 = sold
        FROM swaps
    )
)

-- Step 2: Filter transfers using EXISTS (semi-join) - much more efficient than direct join
, transfers_pruned AS (
    SELECT
          tf.tx_id
        , tf.block_date
    ...
    WHERE 1=1
        ...
        -- Partition pruning on transfers
        {% if is_incremental() -%}
...
-- Step 3: Attach transfer_side to the pruned transfers
...
-- Step 4: Join back to swaps and aggregate
, transfers AS (
    SELECT
          s.block_date
        , s.block_time
