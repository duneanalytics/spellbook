...
WITH
    all_swaps as (
        ...
        FROM 
            {{ source('goosefx_solana', 'gfx_ssl_v2_call_swap') }} sp
        INNER JOIN 
            ...
            AND trs_1.block_time < TIMESTAMP '{{cutoff_date}}'
        INNER JOIN 
            ...
            AND trs_2.block_time < TIMESTAMP '{{cutoff_date}}'
        WHERE 
            sp.call_block_time >= TIMESTAMP '{{project_start_date}}'
            AND sp.call_block_time < TIMESTAMP '{{cutoff_date}}'
)
SELECT
    tb.blockchain
    , tb.project 
