  ...
  WITH
    all_swaps as (
        ...
        FROM (
            ...
            FROM {{ source('gamma_solana', 'gamma_call_swapBaseOutput') }}
            UNION ALL
            SELECT account_poolState, call_is_inner, call_outer_instruction_index, call_inner_instruction_index, call_tx_id, call_block_time, call_block_slot, call_outer_executing_account, call_tx_signer, call_tx_index
            FROM {{ source('gamma_solana', 'gamma_call_swapBaseInput') }}
        ) sp
        ...
        and tk_2.token_balance_owner = 'ALfS4oPB5684XwTvCjWw7XddFfmyTNdcY7xHxbh2Ui8s' --gamma authority
        {% if is_incremental() %}
...
SELECT
    ...
    , 'GAMMA7meSFWaBXF25oSUgmGRwaW6sCMFLmBNiMSdbHVT' as project_main_id
    , tb.trader_id
    , tb.tx_id
