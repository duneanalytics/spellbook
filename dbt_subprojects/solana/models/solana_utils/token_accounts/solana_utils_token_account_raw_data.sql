...
select
  ...
  , block_slot
  , tx_index
  , COALESCE(inner_instruction_index, -1) as inner_instruction_index --8bil+ rows with null, need to coalesce to avoid nulls in merge statement
  , COALESCE(outer_instruction_index, -1) as outer_instruction_index --0 rows with null, but keeping for clarity
  , token_account
