...
WITH state_calculation AS (
  SELECT
    ...
    instruction_uniq_id,
    event_time as block_time, -- Renaming for consistency with SPL token model
    event_type,
    ...
    LEAD(event_time, 1) OVER (PARTITION BY token_account ORDER BY instruction_uniq_id ASC) AS next_block_time,
  ...
  FROM
    {% if is_incremental() %}
      ...
      (
          SELECT raw.*
          FROM {{ ref('alt_solana_utils_token_2022_account_raw_data') }} raw
          INNER JOIN (
              -- Subquery identifies affected accounts using pre-calculated max values
              SELECT DISTINCT src.token_account
              FROM {{ ref('alt_solana_utils_token_2022_account_raw_data') }} src
              WHERE
    ...
    {% else %}
      {{ ref('alt_solana_utils_token_2022_account_raw_data') }}
    {% endif %}
)
