...
, amounts as (
    select
        ...
        , tx_hash
        , call_trace_address
        , max_by(cast(row(transfer_to, transfer_contract_address, transfer_symbol, transfer_amount, transfer_amount_usd, transfer_decimals, trusted) as row(receiver varbinary, address varbinary, symbol varchar, amount uint256, amount_usd double, decimals bigint, trusted boolean)), transfer_amount) filter(where token in same and call_from not in (transfer_from, transfer_to)) as transfered -- trying to find out what actually transfered
    from calls
...
select
    ...
    , tx_gas_used * gas_price * native_price / pow(10, native_decimals) as execution_cost
    , complement
    , remains
    , minute
    , block_date
