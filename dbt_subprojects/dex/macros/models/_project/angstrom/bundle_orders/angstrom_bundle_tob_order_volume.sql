...
WITH 
    ...
    SELECT 
        ...
        asts.price_1over0
    FROM (
        {% if tob_orders_decoding_table is not none %}
        SELECT * FROM {{ tob_orders_decoding_table }}
        {% else %}
        {{ angstrom_decoding_top_of_block_orders(angstrom_contract_addr, earliest_block, blockchain) }}
        {% endif %}
    ) AS ab
    INNER JOIN (
        {{
