...
{%- set table_prefix = 'zeroex_' + blockchain -%}
    ...
    LEFT JOIN zeroex_tx ON zeroex_tx.tx_hash = fills.evt_tx_hash and fills.evt_block_time = zeroex_tx.block_time 
    {% if is_incremental() %}
...
{% macro ERC20BridgeTransfer(blockchain,zeroex_v3_start_date) %}
    ...
    JOIN zeroex_tx ON zeroex_tx.tx_hash = logs.tx_hash and logs.block_time = zeroex_tx.block_time 
...
{%- set table_prefix = 'zeroex_' + blockchain -%}
    ...
    JOIN zeroex_tx ON zeroex_tx.tx_hash = plp.evt_tx_hash and plp.evt_block_time = zeroex_tx.block_time 
...
{%- set table_prefix = 'uniswap_v2_' + blockchain -%}
    ...
    LEFT JOIN {{ source(table_prefix, 'Factory_evt_PairCreated') }} pair ON pair.pair = swap.contract_address
...
{%- set table_prefix = 'uniswap_v3' + blockchain -%}
    SELECT
            ...
            is_gasless 
    FROM {{ source(table_prefix, 'Pair_evt_Swap') }} swap
   LEFT JOIN {{ source(table_prefix, 'Factory_evt_PoolCreated') }} pair ON pair.pool = swap.contract_address
   JOIN zeroex_tx ON zeroex_tx.tx_hash = swap.evt_tx_hash and swap.evt_block_time = zeroex_tx.block_time 
   WHERE sender = 0xdef1c0ded9bec7f1a1670819833240f027b25eff
...
FROM all_tx
INNER JOIN {{ source('{{ blockchain }}' , 'transactions')}} tx ON all_tx.tx_hash = tx.hash and all_tx.block_time = tx.block_time 
