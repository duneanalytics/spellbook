...
{%- if blockchain is none or blockchain == '' -%}
    {{ exceptions.raise_compiler_error("blockchain parameter cannot be null or empty for dex_token_volumes_daily macro") }}
{%- endif -%}

WITH dex_trades_filtered AS (
  SELECT *
  FROM {{ ref('dex_' ~ blockchain ~ '_trades') }} t
  {% if dev_dates -%}
