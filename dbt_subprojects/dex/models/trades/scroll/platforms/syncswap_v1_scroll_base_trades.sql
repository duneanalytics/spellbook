...
{% set syncswap_v1_start_date = "2023-10-16" %}
...
WITH 
    ...
    pools AS (
        ...
        UNION ALL 
        SELECT pool, token0, token1
        FROM {{ source('syncswap_scroll', 'SyncSwapStablePoolFactory_evt_PoolCreated') }}
    )
    
    , base AS (
        SELECT * FROM {{ source('syncswap_scroll', 'SyncSwapStablePool_evt_Swap') }}
        {% if is_incremental() %}
        ...
        UNION
        SELECT * FROM {{ source('syncswap_scroll', 'SyncSwapClassicPool_evt_Swap') }}
        {% if is_incremental() %}
        WHERE {{incremental_predicate('evt_block_time')}}
        {% else %}
        WHERE evt_block_time >= timestamp '{{syncswap_v1_start_date}}'
