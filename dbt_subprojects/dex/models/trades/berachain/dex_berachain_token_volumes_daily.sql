...
WITH trusted_tokens AS (
  ...
  WHERE blockchain = 'berachain'
),

dex_trades_filtered AS (
  SELECT *
  FROM {{ ref('dex_berachain_trades') }} t
...
daily_flows AS (
    ...
    SELECT  
      ...
      , CAST(0 AS double) AS bought_volume_raw
      , CAST(0 AS double) AS bought_volume
      , CAST(0 AS double) AS bought_volume_usd
