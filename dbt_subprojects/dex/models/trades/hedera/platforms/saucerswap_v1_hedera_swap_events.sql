...
-- Dune's factory-flag auto-decoding can't discover SaucerSwap V1 pair contracts.
-- We decode the Swap event from raw hedera.logs ourselves, restricted to pair
-- addresses present in the factory's PairCreated event.
...
with pairs as (
  select f.pair
  from {{ source('saucer_swap_hedera', 'v1_factory_evt_paircreated') }} as f
),
