...
-- Hedera's HTS-based pool deployment doesn't surface in creation_traces, so
-- Dune's factory-flag auto-decoding can't discover SaucerSwap V2 pool contracts.
-- We decode the Swap event from raw hedera.logs ourselves, restricted to pool
-- addresses present in the factory's PoolCreated event.
...
with pools as (
  select f.pool
  from {{ source('saucer_swap_hedera', 'saucerswapv2factory_evt_poolcreated') }} as f
),
