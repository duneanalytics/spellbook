...
WITH token_swaps AS (
    SELECT
        ...
        evt_tx_to AS taker,
        _fromTokenAmount AS token_sold_amount_raw,
        _toTokenAmountOut AS token_bought_amount_raw,
        _fromToken AS token_sold_address,
        _toToken AS token_bought_address,
        contract_address AS project_contract_address,
        evt_tx_hash AS tx_hash,
        evt_index AS evt_index
    FROM
        {{ source('xy_finance_arbitrum', 'xswapper_evt_swappedforuser') }}
    {% if is_incremental() %}
    WHERE
