...
WITH evt_swap AS (
    SELECT
        ...
        , tx_to
        , tx_index
    FROM {{ ref('uniswap_v2_forks_' + blockchain + '_decoded_swap_events') }}
    {% if is_incremental() %}
...
, factory_event_counts AS (
    ...
    FROM {{ ref('uniswap_v2_forks_' + blockchain + '_decoded_factory_events') }}
...
(
    ...
    FROM
        evt_swap t
    INNER JOIN
        {{ ref('uniswap_v2_forks_' + blockchain + '_decoded_factory_events') }} f
