...
exchange_evt_all as (
    select
        ...
        t.evt_index
    from {{ source('ellipsis_finance_bnb', 'StableSwap_evt_TokenExchange') }} t
    {% if is_incremental() %}
...
exchange_und_evt_all as (
    select
        ...
        t.evt_tx_hash as tx_hash,
        t.evt_index
    from {{ source('ellipsis_finance_bnb', 'StableSwap_evt_TokenExchangeUnderlying') }} t
    {% if is_incremental() %}
    where {{ incremental_predicate('t.evt_block_time') }}
    {% endif %}
),
...
dexs as (
    ...
    inner join {{ ref('ellipsis_finance_bnb_pool_tokens') }} pt_bought
    ...
    inner join {{ ref('ellipsis_finance_bnb_pool_tokens') }} pt_sold
