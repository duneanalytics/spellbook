...
WITH dexs AS (
    SELECT
        ...
        token0_decimals.decimals as token0_decimals
    FROM {{ source('somnex_somnia', 'somnexammpair_evt_swap') }} t
    INNER JOIN {{ source('somnex_somnia', 'somnexammfactory_evt_paircreated') }} f
        ON f.pair = t.contract_address
    LEFT JOIN {{ source('tokens', 'erc20') }} token0_decimals
        ON token0_decimals.contract_address = f.token0 
        AND token0_decimals.blockchain = 'somnia'
    {% if is_incremental() %}
...
SELECT
    ...
    dexs.block_time,
    dexs.block_number,
    -- Somnex bug: when token0=18 decimals (or unknown), amount1 inflated by 1e10
    --             when token0=6 decimals, amount0 inflated by 1e10
    CASE 
        WHEN amount0Out = UINT256 '0' AND (token0_decimals IS NULL OR token0_decimals = 18)
            THEN amount1Out / UINT256 '10000000000'
        WHEN amount0Out = UINT256 '0' 
            THEN amount1Out
        WHEN token0_decimals = 6
            THEN amount0Out / UINT256 '10000000000'
        ELSE amount0Out 
    END AS token_bought_amount_raw,
    CASE 
        WHEN (amount0In = UINT256 '0' OR amount1Out = UINT256 '0') AND (token0_decimals IS NULL OR token0_decimals = 18)
