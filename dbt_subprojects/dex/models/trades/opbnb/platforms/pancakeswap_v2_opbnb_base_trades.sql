...
dexs AS (
    ...
    SELECT
        ...
        t.evt_index
    FROM {{ source('pancakeswap_v2_opbnb', 'PancakeSwapMMPool_evt_Swap') }} t
    {% if is_incremental() %}
    ...
    FROM
        (
            SELECT * FROM {{ source('pancakeswap_v2_opbnb', 'PancakeStableSwap_evt_TokenExchange') }}
            UNION ALL
            SELECT * FROM {{ source('pancakeswap_v2_opbnb', 'PancakeStableSwapTwoPool_evt_TokenExchange') }}   
        ) t
    INNER JOIN (
            SELECT a.*
            FROM {{ source('pancakeswap_v2_opbnb', 'PancakeStableSwapFactory_evt_NewStableSwapPair') }} a
            INNER JOIN (
              SELECT swapContract, MAX(evt_block_time) AS latest_time
              FROM {{ source('pancakeswap_v2_opbnb', 'PancakeStableSwapFactory_evt_NewStableSwapPair') }}
