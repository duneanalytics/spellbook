...
joins AS (
    ...
    FROM {{ source('erc20_bnb', 'evt_Transfer') }} e
...
calendar AS (
    ...
    FROM unnest(sequence(date('2020-08-29'), date(now()), interval '1' day)) AS t(date_sequence)
),
...
LEFT JOIN {{ ref('tokens_v1_erc20') }} t ON t.contract_address = b.token_address AND t.blockchain = 'bnb'
