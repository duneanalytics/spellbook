...
bebop_jam_raw_data AS (
    ...
    FROM
        (SELECT
         ...
         FROM {{ source('bebop_jam_polygon', 'JamSettlement_evt_Settlement') }}
         UNION ALL
         SELECT
            evt_index, evt_tx_hash, evt_block_time, ROW_NUMBER() OVER (PARTITION BY evt_tx_hash ORDER BY evt_index) AS row_num
         FROM {{ source('jam_v2_polygon', 'JamSettlement_evt_BebopJamOrderFilled') }}) evt
    LEFT JOIN
        (SELECT
        ...
        FROM {{ source('bebop_jam_polygon', 'JamSettlement_call_settle') }}
        UNION ALL
        SELECT
            call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order",
            ROW_NUMBER() OVER (PARTITION BY call_tx_hash ORDER BY call_block_number) AS row_num
        FROM {{ source('bebop_jam_polygon', 'JamSettlement_call_settleWithPermitsSignatures') }}
        UNION ALL
        SELECT
            call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order",
            ROW_NUMBER() OVER (PARTITION BY call_tx_hash ORDER BY call_block_number) AS row_num
        FROM {{ source('jam_v2_polygon', 'JamSettlement_call_settle') }}
        ) ex
