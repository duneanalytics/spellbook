...
bebop_raw_data AS (
    ...
    FROM
        (
         ...
         FROM {{ source('bebop_v4_optimism', 'BebopSettlement_evt_AggregateOrderExecuted') }}) evt
    LEFT JOIN
        (
        SELECT
        ...
        FROM {{ source('bebop_v4_optimism', 'BebopSettlement_call_SettleAggregateOrder') }}
        UNION ALL
        SELECT
            call_success, call_block_time, call_block_number, call_tx_hash, contract_address, "order",
            ROW_NUMBER() OVER (PARTITION BY call_tx_hash ORDER BY call_block_number) AS row_num
        FROM {{ source('bebop_v4_optimism', 'BebopSettlement_call_SettleAggregateOrderWithTakerPermits') }}
        ) ex
