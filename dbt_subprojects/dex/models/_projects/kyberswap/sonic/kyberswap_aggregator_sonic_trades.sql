...
(
    ...
    schema = 'kyberswap_aggregator_sonic',
    alias = 'trades',
...
{% set project_start_date = '2024-12-17' %}
...
(
        ...
        FROM
            {{ source('kyber_sonic', 'MetaAggregationRouterV2_evt_Swapped') }}
        {% if is_incremental() %}
...
SELECT
    ...
    ,COALESCE(
        ...
        ,(meta_router.token_bought_amount_raw / power(10, (CASE meta_router.token_bought_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 18 ELSE p_bought.decimals END))) * (CASE meta_router.token_bought_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN  p_s.price ELSE p_bought.price END)
        ,(meta_router.token_sold_amount_raw / power(10, (CASE meta_router.token_sold_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 18 ELSE p_sold.decimals END))) * (CASE meta_router.token_sold_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN  p_s.price ELSE p_sold.price END)
        )                                                               AS amount_usd
...
FROM meta_router
INNER JOIN {{ source('sonic', 'transactions')}} tx
...
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ...
    AND p_sold.blockchain = 'sonic'
    {% if is_incremental() %}
    AND {{incremental_predicate('p_sold.minute')}}
    {% else %}
    AND p_sold.minute >= TIMESTAMP '{{ project_start_date }}'
    {% endif %}
LEFT JOIN {{ source('prices', 'usd') }} p_s
    ON p_s.minute = date_trunc('minute', meta_router.block_time)
    AND p_s.blockchain IS NULL
    AND p_s.symbol = 'S'
    {% if is_incremental() %}
    AND {{incremental_predicate('p_s.minute')}}
    {% else %}
    AND p_s.minute >= TIMESTAMP '{{ project_start_date }}'
    {% endif %}
