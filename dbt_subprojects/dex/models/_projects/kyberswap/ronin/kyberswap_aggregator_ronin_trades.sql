...
(
    ...
    schema = 'kyberswap_aggregator_ronin',
    alias = 'trades',
...
(
        ...
        FROM
            {{ source('kyber_ronin', 'MetaAggregationRouterV2_evt_Swapped') }}
        {% if is_incremental() %}
...
SELECT
    ...
    ,COALESCE(
        ...
        ,(meta_router.token_bought_amount_raw / power(10, (CASE meta_router.token_bought_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 18 ELSE p_bought.decimals END))) * (CASE meta_router.token_bought_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN  p_ron.price ELSE p_bought.price END)
        ,(meta_router.token_sold_amount_raw / power(10, (CASE meta_router.token_sold_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 18 ELSE p_sold.decimals END))) * (CASE meta_router.token_sold_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN  p_ron.price ELSE p_sold.price END)
        )                                                               AS amount_usd
...
FROM meta_router
INNER JOIN {{ source('ronin', 'transactions')}} tx
...
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ...
    AND p_sold.blockchain = 'ronin'
    {% if is_incremental() %}
    AND {{incremental_predicate('p_sold.minute')}}
    {% else %}
    AND p_sold.minute >= TIMESTAMP '{{ project_start_date }}'
    {% endif %}
LEFT JOIN {{ source('prices', 'usd') }} p_ron
    ON p_ron.minute = date_trunc('minute', meta_router.block_time)
    AND p_ron.blockchain IS NULL
    AND p_ron.symbol = 'RON'
    {% if is_incremental() %}
    AND {{incremental_predicate('p_ron.minute')}}
    {% else %}
    AND p_ron.minute >= TIMESTAMP '{{ project_start_date }}'
    {% endif %}
