...
(
    ...
    schema = 'kyberswap_aggregator_mantle',
    alias = 'trades',
...
{% set project_start_date = '2024-03-17' %}
...
(
        ...
        FROM
            {{ source('kyber_mantle', 'MetaAggregationRouterV2_evt_Swapped') }}
        {% if is_incremental() %}
...
SELECT
    ...
    ,COALESCE(
        ...
        ,(meta_router.token_bought_amount_raw / power(10, (CASE meta_router.token_bought_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 18 ELSE p_bought.decimals END))) * (CASE meta_router.token_bought_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN  p_mnt.price ELSE p_bought.price END)
        ,(meta_router.token_sold_amount_raw / power(10, (CASE meta_router.token_sold_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 18 ELSE p_sold.decimals END))) * (CASE meta_router.token_sold_address WHEN 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN  p_mnt.price ELSE p_sold.price END)
        )                                                               AS amount_usd
...
FROM meta_router
INNER JOIN {{ source('mantle', 'transactions')}} tx
...
LEFT JOIN {{ source('prices', 'usd') }} p_bought
    ...
    AND p_bought.blockchain = 'mantle'
    {% if is_incremental() %}
...
LEFT JOIN {{ source('prices', 'usd') }} p_sold
    ...
    AND p_sold.blockchain = 'mantle'
    {% if is_incremental() %}
    AND {{incremental_predicate('p_sold.minute')}}
    {% else %}
    AND p_sold.minute >= TIMESTAMP '{{ project_start_date }}'
    {% endif %}
LEFT JOIN {{ source('prices', 'usd') }} p_mnt
    ON p_mnt.minute = date_trunc('minute', meta_router.block_time)
    AND p_mnt.blockchain IS NULL
    AND p_mnt.symbol = 'MNT'
    {% if is_incremental() %}
    AND {{incremental_predicate('p_mnt.minute')}}
    {% else %}
    AND p_mnt.minute >= TIMESTAMP '{{ project_start_date }}'
    {% endif %}
