...
WITH
    dexs_base AS (
        SELECT
            ...
            pool_symbol,
            pool_type
        FROM {{ ref('balancer_v2_polygon_base_trades') }}
    ),
    dexs AS (
        SELECT
            dexs.blockchain,
            ...
            dexs_base.pool_type
        FROM {{ ref('dex_polygon_trades') }} dexs
    ...
    bpa AS (
        ...
        FROM dexs
            LEFT JOIN {{ source('balancer_v2_polygon', 'bpt_prices') }} bpt_prices
...
FROM dexs
    ...
    LEFT JOIN {{ source('balancer_v2_polygon', 'bpt_prices') }} bpa_bpt_prices
    ...
    LEFT JOIN {{ source('balancer_v2_polygon', 'bpt_prices') }} bpb_bpt_prices
