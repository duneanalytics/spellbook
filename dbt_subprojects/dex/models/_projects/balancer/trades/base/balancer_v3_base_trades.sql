...
WITH
    dexs_base AS (
        SELECT
            ...
            pool_symbol,
            pool_type
        FROM {{ ref('balancer_v3_base_base_trades') }}
    ),
    ...
    bpa AS (
        ...
        FROM dexs
            LEFT JOIN {{ source('balancer', 'bpt_prices') }} bpt_prices
                ...
                AND bpt_prices.blockchain = 'base'                
        GROUP BY 1, 2, 3, 4, 5
    ),
...
FROM dexs
    ...
    LEFT JOIN {{ source('balancer', 'bpt_prices') }} bpa_bpt_prices
        ...
        AND bpa_bpt_prices.blockchain = 'base'        
    INNER JOIN bpb
    ...
    LEFT JOIN {{ source('balancer', 'bpt_prices') }} bpb_bpt_prices
        ...
        AND bpb_bpt_prices.blockchain = 'base'
