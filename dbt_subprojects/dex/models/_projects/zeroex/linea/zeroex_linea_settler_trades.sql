...
settler_txs AS (
    ...
    FROM (
        SELECT
            ...
            a.settler_address
        FROM 
            {{ source('linea', 'traces') }} AS tr
        JOIN 
            result_0x_settler_addresses a ON a.settler_address = tr.to AND a.blockchain = 'linea' AND tr.block_time > a.begin_block_time
        WHERE 
...
fills as (
        ...
        from {{ source('linea', 'logs_decoded') }}  l
        join tbl_trades tt on tt.tx_hash = l.tx_hash and l.block_time = tt.block_time and l.block_number = tt.block_number 
        and event_name in ('TokenExchange', 'OtcOrderFilled', 'SelllineaToken', 'Swap', 'BuyGem', 'DODOSwap', 'SellGem', 'Submitted')
        WHERE  1=1 
        {% if is_incremental() %}
...
results AS (
    ...
    FROM 
        tbl_trades trades
    JOIN 
        {{ source('linea', 'transactions') }} tr ON tr.hash = trades.tx_hash AND tr.block_time = trades.block_time AND tr.block_number = trades.block_number
    LEFT JOIN 
        fills f ON f.tx_hash = trades.tx_hash AND f.block_time = trades.block_time AND f.block_number = trades.block_number 
    LEFT JOIN 
        tokens tt ON tt.blockchain = 'linea' AND tt.contract_address = taker_token
    LEFT JOIN 
        tokens tm ON tm.blockchain = 'linea' AND tm.contract_address = maker_token
    LEFT JOIN 
        prices pt ON pt.blockchain = 'linea' AND pt.contract_address = taker_token AND pt.minute = DATE_TRUNC('minute', trades.block_time)
    LEFT JOIN 
        prices pm ON pm.blockchain = 'linea' AND pm.contract_address = maker_token AND pm.minute = DATE_TRUNC('minute', trades.block_time)
    WHERE 
        1=1 
        {% if is_incremental() %}
