...
settler_txs AS (
    ...
    FROM (
        SELECT
            ...
            a.settler_address
        FROM 
            {{ source('scroll', 'traces') }} AS tr
        JOIN 
            result_0x_settler_addresses a ON a.settler_address = tr.to AND a.blockchain = 'scroll' AND tr.block_time > a.begin_block_time
        WHERE 
...
fills as (
        ...
        from {{ source('scroll', 'logs_decoded') }}  l
        join tbl_trades tt on tt.tx_hash = l.tx_hash and l.block_time = tt.block_time and l.block_number = tt.block_number 
        and event_name in ('TokenExchange', 'OtcOrderFilled', 'SellscrollToken', 'Swap', 'BuyGem', 'DODOSwap', 'SellGem', 'Submitted')
        WHERE  1=1 
        {% if is_incremental() %}
...
results AS (
    ...
    FROM 
        tbl_trades trades
    JOIN 
        {{ source('scroll', 'transactions') }} tr ON tr.hash = trades.tx_hash AND tr.block_time = trades.block_time AND tr.block_number = trades.block_number
    LEFT JOIN 
        fills f ON f.tx_hash = trades.tx_hash AND f.block_time = trades.block_time AND f.block_number = trades.block_number 
    LEFT JOIN 
        tokens tt ON tt.blockchain = 'scroll' AND tt.contract_address = taker_token
    LEFT JOIN 
        tokens tm ON tm.blockchain = 'scroll' AND tm.contract_address = maker_token
    LEFT JOIN 
        prices pt ON pt.blockchain = 'scroll' AND pt.contract_address = taker_token AND pt.minute = DATE_TRUNC('minute', trades.block_time)
    LEFT JOIN 
        prices pm ON pm.blockchain = 'scroll' AND pm.contract_address = maker_token AND pm.minute = DATE_TRUNC('minute', trades.block_time)
    WHERE 
        1=1 
        {% if is_incremental() %}
...
results_usd AS (
    SELECT
        ...
        CASE WHEN maker_token IN  (0x5300000000000000000000000000000000000004,
                                    0x06efdbff2a14a7c8e15944d1f4a48f9f95f663a4,
                                    0xf55bec9cafdbe8730f096aa55dad6d22d44099df,
                                    0xca77eb3fefe3725dc33bccb54edefc3d9f764f97) AND  maker_amount IS NOT NULL
            THEN maker_amount
            WHEN taker_token IN   (0x5300000000000000000000000000000000000004,
                                    0x06efdbff2a14a7c8e15944d1f4a48f9f95f663a4,
                                    0xf55bec9cafdbe8730f096aa55dad6d22d44099df,
                                    0xca77eb3fefe3725dc33bccb54edefc3d9f764f97)  AND taker_amount IS NOT NULL
            THEN taker_amount
