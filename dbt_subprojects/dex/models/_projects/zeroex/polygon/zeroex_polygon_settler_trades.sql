...
settler_txs AS (
    ...
    FROM (
        SELECT
            ...
            a.settler_address
        FROM 
            {{ source('polygon', 'traces') }} AS tr
        LEFT JOIN 
            result_0x_settler_addresses a ON a.settler_address = tr.to AND a.blockchain = 'polygon' AND tr.block_time > a.begin_block_time
        WHERE 
            (a.settler_address IS NOT NULL OR tr.to = 0x0000000000001fF3684f28c67538d4D072C22734)
...
fills as (
        ...
        from {{ source('polygon', 'logs_decoded') }}  l
        join tbl_trades tt on tt.tx_hash = l.tx_hash and l.block_time = tt.block_time and l.block_number = tt.block_number 
        and event_name in ('TokenExchange', 'OtcOrderFilled', 'SellpolygonToken', 'Swap', 'BuyGem', 'DODOSwap', 'SellGem', 'Submitted')
        WHERE  1=1 
        {% if is_incremental() %}
...
results AS (
    ...
    FROM 
        tbl_trades trades
    JOIN 
        {{ source('polygon', 'transactions') }} tr ON tr.hash = trades.tx_hash AND tr.block_time = trades.block_time AND tr.block_number = trades.block_number
    LEFT JOIN 
        fills f ON f.tx_hash = trades.tx_hash AND f.block_time = trades.block_time AND f.block_number = trades.block_number 
    LEFT JOIN 
        tokens tt ON tt.blockchain = 'polygon' AND tt.contract_address = taker_token
    LEFT JOIN 
        tokens tm ON tm.blockchain = 'polygon' AND tm.contract_address = maker_token
    LEFT JOIN 
        prices pt ON pt.blockchain = 'polygon' AND pt.contract_address = taker_token AND pt.minute = DATE_TRUNC('minute', trades.block_time)
    LEFT JOIN 
        prices pm ON pm.blockchain = 'polygon' AND pm.contract_address = maker_token AND pm.minute = DATE_TRUNC('minute', trades.block_time)
    WHERE 
        1=1 
        {% if is_incremental() %}
...
results_usd AS (
    SELECT
        ...
        CASE WHEN maker_token IN (0x2791bca1f2de4661ed88a30c99a7a9449aa84174,
                                    0x7ceb23fd6bc0add59e62ac25578270cff1b9f619,
                                    0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270,
                                    0xc2132d05d31c914a87c6611c10748aeb04b58e8f,
                                    0x3c499c542cef5e3811e1192ce70d8cc03d5c3359,
                                    0x1bfd67037b42cf73acf2047067bd4f2c47d9bfd6) AND  maker_amount IS NOT NULL
            THEN maker_amount
            WHEN taker_token IN  (0x2791bca1f2de4661ed88a30c99a7a9449aa84174,
                                    0x7ceb23fd6bc0add59e62ac25578270cff1b9f619,
                                    0x0d500b1d8e8ef31e21c99d1db9a6444d3adf1270,
                                    0xc2132d05d31c914a87c6611c10748aeb04b58e8f,
                                    0x3c499c542cef5e3811e1192ce70d8cc03d5c3359,
                                    0x1bfd67037b42cf73acf2047067bd4f2c47d9bfd6) AND taker_amount IS NOT NULL
            THEN taker_amount
