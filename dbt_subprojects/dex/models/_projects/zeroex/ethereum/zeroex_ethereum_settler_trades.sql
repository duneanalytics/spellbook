...
settler_txs AS (
    ...
    FROM (
        ...
        JOIN 
            result_0x_settler_addresses a ON a.settler_address = tr.to AND a.blockchain = 'ethereum' AND tr.block_time > a.begin_block_time
        WHERE 
...
results AS (
    ...
    FROM 
        tbl_trades trades
    JOIN 
        {{ source('ethereum', 'transactions') }} tr ON tr.hash = trades.tx_hash AND tr.block_time = trades.block_time AND tr.block_number = trades.block_number
    LEFT JOIN 
        tokens tt ON tt.blockchain = 'ethereum' AND tt.contract_address = taker_token
    LEFT JOIN 
        tokens tm ON tm.blockchain = 'ethereum' AND tm.contract_address = maker_token
    LEFT JOIN 
        prices pt ON pt.blockchain = 'ethereum' AND pt.contract_address = taker_token AND pt.minute = DATE_TRUNC('minute', trades.block_time)
    LEFT JOIN 
        prices pm ON pm.blockchain = 'ethereum' AND pm.contract_address = maker_token AND pm.minute = DATE_TRUNC('minute', trades.block_time)
    WHERE 
        1=1 
        {% if is_incremental() %}
...
results_usd AS (
    SELECT
        ...
        taker_token_amount,
        maker_token_amount,
        cast(taker_token_amount_raw as int256) taker_token_amount_raw,
        cast(maker_token_amount_raw as int256) maker_token_amount_raw,
        CASE WHEN maker_token IN    (0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2,
                                    0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48,
                                    0xdac17f958d2ee523a2206206994597c13d831ec7,
                                    0x4fabb145d64652a948d72533023f6e7a623c7c53,
                                    0x6b175474e89094c44da98b954eedeac495271d0f,
                                    0xae7ab96520de3a18e5e111b5eaab095312d7fe84) AND  maker_amount IS NOT NULL
            THEN maker_amount
            WHEN taker_token IN (0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2,
                                    0xa0b86991c6218b36c1d19d4a2e9eb0ce3606eb48,
                                    0xdac17f958d2ee523a2206206994597c13d831ec7,
                                    0x4fabb145d64652a948d72533023f6e7a623c7c53,
                                    0x6b175474e89094c44da98b954eedeac495271d0f,
                                    0xae7ab96520de3a18e5e111b5eaab095312d7fe84) AND  maker_amount IS NOT NULL  AND taker_amount IS NOT NULL
            THEN taker_amount
