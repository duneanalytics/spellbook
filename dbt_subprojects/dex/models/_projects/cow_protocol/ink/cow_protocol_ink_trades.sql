...
atokens_mapping as (
    SELECT
        DISTINCT 'ink' as blockchain,
        aToken as atoken_address,
        asset as underlying_address
    FROM {{source('aave_v3_ink','poolconfigurator_evt_reserveinitialized')}}
),
...
atoken_prices as (
    ...
    WHERE atkm.blockchain = 'ink'
    and prc.blockchain = 'ink'
    {% if is_incremental() %}
...
trades_with_token_units as (
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} ts
                             ON ts.blockchain='ink' AND ts.contract_address = sell_token
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} tb
                             ON tb.blockchain='ink' AND tb.contract_address =
                                (CASE
...
sorted_orders as (
    ...
    from (
        select
            ...
            evt_block_number,
            orderUid
        from {{ source('gnosis_protocol_v2_ink', 'GPv2Settlement_evt_Trade') }}
        {% if is_incremental() %}
        where {{ incremental_predicate('evt_block_time') }}
        {% endif %}
    )
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join {{ source('gnosis_protocol_v2_ink', 'GPv2Settlement_call_settle') }}
...
eth_flow_senders as (
    ...
    from {{ source('cow_protocol_ink', 'CoWSwapEthFlow_evt_OrderPlacement') }} event
    inner join {{ source('cow_protocol_ink', 'CoWSwapEthFlow_call_createOrder') }} call
