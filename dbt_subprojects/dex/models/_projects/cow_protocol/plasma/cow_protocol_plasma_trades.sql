...
trades_with_token_units as (
    SELECT block_date,
           ...
           (CASE
                ...
                WHEN buy_token = 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 'XPL'
                ELSE tb.symbol
               END)                          as buy_token,
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} ts
                             ON ts.blockchain='plasma' AND ts.contract_address = sell_token
...
sorted_orders as (
    ...
    from (
        select
            ...
            evt_block_number,
            orderUid
        from {{ source('gnosis_protocol_v2_plasma', 'GPv2Settlement_evt_Trade') }}
        {% if is_incremental() %}
        where {{ incremental_predicate('evt_block_time') }}
        {% endif %}
    )
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join {{ source('gnosis_protocol_v2_plasma', 'GPv2Settlement_call_settle') }}
...
eth_flow_senders as (
    ...
    from {{ source('cow_protocol_plasma', 'CoWSwapEthFlow_evt_OrderPlacement') }} event
    inner join {{ source('cow_protocol_plasma', 'CoWSwapEthFlow_call_createOrder') }} call
...
valued_trades as (
    SELECT block_date,
           ...
           sell_token_address,
           case when sender is not null then 'XPL' else sell_token end as sell_token,
           buy_token_address,
           buy_token,
           case
