...
trades_with_token_units as (
    SELECT block_date,
           ...
           (CASE
                ...
                WHEN buy_token = 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee THEN 'AVAX'
                ELSE tb.symbol
               END)                          as buy_token,
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} ts
                             ON ts.blockchain='avalanche_c' AND ts.contract_address = sell_token
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} tb
                             ON tb.blockchain='avalanche_c' AND tb.contract_address =
                                (CASE
...
sorted_orders as (
    ...
    from (
        select
            ...
            evt_block_number,
            orderUid
        from {{ source('gnosis_protocol_v2_avalanche_c', 'GPv2Settlement_evt_Trade') }}
        {% if is_incremental() %}
        where {{ incremental_predicate('evt_block_time') }}
        {% endif %}
    )
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join {{ source('gnosis_protocol_v2_avalanche_c', 'GPv2Settlement_call_settle') }}
...
eth_flow_senders as (
    ...
    from {{ source('cow_protocol_avalanche_c', 'CoWSwapEthFlow_evt_OrderPlacement') }} event
    inner join {{ source('cow_protocol_avalanche_c', 'CoWSwapEthFlow_call_createOrder') }} call
