...
trades_with_token_units as (
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} ts
                             ON ts.blockchain='base' AND ts.contract_address = sell_token
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} tb
                             ON tb.blockchain='base' AND tb.contract_address =
                                (CASE
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join {{ source('gnosis_protocol_v2_base', 'GPv2Settlement_call_settle') }}
...
eth_flow_senders as (
    ...
    from {{ source('cow_protocol_base', 'CoWSwapEthFlow_evt_OrderPlacement') }} event
    inner join {{ source('cow_protocol_base', 'CoWSwapEthFlow_call_createOrder') }} call
