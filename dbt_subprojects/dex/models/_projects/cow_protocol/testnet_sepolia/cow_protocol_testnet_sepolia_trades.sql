...
-- Find the PoC Query here: https://dune.com/queries/3783635
WITH
    ...
    FROM {{ source('gnosis_protocol_v2_testnet_sepolia', 'GPv2Settlement_evt_Trade') }} trade
             ...
             LEFT OUTER JOIN {{ source('prices', 'usd') }} as pb
                             ON pb.contract_address = (
                                 ...
                                 AND pb.blockchain = 'testnet_sepolia'
                                 {% if is_incremental() %}
...
trades_with_token_units as (
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} ts
                             ON ts.blockchain='testnet_sepolia' AND ts.contract_address = sell_token
...
sorted_orders as (
    ...
    from (
        select
            ...
            evt_block_number,
            orderUid
        from {{ source('gnosis_protocol_v2_testnet_sepolia', 'GPv2Settlement_evt_Trade') }}
        {% if is_incremental() %}
        where {{ incremental_predicate('evt_block_time') }}
        {% endif %}
    )
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join {{ source('gnosis_protocol_v2_testnet_sepolia', 'GPv2Settlement_call_settle') }}
