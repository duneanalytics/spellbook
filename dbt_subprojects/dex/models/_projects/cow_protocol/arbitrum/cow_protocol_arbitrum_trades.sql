...
atokens_mapping as (
    ...
    FROM {{source('aave_v3_arbitrum','poolconfigurator_evt_reserveinitialized')}} 
),
...
atoken_prices as (
    ...
    WHERE atkm.blockchain = 'arbitrum'
    and prc.blockchain = 'arbitrum'
    {% if is_incremental() %}
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join {{ source('gnosis_protocol_v2_arbitrum', 'GPv2Settlement_call_settle') }}
