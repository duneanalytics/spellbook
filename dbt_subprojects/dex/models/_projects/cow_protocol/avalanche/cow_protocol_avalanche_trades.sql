                             ...
                             ON pb.contract_address = (
                                 CASE
                                     WHEN buyToken = 0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee
                                         THEN 0x49d5c2bdffac6ce2bfdb6640f4f80f226bc10bab
                                     ELSE buyToken
                                     END)
                                 AND pb.minute = date_trunc('minute', evt_block_time)
                                 AND pb.blockchain = 'avalanche'
                                 {% if is_incremental() %}
...
trades_with_token_units as (
    ...
    FROM trades_with_prices
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} ts
                             ON ts.blockchain='avalanche' AND ts.contract_address = sell_token
             LEFT OUTER JOIN {{ source('tokens', 'erc20') }} tb
                             ON tb.blockchain='avalanche' AND tb.contract_address =
                                (CASE
...
sorted_orders as (
    ...
    from (
        select
            ...
            evt_block_number,
            orderUid
        from {{ source('gnosis_protocol_v2_avalanche', 'GPv2Settlement_evt_Trade') }}
        {% if is_incremental() %}
        where {{ incremental_predicate('evt_block_time') }}
        {% endif %}
    )
...
orders_and_trades as (
    select
        ...
        order_ids
    from sorted_orders
    inner join {{ source('gnosis_protocol_v2_avalanche', 'GPv2Settlement_call_settle') }}
...
eth_flow_senders as (
    ...
    from {{ source('cow_protocol_avalanche', 'CoWSwapEthFlow_evt_OrderPlacement') }} event
    inner join {{ source('cow_protocol_avalanche', 'CoWSwapEthFlow_call_createOrder') }} call
