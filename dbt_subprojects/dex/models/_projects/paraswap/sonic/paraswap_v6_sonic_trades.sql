...
with dexs AS (
        SELECT
            ...
            method,
            {{ to_wrapped_native_token('sonic', 'from_hex(destToken)', 'token_bought_address')   }},
            {{ to_wrapped_native_token('sonic', 'from_hex(srcToken)', 'token_sold_address')   }},            
            projectContractAddress as project_contract_address,
            call_tx_hash as tx_hash,
            call_trace_address AS trace_address,
            CAST(-1 as integer) AS evt_index
        FROM {{ ref('paraswap_v6_sonic_trades_decoded') }}
        {% if is_incremental() %}
        WHERE {{ incremental_predicate('blockTime') }}
        {% endif %}
)

SELECT 'sonic' AS blockchain,
    'paraswap' AS project,
    '6' AS version,
    ...
    d.evt_index
FROM dexs d
INNER JOIN {{ source('sonic', 'transactions') }} tx ON d.tx_hash = tx.hash
