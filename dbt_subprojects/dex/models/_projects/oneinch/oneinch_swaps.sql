...
select
    ...
    , get_href(get_chain_explorer_address(blockchain, if(blockchain = 'solana', to_base58(src_token_address), cast(src_token_address as varchar))), coalesce(src_executed_symbol, format('%s…%s', if(blockchain = 'solana', substr(to_base58(src_token_address), 1, 4), substr(src_token_address, 1, 2)), if(blockchain = 'solana', substr(to_base58(src_token_address), -4), substr(cast(src_token_address as varchar), -4))))) as src_link
    , get_href(get_chain_explorer_address(blockchain, if(blockchain = 'solana', to_base58(dst_token_address), cast(dst_token_address as varchar))), coalesce(dst_executed_symbol, format('%s…%s', if(blockchain = 'solana', substr(to_base58(dst_token_address), 1, 4), substr(dst_token_address, 1, 2)), if(blockchain = 'solana', substr(to_base58(dst_token_address), -4), substr(cast(dst_token_address as varchar), -4))))) as dst_link
    , coalesce(src_executed_symbol, format('%s…%s', if(blockchain = 'solana', substr(to_base58(src_token_address), 1, 4), substr(src_token_address, 1, 2)), if(blockchain = 'solana', substr(to_base58(src_token_address), -4), substr(cast(src_token_address as varchar), -4)))) as src_token
    , coalesce(dst_executed_symbol, format('%s…%s', if(blockchain = 'solana', substr(to_base58(dst_token_address), 1, 4), substr(dst_token_address, 1, 2)), if(blockchain = 'solana', substr(to_base58(dst_token_address), -4), substr(cast(dst_token_address as varchar), -4)))) as dst_token
    ...
    , if(mod(length(call_output), 32) = 4, element_at(slice(regexp_extract_all(from_utf8(call_output), '[\w\d\s\.:\(\)]+'), -1, 1), 1)) as call_output_decoded
from executions
