...
{% set c_seaport_first_date = '2024-11-12' %}

with source_abstract_transactions as (
    select *
    from {{ source('abstract','transactions') }}
...
,ref_seaport_abstract_base_pairs as (
      select *
      from {{ ref('seaport_abstract_base_pairs') }}
      where 1=1
      {% if is_incremental() %}
...
,iv_base_pairs_priv as (
  select a.block_date
        ...
        ,a.is_traded_nft
        ,a.is_moved_nft
  from ref_seaport_abstract_base_pairs a
  where 1=1
  ...
  select a.block_date
        ...
        ,a.is_moved_nft
  from ref_seaport_abstract_base_pairs a
  left join ref_seaport_abstract_base_pairs b on b.tx_hash = a.tx_hash
...
,iv_trades as (
  ...
  from iv_nfts a
  inner join source_abstract_transactions t on t.hash = a.tx_hash
