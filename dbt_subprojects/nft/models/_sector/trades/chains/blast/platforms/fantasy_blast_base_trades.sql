...
WITH trades AS (
    ...
    --, tx_from
    --, tx_to
    , CAST((CASE WHEN block_number < 18397361 THEN 0.015 ELSE 0.003 END)*CAST(price_raw AS double) AS UINT256) AS platform_fee_amount_raw
    , CAST((CASE WHEN block_number < 18397361 THEN 0.015 ELSE 0.027 END)*CAST(price_raw AS double) AS UINT256) AS royalty_fee_amount_raw
