...
{% set nft_start_date = "TIMESTAMP '2024-11-12'" %}
...
WITH erc721_trades AS (
    ...
    FROM {{ source ('zeroex_abstract', 'ExchangeProxy_evt_ERC721OrderFilled') }}
...
, erc1155_trades as (
    ...
    SELECT CASE when direction = 0 THEN 'buy' ELSE 'sell' END AS trade_category,
          ...
          erc20Token as original_erc20_token
    FROM {{ source ('zeroex_abstract', 'ExchangeProxy_evt_ERC1155OrderFilled') }}
    WHERE starts_with(cast(nonce as varchar),'{{magic_eden_nonce}}')
        {% if not is_incremental() %}
        AND evt_block_time >= {{nft_start_date}}
        {% endif %}
...
,erc721_fees as (
    ...
    WITH orders as (
    select call_tx_hash, sellOrder as order_data from {{ source('zeroex_abstract','ExchangeProxy_call_buyERC721') }}
    union all
    select call_tx_hash, buyOrder as order_data from {{ source('zeroex_abstract','ExchangeProxy_call_sellERC721') }}
    union all
    select * from (
        select
        call_tx_hash
        ,order_data
        from {{ source('zeroex_abstract','ExchangeProxy_call_batchBuyERC721s') }}
...
,erc1155_fees as (
    WITH orders as (
    select call_tx_hash, sellOrder as order_data from {{ source('zeroex_abstract','ExchangeProxy_call_buyERC1155') }}
    union all
    select call_tx_hash, buyOrder as order_data from {{ source('zeroex_abstract','ExchangeProxy_call_sellERC1155') }}
    union all
    select * from (
        select
        call_tx_hash
        ,order_data
        from {{ source('zeroex_abstract','ExchangeProxy_call_batchBuyERC1155s') }}
        cross join unnest(sellOrders) as foo(order_data)
        )
    )
    select
...
{{ add_nft_tx_data('base_trades', 'abstract') }}
