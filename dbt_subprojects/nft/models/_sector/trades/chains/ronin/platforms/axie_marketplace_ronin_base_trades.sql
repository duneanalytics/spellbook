...
UNION ALL
  ...
  coalesce(cast(json_extract_scalar(json_parse(json_extract_scalar("order", '$.info')), '$.kind') as UINT256),
            cast(json_extract_scalar("order", '$.kind') as UINT256)) as kind,
  coalesce(cast(json_extract_scalar(json_parse(replace(json_extract_scalar(json_extract(json_parse(json_extract_scalar("order", '$.info')), '$.assets'), '$[0]'), '\\', '')), '$.erc') as UINT256),
  cast(json_extract_scalar(replace(json_extract_scalar(json_extract("order", '$.assets'), '$[0]'), '\\', ''), '$.erc') as UINT256)) as erc,
  coalesce(from_hex(json_extract_scalar(replace(json_extract_scalar(json_parse(json_extract_scalar("order", '$.info')),'$.assets[0]'),'\\', ''),'$.addr')),
            from_hex(json_extract_scalar(replace(json_extract_scalar(json_extract("order", '$.assets'), '$[0]'), '\\', ''), '$.addr'))) AS nft_contract_address,
  
  coalesce(cast(json_extract_scalar(replace(json_extract_scalar(json_parse(json_extract_scalar("order", '$.info')),'$.assets[0]'),'\\', ''),'$.id') as double),
            cast(json_extract_scalar(replace(json_extract_scalar(json_extract("order", '$.assets'), '$[0]'), '\\', ''), '$.id') as double)) as nft_token_id,
  coalesce(cast(json_extract_scalar(replace(json_extract_scalar(json_parse(json_extract_scalar("order", '$.info')),'$.assets[0]'),'\\', ''),'$.quantity')as DOUBLE),
            cast(json_extract_scalar(replace(json_extract_scalar(json_extract("order", '$.assets'), '$[0]'), '\\', ''), '$.quantity') as UINT256)) as quantity,
  coalesce(from_hex(json_extract_scalar(json_parse(json_extract_scalar("order", '$.info')), '$.paymentToken')),
           from_hex(json_extract_scalar("order",'$.paymentToken'))) as currency_address,
  coalesce(cast(json_extract_scalar("order", '$.realPrice') as DOUBLE),cast(realPrice as DOUBLE)) as price_raw,
  coalesce(cast(json_extract_scalar(json_parse(json_extract_scalar("order", '$.info')), '$.baseUnitPrice') as DOUBLE),
                cast(json_extract_scalar("order",'$.basePrice') as DOUBLE)) as base_unit_price_raw,
