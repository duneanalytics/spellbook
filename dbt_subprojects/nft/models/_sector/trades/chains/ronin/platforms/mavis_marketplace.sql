...
select 
  ...
  evt_tx_to as tx_to,
  evt_index,
  COALESCE(FROM_HEX(json_extract_scalar(json_parse(cast(concat('[', array_join(receivedAllocs, ','), ']') as varchar)),'$[5].recipient')),FROM_HEX(json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')), '$.maker'))) as seller,
  FROM_HEX(json_extract_scalar(m."order", '$.recipient')) as buyer,
  json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')), '$.kind') as kind,
  cast(json_extract_scalar(replace(json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')),'$.assets[0]'),'\\', ''),'$.erc')as INT) as erc,
  FROM_HEX(json_extract_scalar(replace(json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')),'$.assets[0]'),'\\', ''),'$.addr')) as nft_contract_address,
  json_extract_scalar(replace(json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')),'$.assets[0]'),'\\', ''),'$.id') as nft_token_id,
  json_extract_scalar(replace(json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')),'$.assets[0]'),'\\', ''),'$.quantity') as quantity,
  FROM_HEX(json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')), '$.paymentToken')) as currency_address,
  CAST(json_extract_scalar(m."order", '$.realPrice') AS DOUBLE) as price_raw,
  CAST(json_extract_scalar(json_parse(json_extract_scalar(m."order", '$.info')), '$.baseUnitPrice') AS DOUBLE) as base_unit_price_raw,
  FROM_HEX(json_extract_scalar(m."order", '$.refunder')) as refunder,
