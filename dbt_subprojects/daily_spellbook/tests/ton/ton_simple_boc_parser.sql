WITH test_data AS (
    SELECT boc, expected
    FROM (
        VALUES
        (
            ,
            map_from_entries(ARRAY[
                ('opcode', CAST(CAST(2 AS int) AS JSON)),
                ('owner_address', CAST('0:A30C3372646B7A659D279C52CB1DDD7B210C0EB24BBC0830F6D30870E61C1534' AS JSON)),
                ('sender_address', CAST('0:76313627AB7454776C52C547B958773A3EE4C84BB2BDA21AC7A67D3A56F86B1B' AS JSON)),
                ('recipient_address', CAST('0:A30C3372646B7A659D279C52CB1DDD7B210C0EB24BBC0830F6D30870E61C1534' AS JSON)),
                ('current_time', CAST(CAST(1741684835 AS int) AS JSON)),
                ('asset_id', CAST('91621667903763073563570557639433445791506232618002614896981036659302854767224' AS JSON))
                ]) 
        )
    )
    AS temp (boc, expected)
), test_results AS (    
    SELECT {{ ton_from_boc('boc', [
    ton_begin_parse(),
    ton_load_uint(8, 'opcode'),
    ton_load_address('owner_address'),
    ton_load_address('sender_address'),
    ton_load_address('recipient_address'),
    ton_load_uint(32, 'current_time'),
    ton_skip_refs(1),
    ton_load_ref(),
    ton_begin_parse(),
    ton_load_uint(256, 'asset_id')
    ]) }} as result, expected 
    FROM test_data
)
SELECT json_format(CAST(result AS json)) AS result, json_format(CAST(expected AS json)) AS expected FROM test_results
WHERE result != expected