...
WHERE name IS NOT NULL)
    ...
    SELECT DISTINCT
         ...
         , g.label_type
    FROM gauges g
    LEFT JOIN {{ source('balancer_ethereum', 'OptimismRootGauge_call_killGauge') }} k1 ON g.address = k1.contract_address AND k1.call_success
    LEFT JOIN {{ source('balancer_ethereum', 'CappedOptimismRootGauge_call_killGauge') }} k2 ON g.address = k2.contract_address AND k2.call_success