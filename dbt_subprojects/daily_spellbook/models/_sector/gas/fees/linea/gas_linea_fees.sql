...
SELECT
     ...
     (txns.gas_used/1e18) * txns.gas_price as tx_fee_native,
     (txns.gas_used/1e18) * txns.gas_price * p.price AS tx_fee_usd,
...
FROM {{ source('linea','transactions') }} txns
INNER JOIN {{ source('linea','blocks') }} blocks
