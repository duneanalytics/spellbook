...
updated_metrics AS (
    SELECT
        ...
        blockchain,
        currency_symbol,
        COALESCE(cm.number_of_txs, 0) + COUNT(*) FILTER (where block_time > lw.last_block_time_of_incremental_update) as number_of_txs,
        COALESCE(cm.gas_spent_usd_total, 0) + SUM(CASE WHEN block_time > lw.last_block_time_of_incremental_update THEN lw.gas_cost_usd END) as gas_spent_usd_total,
        SUM(CASE WHEN block_time >= now() - INTERVAL '1' DAY THEN lw.gas_cost_usd END) as gas_spent_usd_24_hours,
        SUM(CASE WHEN block_time >= now() - INTERVAL '7' DAY THEN lw.gas_cost_usd END) as gas_spent_usd_7_days,
        SUM(CASE WHEN block_time >= now() - INTERVAL '30' DAY THEN lw.gas_cost_usd END) as gas_spent_usd_30_days,
        SUM(cm.gas_spent_usd_1_year) + SUM(CASE WHEN incremental_predicate('block_time') THEN lw.gas_cost_usd END) - COALESCE(wtf.gas_spent_usd, 0) as gas_spent_usd_1_year,
        COALESCE(cm.gas_spent_native_total, 0) + SUM(CASE WHEN block_time > lw.last_block_time_of_incremental_update THEN lw.gas_cost_native END) as gas_spent_native_total,
        SUM(CASE WHEN block_time >= now() - INTERVAL '1' DAY THEN lw.gas_cost_native END) as gas_spent_native_24_hours,
        SUM(CASE WHEN block_time >= now() - INTERVAL '7' DAY THEN lw.gas_cost_native END) as gas_spent_native_7_days,
        SUM(CASE WHEN block_time >= now() - INTERVAL '30' DAY THEN lw.gas_cost_native END) as gas_spent_native_30_days,
        SUM(cm.gas_spent_native_1_year) + SUM(CASE WHEN incremental_predicate('block_time') THEN lw.gas_cost_native END) - COALESCE(wtf.gas_spent_native, 0) as gas_spent_native_1_year,
