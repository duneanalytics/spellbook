{{
  ...
  WITH vault_mappings AS (
    SELECT DISTINCT
      ...
      amountUnderlying,
      amountWrapped
    FROM {{ source('balancer_v3_hyperevm','Vault_evt_LiquidityAddedToBuffer') }}  b
  ...
  underlying_tokens AS (
    ...
    JOIN {{ source('erc20_hyperevm','evt_Transfer') }} t ON t.evt_tx_hash = vm.evt_tx_hash
