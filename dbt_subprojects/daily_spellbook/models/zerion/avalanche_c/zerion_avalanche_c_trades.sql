...
{% set native_wrapped_currency_address = '0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7' %}
{% set native_currency_symbol = 'AVAX' %}
...
WITH zerion_trades AS (
    ...
    INNER JOIN {{ source('avalanche_c','transactions') }} pt ON pt.block_number=swap.evt_block_number
        ...
        {% endif %}
    LEFT JOIN {{ source('tokens_avalanche_c', 'erc20') }} tok_sold ON tok_sold.contract_address=swap.inputToken
    LEFT JOIN {{ source('tokens_avalanche_c', 'erc20') }} tok_bought ON tok_bought.contract_address=swap.outputToken
    {% if not is_incremental() %}
...
LEFT JOIN {{ source('prices','usd_forward_fill') }} pu_sold ON pu_sold.blockchain='avalanche_c'
    ...
    {% endif %}
LEFT JOIN {{ source('prices','usd_forward_fill') }} pu_bought ON pu_bought.blockchain='avalanche_c'
