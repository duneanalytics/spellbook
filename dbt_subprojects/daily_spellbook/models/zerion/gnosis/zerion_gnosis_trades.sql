...
WITH zerion_trades AS (
    ...
    INNER JOIN {{ source('gnosis','transactions') }} pt ON pt.block_number=swap.evt_block_number
        ...
        {% endif %}
    LEFT JOIN {{ source('tokens_gnosis', 'erc20') }} tok_sold ON tok_sold.contract_address=swap.inputToken
    LEFT JOIN {{ source('tokens_gnosis', 'erc20') }} tok_bought ON tok_bought.contract_address=swap.outputToken
    {% if not is_incremental() %}
...
LEFT JOIN {{ source('prices','usd_forward_fill') }} pu_sold ON pu_sold.blockchain='gnosis'
    ...
    {% endif %}
LEFT JOIN {{ source('prices','usd_forward_fill') }} pu_bought ON pu_bought.blockchain='gnosis'
