...
with supply_data as (
    select
        ...
        case
            ...
            then try(regexp_extract(cast(type_ as varchar), '::token_registry::Key<([^>]+)>', 1))
        ...
        case
            ...
            when cast(type_ as varchar) like '%::TreasuryCapManager' 
            then try(cast(json_extract_scalar(object_json, '$.treasury.total_supply.value') as double))
            
            -- For TreasuryCap (top-level total_supply:value)
            when cast(type_ as varchar) like '0x2::coin::TreasuryCap<%>%' 
            then try(cast(json_extract_scalar(object_json, '$.total_supply.value') as double))
            ...
            then try(cast(json_extract_scalar(object_json, '$.treasury_cap.total_supply.value') as double))
            
            -- For Dynamic Fields (Token Registry/Wrapped Asset) (value:treasury_cap:total_supply:value)
            when cast(type_ as varchar) like '0x2::dynamic_field::Field<%::token_registry::Key<%>%wrapped_asset::WrappedAsset<%>%' 
            then try(cast(json_extract_scalar(object_json, '$.value.treasury_cap.total_supply.value') as double))
