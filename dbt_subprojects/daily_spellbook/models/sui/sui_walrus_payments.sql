...
wal_moves_bc as (
  select
      ('0x' || lower(to_hex(from_base58(bc.transaction_digest)))) as transaction_digest_hex
    , sum(cast(bc.amount as decimal(38,0))) as wal_amount_raw
  from {{ source('sui','balance_changes') }} bc
  where lower(bc.coin_type) = lower('{{ WAL_BASE }}')
    and ('0x' || lower(to_hex(from_base58(bc.transaction_digest)))) in (select transaction_digest_hex from walrus_tx)
  group by 1
),
...
wal_moves as (
  select x.transaction_digest_hex,
         coalesce(bc.wal_amount_raw, ev.wal_amount_raw) as wal_amount_raw
  from (select transaction_digest_hex from walrus_tx) x
  left join wal_moves_bc    bc on bc.transaction_digest_hex = x.transaction_digest_hex
  left join wal_moves_events ev on ev.transaction_digest_hex = x.transaction_digest_hex
),

-- D) WAL decimals (from coin_info)
wal_meta as (
  select coin_decimals as wal_decimals
  from {{ ref('dex_sui_coin_info') }}
  where lower(coin_type) = lower('{{ WAL_BASE }}')
  limit 1
)
