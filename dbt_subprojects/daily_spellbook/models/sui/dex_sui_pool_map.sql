...
-- 1) Pools we actually see
with used_pools as (
  select distinct
      case
        when starts_with(lower(pool_id), '0x') then lower(pool_id)
        else concat('0x', lower(pool_id))
      end as pool_id
  from {{ ref('dex_sui_base_trades') }}
  where pool_id is not null
)

-- 2) Latest object row for each pool_id
, latest as (
  select
      ('0x' || lower(to_hex(o.object_id))) as object_id_hex -- VARCHAR
...
-- 4) Split first two generics (A,B) and trim
, split as (
  select
      pool_id
    , type_str
...
-- 5) Unwrap 0x2::coin::Coin<…> -> … (leave as-is if already underlying)
, unwrapped as (
  select
      pool_id
    , type_str
...
-- 6) Keep only pool-ish structs and canonicalize addresses
, filtered as (
  select
      pool_id
