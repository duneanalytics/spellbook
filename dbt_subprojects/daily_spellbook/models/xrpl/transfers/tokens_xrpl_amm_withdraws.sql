{{
    config(
        schema = 'tokens_xrpl_amm_withdraws',
        alias = 'transfers',
...
successful_amm_withdraw_transactions AS (
    SELECT 
        hash AS tx_hash
        ...
        ,JSON_EXTRACT_SCALAR(metadata, '$.LPTokenIn.value') AS lp_token_in_value
        ,JSON_EXTRACT_SCALAR(metadata, '$.LPTokenIn.currency') AS lp_token_in_currency
        ,JSON_EXTRACT_SCALAR(metadata, '$.LPTokenIn.issuer') AS lp_token_in_issuer
        ,metadata
...
valid_amm_nodes AS (
    ...
    FROM successful_amm_withdraw_transactions
...
-- LP Token Burn (User → AMM Pool)
lp_token_burns AS (
    SELECT
        tx_hash || '_lp_burn' AS unique_key
        ,'xrpl' AS blockchain
        ...
        ,COALESCE(
            CASE WHEN node_index IS NOT NULL THEN
                JSON_EXTRACT_SCALAR(metadata, CONCAT('$.AffectedNodes[', CAST(node_index AS VARCHAR), '].ModifiedNode.FinalFields.LPTokenBalance.issuer'))
            END,
            lp_token_in_issuer
        ) AS issuer
        ,COALESCE(
            CASE WHEN node_index IS NOT NULL THEN
                JSON_EXTRACT_SCALAR(metadata, CONCAT('$.AffectedNodes[', CAST(node_index AS VARCHAR), '].ModifiedNode.FinalFields.LPTokenBalance.currency'))
            END,
            lp_token_in_currency
        ) AS currency
        ,COALESCE(
        ...
        ,COALESCE(
            TRY_CAST(lp_token_in_value AS DOUBLE),
            CASE WHEN node_index IS NOT NULL THEN
                TRY_CAST(JSON_EXTRACT_SCALAR(metadata, CONCAT('$.AffectedNodes[', CAST(node_index AS VARCHAR), '].ModifiedNode.PreviousFields.LPTokenBalance.value')) AS DOUBLE) - 
                TRY_CAST(JSON_EXTRACT_SCALAR(metadata, CONCAT('$.AffectedNodes[', CAST(node_index AS VARCHAR), '].ModifiedNode.FinalFields.LPTokenBalance.value')) AS DOUBLE)
            END
    ...
    WHERE ((lp_token_in_value IS NOT NULL AND TRY_CAST(lp_token_in_value AS DOUBLE) > 0)
           OR (node_index IS NOT NULL 
               ...
               AND TRY_CAST(JSON_EXTRACT_SCALAR(metadata, CONCAT('$.AffectedNodes[', CAST(node_index AS VARCHAR), '].ModifiedNode.PreviousFields.LPTokenBalance.value')) AS DOUBLE) > 
                   TRY_CAST(JSON_EXTRACT_SCALAR(metadata, CONCAT('$.AffectedNodes[', CAST(node_index AS VARCHAR), '].ModifiedNode.FinalFields.LPTokenBalance.value')) AS DOUBLE)))
),

-- Asset 1 Withdrawal (AMM Pool → User)
asset1_withdrawals AS (
    SELECT
        tx_hash || '_withdraw1' AS unique_key
        ,'xrpl' AS blockchain
        ,CAST(date_trunc('month', block_time) AS DATE) AS block_month
        ,CAST(block_time AS DATE) AS block_date
        ,block_time
        ...
        ,CASE WHEN amount_currency = 'XRP' THEN CAST(NULL AS VARCHAR) ELSE amount_issuer END AS issuer
...
-- Asset 2 Withdrawal (AMM Pool → User) - if present
asset2_withdrawals AS (
    SELECT
        tx_hash || '_withdraw2' AS unique_key
        ,'xrpl' AS blockchain
        ,CAST(date_trunc('month', block_time) AS DATE) AS block_month
        ,CAST(block_time AS DATE) AS block_date
        ,block_time
        ...
        ,tx_from AS to_address
        ,CASE WHEN amount2_currency = 'XRP' THEN CAST(NULL AS VARCHAR) ELSE amount2_issuer END AS issuer
...
-- Union all withdraw transfers
all_amm_withdraw_transfers AS (
    SELECT * FROM lp_token_burns
    UNION ALL
    SELECT * FROM asset1_withdrawals
    UNION ALL
    SELECT * FROM asset2_withdrawals
)
...
FROM all_amm_withdraw_transfers t
