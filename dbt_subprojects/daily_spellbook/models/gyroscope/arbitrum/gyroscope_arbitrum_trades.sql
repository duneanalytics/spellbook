...
with E_CLPs AS (
    SELECT 
        x.pool
        , y.min_block_time
        FROM {{ source('gyroscope_arbitrum','GyroECLPPoolFactory_evt_PoolCreated') }} x
        LEFT JOIN (
            SELECT min(evt_block_time) AS min_block_time 
            FROM {{ source('gyroscope_arbitrum','GyroECLPPoolFactory_evt_PoolCreated') }} 
        ) y
...
SELECT
    ...
    tx_to,
    evt_index
FROM {{ source('balancer_v2_arbitrum', 'trades') }} x 
inner join E_CLPs y
