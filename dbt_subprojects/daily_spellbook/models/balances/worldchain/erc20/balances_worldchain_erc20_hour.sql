...
years as (
    select year
    from (values (sequence(timestamp '2023-09-01', cast(date_trunc('year', now()) as timestamp), interval '1' year))) s(year_array)
...
hourly_balances as (
    ...
    FROM 
    {{ ref('transfers_worldchain_erc20_rolling_hour') }}
)
...
{{ source('prices', 'usd') }} p
    ...
    AND p.blockchain = 'worldchain'
-- Removes likely non-compliant tokens due to negative balances
LEFT JOIN 
{{ ref('balances_worldchain_erc20_noncompliant') }} nc
