{{ config(
    schema = 'thorchain',
    alias = 'defi_mint_burn_events',
    materialized = 'incremental',
    file_format = 'delta',
    incremental_strategy = 'merge',
    unique_key = ['block_month', 'fact_mint_burn_events_id'],
    partition_by = ['block_month'],
    incremental_predicates = [incremental_predicate('DBT_INTERNAL_DEST.block_time')],
    tags = ['thorchain', 'defi', 'mint_burn_events', 'fact'],
    post_hook='{{ expose_spells(\'["thorchain"]\',
                              "defi",
                              "defi_mint_burn_events",
                              \'["krishhh"]\') }}'
) }}
...
SELECT
    {{ dbt_utils.generate_surrogate_key(['a.event_id']) }} AS fact_mint_burn_events_id,
    b.block_time,
