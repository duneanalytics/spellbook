{{ config(
    schema = 'thorchain',
    alias = 'defi_trade_account_deposit_events',
    ...
    tags = ['thorchain', 'defi', 'trade_account_deposit_events', 'fact'],
    post_hook='{{ expose_spells(\'["thorchain"]\',
                              "defi",
                              "defi_trade_account_deposit_events",
                              \'["krishhh"]\') }}'
) }}
...
WITH deduplicated AS (
    ...
    FROM {{ source('thorchain', 'trade_account_deposit_events') }}
...
SELECT
    {{ dbt_utils.generate_surrogate_key([
        ...
        'a.event_id',
        'a.block_timestamp'
    ]) }} AS fact_trade_account_deposit_events_id,
    b.block_time,
