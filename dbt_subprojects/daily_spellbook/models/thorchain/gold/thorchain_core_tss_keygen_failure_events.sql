{{ config(
    schema = 'thorchain',
    alias = 'core_tss_keygen_failure_events',
    materialized = 'incremental',
    file_format = 'delta',
    incremental_strategy = 'merge',
    unique_key = ['block_month', 'fact_tss_keygen_failure_events_id'],
    partition_by = ['block_month'],
    incremental_predicates = [incremental_predicate('DBT_INTERNAL_DEST.block_time')],
    tags = ['thorchain', 'core', 'tss_keygen_failure_events', 'fact'],
    post_hook='{{ expose_spells(\'["thorchain"]\',
                              "core",
                              "core_tss_keygen_failure_events",
                              \'["krishhh"]\') }}'
) }}
...
WITH deduplicated AS (
    ...
    FROM {{ source('thorchain', 'tss_keygen_failure_events') }}
    {% if is_incremental() %}
