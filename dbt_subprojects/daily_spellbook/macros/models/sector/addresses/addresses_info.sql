...
, ARRAY_MAX(FILTER(x -> x IS NOT NULL, ARRAY[last_tx_block_time, last_transfer_block_time])) AS last_seen
, ARRAY_MAX(FILTER(x -> x IS NOT NULL, ARRAY[last_tx_block_number, last_received_block_number, last_sent_block_number])) AS last_seen_block
FROM transfers
...
, ARRAY_MAX(FILTER(x -> x IS NOT NULL, ARRAY[nd.last_tx_block_time, nd.last_transfer_block_time, t.last_seen])) AS last_seen
, ARRAY_MAX(FILTER(x -> x IS NOT NULL, ARRAY[nd.last_tx_block_number, nd.last_received_block_number, nd.last_sent_block_number, t.last_seen_block])) AS last_seen_blockFROM new_data nd
