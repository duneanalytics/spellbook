{% macro 
    oneinch_project_swaps_macro_test(
        blockchain
...
, orders as (
    ...
    from {{ ref('oneinch_' + blockchain + '_project_orders_' + partition_number) }}
    where
        {% if is_incremental() %}
...
, calls as (
    ...
    from (
        ...
        from {{ ref('oneinch_' + blockchain + '_project_calls_' + partition_number) }}
        where
            {% if is_incremental() %}
                {{ incremental_predicate('block_time') }}
            {% else %}
