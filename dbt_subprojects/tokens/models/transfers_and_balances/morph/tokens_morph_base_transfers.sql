...
-- morph supports both ERC20 and TIP20 token standards. TIP20 transfers are emitted
-- via tip20_morph.evt_transfer (separate event signature from ERC20). We ingest
-- TIP20 transfers and then take ERC20 transfers, deduplicating on (tx_hash, evt_index)
-- to avoid double-counting any contract that emits both.
...
with tip20_transfers as (
	...
	from
		{{ source('tip20_morph', 'evt_Transfer') }} as t
	{% if is_incremental() -%}
	where
...
tip20_base as (
	...
	inner join {{ source('morph', 'transactions') }} as tx
