...
with transfers as (
	select
		...
		, amount_raw
	from
		{{ ref('tokens_arbitrum_transfers_from_traces_base') }}
	where
...
from
	(
		{{ transfers_base_erc4626(
			...
			erc4626_deposit=source('erc4626_arbitrum', 'evt_deposit'),
			erc4626_withdraw=source('erc4626_arbitrum', 'evt_withdraw'),
		) }}
	)
