...
-- Inlined body of `transfers_from_traces_base_macro` with an added CI-target source-scan bound.
-- Kept inline (rather than in the shared macro) so `state:modified.macros` does not cascade the
-- CI bound to the ~50 other chains that call the shared macro. Non-CI compiled SQL is
-- semantically equivalent to `transfers_from_traces_base_macro(blockchain='ethereum')`.
