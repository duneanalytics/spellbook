...
with raw_transfers as (
    select
        blockchain
        , block_date
        , COALESCE(CAST(contract_address AS varchar), '{{ native_contract_address }}') as contract_address
        , symbol
        , tx_hash
        , "from" as address
        , 'sent' as transfer_direction
    ...
    group by
        blockchain
        , block_date
        , COALESCE(CAST(contract_address AS varchar), '{{ native_contract_address }}')
        , symbol
        , "from"
        , 'sent'
        , tx_hash
