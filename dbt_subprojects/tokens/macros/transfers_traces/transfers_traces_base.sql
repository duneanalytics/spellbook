{%- macro transfers_traces_base(blockchain, easy_dates=False) -%}
...
{% set native_address = '0x0000000000000000000000000000000000001010' if blockchain == 'polygon' else '0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee' %}
...
{% set mint_selector = '0x40c10f19' %} -- DAI, etc
{% set burn_selector = '0x9dc29fac' %} -- DAI, etc
{% set deposit_selector = '0xd0e30db0' %} -- wrapped
{% set withdraw_selector = '0x2e1a7d4d' %} -- wrapped
...
select
    ...
    , if(value > uint256 '0', 'native', '{{token_standard_20}}') as token_standard
    ...
    , {{ dbt_utils.generate_surrogate_key(["block_number", "tx_hash", "array_join(trace_address, ',')"])}} as unique_key
...
where
    ...
    and (call_type = 'call' or call_type is null) -- call_type should be only call if present // call type = null is contract creation
    and (tx_success or tx_success is null) -- tx_success = null is for old ethereum data
    and success
