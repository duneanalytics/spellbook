...
WHERE
    (
    traces."to" = (SELECT router FROM {{ref('chainlink_ccip_network_meta')}} WHERE blockchain = 'bnb' AND "version" = 'v1.0.0')
    OR
    traces."to" = (SELECT router FROM {{ref('chainlink_ccip_network_meta')}} WHERE blockchain = 'bnb' AND "version" = 'v1.2.0')
  )
