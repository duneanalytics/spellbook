WITH unit_test AS (
    SELECT
        ...
        -- test the effective_gas_price
        CASE 
            WHEN test.effective_gas_price = actual.effective_gas_price THEN TRUE
            ELSE FALSE
    ...
    FROM
        {{ ref('rubicon_arbitrum_offers') }} AS actual
    INNER JOIN {{ ref('rubicon_arbitrum_offers_seed') }} AS test
