WITH unit_test AS (
    SELECT
        ...
        CASE 
            WHEN LOWER(CAST(test.sell_amount_raw AS VARCHAR(100))) = LOWER(CAST(actual.sell_amount_raw AS VARCHAR(100))) THEN TRUE
            ELSE FALSE
        END AS sell_amount_raw_test,

        -- test the buy_amount_raw
        CASE 
            WHEN LOWER(CAST(test.buy_amount_raw AS VARCHAR(100))) = LOWER(CAST(actual.buy_amount_raw AS VARCHAR(100))) THEN TRUE
            ELSE FALSE
        ...
        CASE 
            WHEN LOWER(CAST(test.sold_amount_raw AS VARCHAR(100))) = LOWER(CAST(actual.sold_amount_raw AS VARCHAR(100))) THEN TRUE
            ELSE FALSE
        END AS sold_amount_raw_test,

        -- test the bought_amount_raw
        CASE 
            WHEN LOWER(CAST(test.bought_amount_raw AS VARCHAR(100))) = LOWER(CAST(actual.bought_amount_raw AS VARCHAR(100))) THEN TRUE
            ELSE FALSE
