...
WITH unit_tests as
(SELECT case when test_data.original_amount = x2y2_trades.amount_original then True else False end as price_test
FROM {{ ref('x2y2_ethereum_trades') }} x2y2_trades
JOIN {{ ref('x2y2_ethereum_trades_etherscan') }} test_data ON test_data.tx_hash = x2y2_trades.tx_hash
WHERE x2y2_trades.block_time = '2022-07-20' or x2y2_trades.block_time = '2022-06-09'
)
