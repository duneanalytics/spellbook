...
with raw_events AS (
    SELECT evt_block_time as raw_block_time,
          ...
          cast(evt_tx_hash as varchar)|| '-Trade-' || cast(evt_index as varchar) || '-' || cast(erc721TokenId as varchar) || '-1'  AS raw_unique_trade_id
    ...
    SELECT evt_block_time as raw_block_time,
          evt_tx_hash as raw_tx_hash,
          cast(evt_tx_hash as varchar) || '-Trade-' || cast(evt_index as varchar) || '-' || cast(erc1155TokenId as varchar) || '-' || cast(erc1155FillAmount as varchar)  AS raw_unique_trade_id
