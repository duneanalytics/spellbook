...
{% test check_seed(model, seed_file, match_columns=[], check_columns=[], blockchain=None, project=None, version=None) %}
    {#  
        --jinja comment
    ...
    with matched_records as (
        ...
        ON 1=1
            ...
            {% endfor -%}
        where 1=1
            {% if blockchain is not none %} and seed.blockchain = '{{blockchain}}' {%- endif %}
            {% if project is not none %}    and seed.project = '{{project}}' {%- endif %}
            {% if version is not none %}    and seed.version = '{{version}}' {%- endif %}
    ),
