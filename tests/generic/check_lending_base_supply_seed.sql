-- this tests checks a lending base supply model for every row in a seed file.
-- actual implementation in macros/test-helpers/check_seed.sql
{% test check_lending_base_supply_seed(model, seed_file, filter=None) %}

    {%- set seed_check_columns = ['block_number', 'depositor', 'amount'] -%}
