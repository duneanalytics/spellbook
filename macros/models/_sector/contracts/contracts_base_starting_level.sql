  ...
  FROM (
    ...
    from (
          with new_contracts AS (
            ...
            -- find contracts that we need to re-run the reinit counter over
            UNION ALL

            SELECT
              old.blockchain
            , old.trace_creator_address
            , old.contract_address
            , old.created_time
            , old.created_block_number
            , old.created_tx_hash
            , old.top_level_time
            , old.top_level_block_number
            , old.top_level_tx_hash
            , old.top_level_tx_from
            , old.top_level_tx_to
            , old.top_level_tx_method_id
            , old.created_tx_from
            , old.created_tx_to
            , old.created_tx_method_id
            , old.created_tx_index
            , old.code
            , old.code_bytelength
