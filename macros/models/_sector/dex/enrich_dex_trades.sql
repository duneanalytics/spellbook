...
, enrichments AS (
    SELECT
        ...
        , base_trades.token_bought_amount_raw / power(10, if(override_bought_decimals, 18, if(swap_bought_decimals, erc20_sold.decimals, erc20_bought.decimals))) AS token_bought_amount
        , base_trades.token_sold_amount_raw / power(10, if(override_sold_decimals, 18, if(swap_sold_decimals, erc20_bought.decimals, erc20_sold.decimals))) AS token_sold_amount
        , base_trades.token_bought_amount_raw
        , base_trades.token_sold_amount_raw
        , coalesce(
                base_trades.token_bought_amount_raw / power(10, if(override_bought_decimals, 18, if(swap_bought_decimals, erc20_sold.decimals, erc20_bought.decimals))) * p_bought.price,
                base_trades.token_sold_amount_raw / power(10, if(override_sold_decimals, 18, if(swap_sold_decimals, erc20_bought.decimals, erc20_sold.decimals))) * p_sold.price
            ) AS amount_usd
