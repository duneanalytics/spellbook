...
select
        ...
        ,'seaport-' || CAST(tx_hash AS varchar) || '-' || cast(evt_index as VARCHAR(10)) || '-' || CAST(nft_contract_address AS varchar) || '-' || cast(nft_token_id as varchar) || '-' || cast(sub_type as VARCHAR(20)) || '-' || cast(sub_idx as VARCHAR(10)) as unique_trade_id
  from   iv_trades
