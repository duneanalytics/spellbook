...
,iv_base_pairs_priv as (
  ...
  select distinct -- temporary solution to https://github.com/duneanalytics/spellbook/issues/2404
        ...
        ,a.is_moved_nft
  from ref_seaport_base_pairs a
  left join ref_seaport_base_pairs b on b.tx_hash = a.tx_hash
...
,iv_trades as (
  ...
  from iv_nfts a
  inner join source_transactions t on t.hash = a.tx_hash
...
select
  *
  ...
  ,case when amount_raw > uint256 '0' then CAST ((platform_fee_amount_raw / amount_raw * 100) AS DOUBLE) end platform_fee_percentage
  ,case when amount_raw > uint256 '0' then CAST((royalty_fee_amount_raw / amount_raw * 100) AS DOUBLE) end royalty_fee_percentage
  ,'seaport-' || CAST(tx_hash AS varchar) || '-' || cast(evt_index as varchar) || '-' || CAST(nft_contract_address AS varchar) || '-' || cast(token_id as varchar) || '-' || cast(sub_idx as varchar) as unique_trade_id
from iv_columns
{% endmacro %}
