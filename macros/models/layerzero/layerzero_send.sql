...
WITH send_detail AS (
    ...
    FROM {{endpoint_call_send}} s
    INNER JOIN {{ source(blockchain,'transactions') }} t on t.block_number = s.call_block_number
...
destination_gas_detail AS (
    ...
    FROM send_detail s
    INNER JOIN {{ source(blockchain, 'traces') }} e on e.block_number = s.block_number
...
trans_detail AS (
    ...
    FROM (
        ...
        INNER JOIN {{ source('erc20_' + blockchain, 'evt_transfer') }} et on et.evt_block_number = s.block_number
...
LEFT JOIN {{ ref('tokens_erc20') }} erc ON erc.blockchain = '{{blockchain}}' AND erc.contract_address = t.currency_contract
LEFT JOIN {{ source('prices', 'usd') }} p ON p.blockchain = '{{blockchain}}' AND p.contract_address = t.currency_contract
