    ...
    set cfg = {
        ...
        , 'AggregationRouterV5': {
            ...
            , 'methods': {
                ...
                , 'fillOrderToWithPermit': {'order_hash': 'output_2'}
                , 'fillOrderRFQ': {'order_hash': 'output_2'}
                , 'fillOrderRFQTo': {'empty': 'empty'
                    , 'making_amount': 'output_filledMakingAmount'
                    , 'taking_amount': 'output_filledTakingAmount'
                    , 'order_hash': 'output_orderHash'
                }
                , 'fillOrderRFQToWithPermit': {'order_hash': 'output_2'}
...
orders as (
    ...
    {% for contract, contract_data in cfg.items() %}
        {% if blockchain in contract_data.blockchains %}
