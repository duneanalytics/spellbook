...
with t as (
    ...
    select
        ...
        , case
            ...
            when {{ selector }} in ({{ mint_selector }}) then {{ null_address }} -- mint
            else "from" -- transfer, native, withdraw, deposit
        ...
        , case
            ...
            when {{ selector }} in ({{ burn_selector }}, {{ withdraw_selector }}, {{ deposit_selector }}) then {{ null_address }} -- burn, withdraw, deposit
            else "to" -- native
        end as transfer_to
...
, deposit_as_wrapped as (
    select 
        ...
        , value as amount
        , {{ null_address }} as transfer_from
        , "from" as transfer_to
    from t
    where type = 'deposit' and value > uint256 '0'
)
