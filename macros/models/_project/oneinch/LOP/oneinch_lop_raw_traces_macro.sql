...
with decoded_calls as (
    {% for contract, contract_data in oneinch_lop_cfg_contracts_macro().items() if blockchain in contract_data.blockchains %}
        ...
        {% for method, method_data in contract_data.methods.items() if blockchain in method_data.get('blockchains', contract_data.blockchains) %} -- method-level blockchains override contract-level blockchains
            select call_tx_hash as tx_hash, call_block_number as block_number, call_trace_address as trace_address, call_block_time as block_time from {{ source('oneinch_' + blockchain, contract + '_call_' + method) }}
            {% if is_incremental() %}
...
select * from traces 
join decoded_calls using(tx_hash, block_number, trace_address, block_time)
