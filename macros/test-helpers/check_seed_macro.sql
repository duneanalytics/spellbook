...
{% macro check_seed_macro(model, seed_file, seed_matching_columns=[], seed_check_columns=[], filter=None) %}
    ...
    with matched_records as (
        ...
        left join (
            ...
            inner join {{model}} model
                ON 1=1
                    ...
                    AND COALESCE(CAST(split(seed.{{column_name}}, ',') as array<INT256>), ARRAY[]) = model.{{column_name}}
                    {% else %}
        ...
        ON 1=1
            {%- for column_name in seed_matching_columns %}
            {% if column_name == 'trace_address' %}
            AND COALESCE(CAST(split(seed.{{column_name}}, ',') as array<INT256>), ARRAY[]) = model_sample.{{column_name}}
            {% else %}
